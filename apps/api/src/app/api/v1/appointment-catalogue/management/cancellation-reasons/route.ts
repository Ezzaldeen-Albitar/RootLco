/**
 * GET /api/v1/appointment-catalogue/management/cancellation-reasons (P1-27
 * remediation executed by P1-18, `P1-27-INT-018`).
 *
 * The catalogue of cancellation reasons as an ADMINISTRATOR sees it — the read the
 * management commands were shipped without.
 *
 * `GET /appointment-catalogue/cancellation-reasons` is the PICKER read and stays
 ...
 *
 * `apt.appointment-cancel` demands a catalogued `cancellationReasonId` with no
 * free-text escape, so the reason list is a policy statement an operator revises
 * over time — which means seeing the ones they have already withdrawn.
 *
...
export const CANCELLATION_REASON_MANAGEMENT_LIST_OPERATION = defineOperation({
  id: 'apt.catalogue-cancellation-reason-management-list',
  module: 'reception',
  method: 'GET',
  path: '/appointment-catalogue/management/cancellation-reasons',
  summary:
    'List every cancellation reason in the caller tenant catalogue, retired entries included.',
