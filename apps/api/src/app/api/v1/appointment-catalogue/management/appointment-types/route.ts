/**
 * GET /api/v1/appointment-catalogue/management/appointment-types (P1-27
 * remediation executed by P1-18, `P1-27-INT-018`).
 *
 * The catalogue of appointment types as an ADMINISTRATOR sees it — the read the
 * management commands were shipped without.
 *
 * `GET /appointment-catalogue/appointment-types` is the PICKER read and stays
 ...
 *
 * `appointmentTypeId` is a REQUIRED uuid on `apt.appointment-create`, so this
 * is the catalogue an operator must populate before a single appointment can be
 * booked — and the one they are most likely to need to correct afterwards.
 *
...
export const APPOINTMENT_TYPE_MANAGEMENT_LIST_OPERATION = defineOperation({
  id: 'apt.catalogue-appointment-type-management-list',
  module: 'reception',
  method: 'GET',
  path: '/appointment-catalogue/management/appointment-types',
  summary: 'List every appointment type in the caller tenant catalogue, retired entries included.',
