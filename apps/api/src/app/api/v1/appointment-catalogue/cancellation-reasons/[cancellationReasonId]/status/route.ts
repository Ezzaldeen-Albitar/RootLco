/**
 * POST /api/v1/appointment-catalogue/cancellation-reasons/{cancellationReasonId}/status
 * (P1-27 remediation executed by P1-18, `P1-27-INT-018`).
 *
 * Retires or restores one tenant cancellation reason. This is the ONLY removal
 * affordance, and it is deliberate in both directions.
 *
 * ## There is no delete, and there cannot be
 *
 * `app_runtime` holds no DELETE grant on `apt.cancellation_reasons` and no DELETE policy
 * exists, so a hard removal is refused by the database however it is asked for.
 * That is the property that matters: every cancelled appointment names the reason it was cancelled under, and a catalogue entry
 ...
 * being offered in the cancel dialog while every existing reference stays
 * satisfied.
 *
 * ## Why it restores as well as retires
 *
 * `uq_cancellation_reasons_tenant_code` is `(tenant_id, code)
...
export const CANCELLATION_REASON_STATUS_OPERATION = defineOperation({
  id: 'apt.catalogue-cancellation-reason-status-set',
  module: 'reception',
  method: 'POST',
  path: '/appointment-catalogue/cancellation-reasons/{cancellationReasonId}/status',
  summary: 'Retire or restore a cancellation reason in the caller tenant catalogue.',
  permissions: ['apt.catalogue.manage'],
  scope: 'tenant',
  auditClass: 'privileged',
  auditAction: 'apt.cancellation_reason.status_changed',
  idempotent: true,
  versionGuarded: true,
