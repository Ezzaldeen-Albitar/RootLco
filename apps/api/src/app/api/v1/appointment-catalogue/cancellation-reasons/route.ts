/**
 * GET / POST /api/v1/appointment-catalogue/cancellation-reasons (P1-27 remediation executed by
 * P1-18, `P1-27-INT-018`).
 *
 * The POST is the management half. `apt.appointment-cancel` demands a mandatory catalogued `cancellationReasonId` with NO free-text escape, the table ships zero rows by the no-fake-data policy, and no operation anywhere could add one — so no appointment could be cancelled at all. The database has permitted the
 * write since the table was created (`GRANT SELECT, INSERT, UPDATE ... TO
 * app_runtime` plus `ins_cancellation_reasons_tenant`); only the API published nothing.
 *
 * Creating is NOT seeding: no row ships, and the operator decides what their
 * catalogue contains.
 *
 * `apt.cancellation_reasons` is a dual-scope configuration catalogue the intake writes
 * reference by uuid, and nothing published its rows — a cancellation demands a catalogued reason id and no picker could name one, which made the cancel dialog unbuildable.
 *
...
export const CANCELLATION_REASON_LIST_OPERATION = defineOperation({
  id: 'apt.catalogue-cancellation-reason-list',
  module: 'reception',
  method: 'GET',
  path: '/appointment-catalogue/cancellation-reasons',
  summary: 'List the active appointment cancellation reasons visible to the caller tenant.',
...
export async function GET(request: Request): Promise<Response> {
  return handleOperation(
    ...
    async ({ db, request: raw }) => ({
      body: await receptionModule().intakeCatalogues.listCancellationReasons(
        db,
...
export const CANCELLATION_REASON_CREATE_OPERATION = defineOperation({
  ...
  method: 'POST',
  path: '/appointment-catalogue/cancellation-reasons',
  summary: 'Add a cancellation reason to the caller tenant catalogue.',
  permissions: ['apt.catalogue.manage'],
  scope: 'tenant',
  auditClass: 'privileged',
  auditAction: 'apt.cancellation_reason.created',
  idempotent: true,
