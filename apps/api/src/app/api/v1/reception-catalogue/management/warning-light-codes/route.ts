/**
 * GET /api/v1/reception-catalogue/management/warning-light-codes (P1-27
 * remediation executed by P1-18, `P1-27-INT-018`).
 *
 * The catalogue of warning-light codes as an ADMINISTRATOR sees it — the read the
 * management commands were shipped without.
 *
 * `GET /reception-catalogue/warning-light-codes` is the PICKER read and stays
 ...
 *
 * The `warning_light` evidence kind requires a code id (RMC-11), and the code
 * set grows model by model, so this is the catalogue most often edited after it
 * was first populated.
 *
...
export const WARNING_LIGHT_CODE_MANAGEMENT_LIST_OPERATION = defineOperation({
  id: 'rec.catalogue-warning-light-code-management-list',
  module: 'reception',
  method: 'GET',
  path: '/reception-catalogue/management/warning-light-codes',
  summary:
    'List every warning-light code in the caller tenant catalogue, retired entries included.',
