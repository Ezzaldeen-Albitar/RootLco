/**
 * GET /api/v1/reception-catalogue/management/refusal-reasons (P1-27
 * remediation executed by P1-18, `P1-27-INT-018`).
 *
 * The catalogue of refusal reasons as an ADMINISTRATOR sees it — the read the
 * management commands were shipped without.
 *
 * `GET /reception-catalogue/refusal-reasons` is the PICKER read and stays
 ...
 *
 * A recorded refusal is preserved as its own fact and keeps naming the reason it
 * was recorded under, so the list only ever grows and shrinks by status.
 *
...
export const REFUSAL_REASON_MANAGEMENT_LIST_OPERATION = defineOperation({
  id: 'rec.catalogue-refusal-reason-management-list',
  module: 'reception',
  method: 'GET',
  path: '/reception-catalogue/management/refusal-reasons',
  summary: 'List every refusal reason in the caller tenant catalogue, retired entries included.',
