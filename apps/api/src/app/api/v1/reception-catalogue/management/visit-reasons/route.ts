/**
 * GET /api/v1/reception-catalogue/management/visit-reasons (P1-27
 * remediation executed by P1-18, `P1-27-INT-018`).
 *
 * The catalogue of visit reasons as an ADMINISTRATOR sees it — the read the
 * management commands were shipped without.
 *
 * `GET /reception-catalogue/visit-reasons` is the PICKER read and stays
 ...
 *
 * `rec.visit_reason_links` records why each vehicle came in, so the reason list
 * is the vocabulary every past visit is described in and entries leave it by
 * being retired, never removed.
 *
...
export const VISIT_REASON_MANAGEMENT_LIST_OPERATION = defineOperation({
  id: 'rec.catalogue-visit-reason-management-list',
  module: 'reception',
  method: 'GET',
  path: '/reception-catalogue/management/visit-reasons',
  summary: 'List every visit reason in the caller tenant catalogue, retired entries included.',
