...
export default async function AcknowledgementPage({
  ...
  const [parties, authorizations, evidence] = await Promise.all([
    ...
    // The receiving employee is NOT read here. `detail` already carries
    // `receivingEmployeeDisplayName`, the snapshot written when custody was
    // accepted, so this sheet costs one request fewer AND cannot reprint a past
    // handover under a name the account acquired afterwards.
  ]);
