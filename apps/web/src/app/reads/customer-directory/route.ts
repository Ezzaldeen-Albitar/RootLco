...
/**
 * POST /reads/customer-directory — the customer search, cancellable (P1-32-PRE-OD-READ).
 *
...
export function POST(request: Request): Promise<Response> {
  return serveBrowserRead(request, {
    input: 'body',
    schema: customerDirectoryQuery,
    run: (params, signal) => readCustomerDirectory(...customerDirectoryArgs(params), signal),
    failure: pageFailure,
  });
}
