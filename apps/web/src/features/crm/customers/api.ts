 ...
 * `expensive-read`. See `contract.ts` for what the operation does and does not
 * accept; this file only turns criteria into that request and the response into
 * a view state.
 *
 ...
 * 422 rather than a differently-ordered page. The results table therefore
 * declares no sortable column, and this adapter has nowhere to read one from.
 */
 ...
 *
 * The screen must not call the backend before the operator expresses intent —
 * partly because an unasked query is a wasted request against a 30-per-minute
 * budget, and partly because "here is everything" is not what a search screen
...
export async function searchCustomers(
  ...
  if (isEmptyCriteria(criteria)) {
    // Not an error and not an empty result — the screen renders its "how to
  ...
  return {
    ...
    // is what makes the table render Previous/Next without a range instead of a
    // number it made up.
  };
}
