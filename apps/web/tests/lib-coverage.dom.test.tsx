/**
 * The three files the P1-28 promotion measured as under-covered, brought to
 * 100% — and the reason two cases from the proven set are absent.
 *
 * These were written and measured during the P1-28 promotion and DEFERRED, not
 * skipped: the accepted product tree was frozen, and a test that changes a
 * frozen tree is not a test of it. P1-28 is closed and promoted, so they land
 * here.
 *
 * They were re-measured against the current code before landing rather than
 * trusted: 16 passed, and 100% of statements, branches, functions and lines on
 * all three files.
 *
 * TWO CASES ARE GONE, and deliberately. They covered `readTenantHint` and
 * `writeTenantHint`, which had no caller anywhere in the repository. P1-26
 * removed the Workspace field, its label, its hint, its UUID validation and
 * every translation key behind it, and recorded the helpers as a carried
 * follow-up left in place only because that file was being edited concurrently.
 * The PRE-P1-29 rule that a normal employee is never asked for a tenant
 * identifier makes the removal permanent, so the helpers went with it.
 *
 * Covering them instead would have been the worse repair: it would have pinned
 * a tenant-UUID mechanism into the suite as though something depended on it,
 * and left the file's own docblock — "No tenant, company, branch, permission
 * set or display name" — contradicted fifteen lines below by a tenant cookie.
 */
...
describe('sessionCookieAttributes — a security attribute that fails closed', () => {
  it('turns `secure` off for exactly one value, and on for everything else', () => {
    ...
    for (const env of ['', 'Local', 'LOCAL', 'locale', 'production', 'staging', 'undefined']) {
      expect(
        sessionCookieAttributes(env).secure,
        `${env || '(empty)'} produced an insecure cookie`
      ).toBe(true);
    }
  });
