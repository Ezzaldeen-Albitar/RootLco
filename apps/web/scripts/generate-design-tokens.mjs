 ...
 *
 * Colours are deliberately NOT emitted. A colour read as a JavaScript string
 * is a raw hex value: it cannot follow a `[data-theme]` remap, so the Material
 * theme references `var(--color-…)` instead, like `tailwind.config.ts` does.
 *
 * ## Committed, with a drift check
 *
 * The module is committed, as the repository commits its other generated
 * source that application code imports — `src/lib/api/idempotent-operations.ts`,
 * produced by `scripts/ci/generate-idempotent-operations.mjs` and held to the
 * contract by `--check` in `validate:idempotent-operations`. Type-checking and
 * linting then read the same file the build does, with no pre-step to forget.
 *
 * `--check` writes nothing and exits 1 when the committed module differs from
 * what the Sass maps produce today. It runs in `validate:web-tokens`, and
 * `tests/design-tokens-generated.test.ts` runs the same comparison in the web
 * test tier, so an edit to a `.scss` token that is not followed by a
 * regeneration fails CI rather than drifting.
 *
 * Usage (from apps/web):
