...
- The current phase is **Phase 1-3: Organization-Structure Schema**. Phase 1-1 closed with a recorded owner **Go** on 2026-07-16 ([gate record](docs/phase-1/phase-1-1/phase-1-1-owner-gate.md)); **Phase 1-2 closed with Go — Technical Gate Passed on 2026-07-17** ([gate record](docs/phase-1/phase-1-2/phase-1-2-owner-gate.md)), which authorizes Phase 1-3.
- Phase 1-2 established standards and the shared foundation only, and **no business-domain tables** (tenants, companies, branches, users, customers, vehicles, appointments, inspections, quotations, work orders, inventory, invoices, payments) were created during it. Phase 1-3 introduces the `org` tables (tenants, companies, branches) and the composite scope foreign keys; every other business-domain table remains out of scope until its own phase.
- The Phase 1-2 standards under `docs/database/` are **binding** on all subsequent database work: naming, migrations, seeds, RLS, roles and grants, transactions and concurrency, retention, extensions, number sequences, and test fixtures.
