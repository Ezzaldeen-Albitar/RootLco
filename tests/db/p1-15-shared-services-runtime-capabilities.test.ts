...
describe('P1-15 / global security posture', () => {
  it('the repository declares exactly 151 migrations, with the newest twenty-two named', () => {
    ...
    // again rather than slid; twenty-two and 151 with slice 3c (material draw
    // enforcement), widened again rather than slid.
    expect(files).toHaveLength(151);
    expect(files.at(-22)).toBe('20260831091000_org_tenant_status_transition_guard.sql');
    expect(files.at(-21)).toBe('20260831092000_org_tenant_status_history_emission.sql');
    expect(files.at(-20)).toBe('20260831093000_iam_platform_privilege_graph.sql');
    ...
    expect(files.at(-19)).toBe('20260901090000_org_company_status_lifecycle.sql');
    ...
    expect(files.at(-18)).toBe('20260901100000_wo_jobs_department_routing.sql');
    ...
    expect(files.at(-17)).toBe('20260902120000_wo_job_blocker_events.sql');
    ...
    expect(files.at(-16)).toBe('20260902130000_iam_platform_bootstrap_catalogue_and_backstop.sql');
    ...
    expect(files.at(-15)).toBe('20260906090000_sal_payment_method_tenant_bootstrap.sql');
    ...
    expect(files.at(-14)).toBe('20260907090000_inv_opening_movement_cell_uniqueness.sql');
    ...
    expect(files.at(-13)).toBe('20260909090000_sal_complete_delivery_active_template_gate.sql');
    ...
    expect(files.at(-12)).toBe('20260910090000_org_employees.sql');
    ...
    expect(files.at(-11)).toBe('20260910091000_sal_delivery_delivering_employee_identity.sql');
    ...
    // 151 is P1-32 preparatory slice 3c: every work-order reservation and part issue
