...
describe('P1-15 / global security posture', () => {
  ...
  it('both new permission codes exist exactly once and the catalog totals 61', async () => {
    ...
    // veh.vehicle.relationship.manage, and veh.vehicle.odometer.record, taking the
    // catalog to 61. Permissions live in the seed, not a migration.
    const total = await admin.query<{ n: string }>(
      `SELECT count(*)::text AS n FROM iam.permissions`
    );
    expect(Number(total.rows[0]?.n)).toBe(61);
  });
