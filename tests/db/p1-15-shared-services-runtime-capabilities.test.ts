...
describe('P1-15 / global security posture', () => {
  it('the repository declares exactly 150 migrations, with the newest twenty-one named', () => {
    ...
    // twenty-one and 150 with P1-32 preparatory slice 3a (unit conversions, vehicle
    // specifications, material requirements, truthful transfer receipts), widened
    // again rather than slid.
    expect(files).toHaveLength(150);
    expect(files.at(-21)).toBe('20260831091000_org_tenant_status_transition_guard.sql');
    expect(files.at(-20)).toBe('20260831092000_org_tenant_status_history_emission.sql');
    expect(files.at(-19)).toBe('20260831093000_iam_platform_privilege_graph.sql');
    ...
    expect(files.at(-18)).toBe('20260901090000_org_company_status_lifecycle.sql');
    ...
    expect(files.at(-17)).toBe('20260901100000_wo_jobs_department_routing.sql');
    ...
    expect(files.at(-16)).toBe('20260902120000_wo_job_blocker_events.sql');
    ...
    expect(files.at(-15)).toBe('20260902130000_iam_platform_bootstrap_catalogue_and_backstop.sql');
    ...
    expect(files.at(-14)).toBe('20260906090000_sal_payment_method_tenant_bootstrap.sql');
    ...
    expect(files.at(-13)).toBe('20260907090000_inv_opening_movement_cell_uniqueness.sql');
    ...
    expect(files.at(-12)).toBe('20260909090000_sal_complete_delivery_active_template_gate.sql');
    ...
    expect(files.at(-11)).toBe('20260910090000_org_employees.sql');
    ...
    expect(files.at(-10)).toBe('20260910091000_sal_delivery_delivering_employee_identity.sql');
    ...
    expect(files.at(-9)).toBe('20260917090000_inv_transfers_receipts_counts.sql');
    ...
    expect(files.at(-8)).toBe('20260917091000_inv_item_identifiers.sql');
    ...
    expect(files.at(-7)).toBe('20260917092000_inv_item_sale_prices.sql');
    ...
    expect(files.at(-6)).toBe('20260917093000_sal_counter_sales.sql');
    ...
    expect(files.at(-5)).toBe('20260917094000_inv_sales_returns.sql');
    // 147 is P1-32 preparatory slice 3a: inv.item_unit_conversions, exact factors in
    // one direction with no implied reverse and no rounding.
    expect(files.at(-4)).toBe('20260917095000_inv_item_unit_conversions.sql');
    // 148 is inv.vehicle_fluid_specifications: a sourced capacity that resolves only
    // once confirmed, and never as zero.
    expect(files.at(-3)).toBe('20260917096000_inv_vehicle_fluid_specifications.sql');
    // 149 is material demand control: approved requirements, finite two-person
    // exceptions, material requests and their fulfillment links, bounded under the
    // requirement row lock.
    expect(files.at(-2)).toBe('20260917097000_inv_material_requirements.sql');
    // 150 is the truthful transfer receipt: a short delivery stays in transit until a
    ...
    // unchanged by them.
    expect(files.at(-1)).toBe('20260917098000_inv_transfer_partial_receipt.sql');
  });
