...
describe('P1-15 / global security posture', () => {
  it('the repository declares exactly 148 migrations, with the newest nineteen named', () => {
    ...
    // tree instead: nineteen names below, 148 files. The platform console trio
    // (090000..092000) sorts before the capacity migration (093000) and the
    // friendly-search pair (094000, 095000), so the merge of the two branches
    // places them in that filename order.
    expect(files).toHaveLength(148);
    expect(files.at(-19)).toBe('20260831091000_org_tenant_status_transition_guard.sql');
    expect(files.at(-18)).toBe('20260831092000_org_tenant_status_history_emission.sql');
    expect(files.at(-17)).toBe('20260831093000_iam_platform_privilege_graph.sql');
    ...
    expect(files.at(-16)).toBe('20260901090000_org_company_status_lifecycle.sql');
    ...
    expect(files.at(-15)).toBe('20260901100000_wo_jobs_department_routing.sql');
    ...
    expect(files.at(-14)).toBe('20260902120000_wo_job_blocker_events.sql');
    ...
    expect(files.at(-13)).toBe('20260902130000_iam_platform_bootstrap_catalogue_and_backstop.sql');
    ...
    expect(files.at(-12)).toBe('20260906090000_sal_payment_method_tenant_bootstrap.sql');
    ...
    expect(files.at(-11)).toBe('20260907090000_inv_opening_movement_cell_uniqueness.sql');
    ...
    expect(files.at(-10)).toBe('20260909090000_sal_complete_delivery_active_template_gate.sql');
    ...
    expect(files.at(-9)).toBe('20260910090000_org_employees.sql');
    ...
    expect(files.at(-8)).toBe('20260910091000_sal_delivery_delivering_employee_identity.sql');
    ...
    expect(files.at(-7)).toBe('20260916090000_platform_console_read_graph.sql');
    expect(files.at(-6)).toBe('20260916091000_org_subscription_commerce.sql');
    expect(files.at(-5)).toBe('20260916092000_org_subscription_billing.sql');
    ...
    expect(files.at(-4)).toBe('20260916093000_org_capacity_enforcement.sql');
    ...
    expect(files.at(-3)).toBe('20260916094000_shared_text_folding.sql');
    ...
    expect(files.at(-2)).toBe('20260916095000_search_expression_indexes.sql');
    ...
    expect(files.at(-1)).toBe('20260916096000_platform_organization_growth.sql');
  });
