...
describe('P1-15 / global security posture', () => {
  it('the repository declares exactly 154 migrations, with the newest twenty-five named', () => {
    ...
    expect(files.at(-25)).toBe('20260831091000_org_tenant_status_transition_guard.sql');
    expect(files.at(-24)).toBe('20260831092000_org_tenant_status_history_emission.sql');
    expect(files.at(-23)).toBe('20260831093000_iam_platform_privilege_graph.sql');
    ...
    expect(files.at(-22)).toBe('20260901090000_org_company_status_lifecycle.sql');
    ...
    expect(files.at(-21)).toBe('20260901100000_wo_jobs_department_routing.sql');
    ...
    expect(files.at(-20)).toBe('20260902120000_wo_job_blocker_events.sql');
    ...
    expect(files.at(-19)).toBe('20260902130000_iam_platform_bootstrap_catalogue_and_backstop.sql');
    ...
    expect(files.at(-18)).toBe('20260906090000_sal_payment_method_tenant_bootstrap.sql');
    ...
    expect(files.at(-17)).toBe('20260907090000_inv_opening_movement_cell_uniqueness.sql');
    ...
    expect(files.at(-16)).toBe('20260909090000_sal_complete_delivery_active_template_gate.sql');
    ...
    expect(files.at(-15)).toBe('20260910090000_org_employees.sql');
    ...
    expect(files.at(-14)).toBe('20260910091000_sal_delivery_delivering_employee_identity.sql');
    // 142 is the Owner directive capacity enforcement: org.capacity_limit,
    ...
    expect(files.at(-13)).toBe('20260916093000_org_capacity_enforcement.sql');
    // 143 is P1-32 friendly search: shared.fold_digits and shared.fold_search_text,
    ...
    expect(files.at(-12)).toBe('20260916094000_shared_text_folding.sql');
    // 144 is the index half of that slice: seven expression indexes, six of them
    // GIN trigram, so the widened contains-matches are index-eligible. Indexes only
    // — nothing in `shared` moves.
    expect(files.at(-11)).toBe('20260916095000_search_expression_indexes.sql');
    // 145 is the P1-32 preparatory inventory slice: stock transfers with a
    ...
    // 146 is P1-32 preparatory slice 2: inv.item_identifiers, its normalisation and
    ...
    // 147 is the item selling price: inv.item_sale_prices with its resolver and its
    ...
    // 148 is the counter sale: sal.invoices.sale_kind, the inventory columns on
    ...
    // 149 is the sales return: inv.sales_returns, a ceiling that counts the legacy
    ...
    // 153 is the truthful transfer receipt: a short delivery stays in transit until a
    ...
    // 154 is P1-32 preparatory slice 3c: every work-order reservation and part issue
