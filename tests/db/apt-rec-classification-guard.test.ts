/**
 * Phase 1-8 classification-guard negative fixtures (P1-08-DO-001 / SEC-003).
 *
 * Spawns the ONE canonical validator (scripts/check-aptrec-classification.mjs —
 * the same implementation CI runs) and proves it actually fails on each tampered
 * registry: a restricted column flipped searchable, a missing live column, a
 * stale entry, an unknown classification, and a data-type drift. The untampered
 * committed registry must pass (exit 0).
 */
...
const SCRIPT = join(ROOT, 'scripts', 'check-aptrec-classification.mjs');
const REAL = join(ROOT, 'docs', 'database', 'apt-rec-personal-data-classification.json');

const tmp = mkdtempSync(join(tmpdir(), 'aptrec-class-'));
afterAll(() => {
...
function runWith(registryPath?: string): { code: number; output: string } {
  try {
    const output = execFileSync(process.execPath, [SCRIPT], {
      ...
      encoding: 'utf8',
      env: registryPath
        ? { ...process.env, APTREC_CLASSIFICATION_REGISTRY: registryPath }
        : { ...process.env },
    });
...
describe('apt/rec classification guard — negative fixtures (canonical script)', () => {
  ...
  it('fails when a restricted column is flipped searchable', () => {
    const p = tampered('restricted-searchable', (d) => {
      const col = d.columns['rec.complaint_details.complaint_text'];
  ...
  it('fails when a live column has no entry (missing coverage)', () => {
    const p = tampered('missing-column', (d) => {
      delete d.columns['rec.reception_visits.reception_status'];
    });
    const r = runWith(p);
    expect(r.code).toBe(1);
  ...
  it('fails on a stale entry for a non-existent column', () => {
    const p = tampered('stale-entry', (d) => {
      d.columns['rec.reception_visits.ghost_column'] = {
  ...
  it('fails on an unknown classification value', () => {
    const p = tampered('bad-class', (d) => {
      const col = d.columns['apt.appointments.lifecycle_status'];
      if (!col) throw new Error('fixture column missing');
  ...
  it('fails on registry/live data-type drift', () => {
    const p = tampered('type-drift', (d) => {
      const col = d.columns['rec.reception_visits.ev_soc_percent'];
      if (!col) throw new Error('fixture column missing');
      col.dataType = 'text';
    });
    const r = runWith(p);
