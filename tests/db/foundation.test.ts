...
const ALLOWED_TABLES = new Set([
  // Phase 1-9 Work Order / Technician / Diagnostics / Quality (wo, tech, dia, qms).
  ...
  // Phase 1-7 vehicle — plate/ownership/relationships/evidence (P1-07-DB-010/011/012/013).
  'veh.plate_history',
  'veh.ownership_history',
  'veh.vehicle_relationships',
  'veh.relationship_evidence',
  // Phase 1-7 vehicle — odometer/status/alerts/duplicates/merges (P1-07-DB-014..018).
...
const ALLOWED_ROUTINES = new Set([
  // Phase 1-9 wo/tech/dia/qms functions.
  ...
  // Phase 1-7 vehicle — plate/ownership/relationship resolvers + scope validator
  // (P1-07-DB-010/011/012).
  'veh.plate_at',
  'veh.owner_at',
  'veh.relationships_at',
  'veh.valid_authorization_scope',
  // Phase 1-7 vehicle — odometer stamp/guard + resolvers (P1-07-DB-014).
  'veh.stamp_odometer_reading',
  'veh.guard_odometer_reading',
  'veh.latest_odometer',
  'veh.odometer_at',
  // Phase 1-7 vehicle — status-history coherence guard + emitter (P1-07-DB-015).
  'veh.guard_status_history_coherence',
  'veh.emit_vehicle_status_history',
  // Phase 1-7 vehicle — duplicate/merge jsonb validators, merge primitive, resolver
  // (P1-07-DB-017/018).
...
describe('database foundation', () => {
  ...
  it('has all eleven module schemas', async () => {
    const { rows } = await admin.query(
      `SELECT nspname FROM pg_namespace
       WHERE nspname IN ('apt','org','iam','shared','crm','rec','veh','wo','dia','tech','qms') ORDER BY nspname`
    );
  ...
  it('module-schema tables carry EXACTLY the approved triggers and policies', async () => {
    const triggers = await admin.query(
      `SELECT t.tgname FROM pg_trigger t
       ...
       WHERE n.nspname IN ('apt','org','iam','shared','crm','rec','veh','wo','dia','tech','qms') AND NOT t.tgisinternal
       ORDER BY 1`
    );
