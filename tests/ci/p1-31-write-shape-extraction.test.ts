/**
 * Turns the P1-31 in-scope request bodies into canonical JSON Schema.
 *
 * Runs under vitest because that is the only place the route modules can be
 * imported with their `@/` aliases resolved; `check-p1-31-write-shape.mjs`
 * shells out to this file with `P1_31_OPERATIONS` (the rows to convert) and
 * `P1_31_SCHEMAS` (where to write them). Run bare, it converts every in-scope
 * body it can find and asserts none is missing — which is itself a check that
 * every `wty`/`rpt` write exports the schema its handler parses.
 *
 * A sibling of `p1-29-payload-extraction.test.ts` and
 * `p1-30-payload-extraction.test.ts` with P1-31's scope; the schema locator is
 * the P1-29 gate's own.
 */
...
import { P1_31_WRITE_DOMAINS } from '../../scripts/ci/check-p1-31-write-shape.mjs';
...
function census(): Row[] {
  ...
  const inScope = new RegExp(`^(${P1_31_WRITE_DOMAINS.join('|')})\\.`);
  const rows: Row[] = [];
...
describe('P1-31 request payload extraction', () => {
  it('converts every declared body to canonical JSON Schema', () => {
    const inPath = process.env.P1_31_OPERATIONS;
    const outPath = process.env.P1_31_SCHEMAS;
    const rows: Row[] =
