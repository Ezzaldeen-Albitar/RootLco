...
describe('the API application lives in the workspace', () => {
  ...
  it('discovers the whole route surface, and would notice one missing', () => {
    ...
    // 322 with the Owner directive organisation administration: three operations
    ...
    expect(routeFiles.length).toBe(322);
  ...
  it('discovers the same 416 operations from the root, apps/api and apps/web', () => {
    ...
    // 416 with the Owner directive organisation administration: three operations
    // over one new module, for the reason stated above the route-module count.
    expect(report.operations).toHaveLength(416);
