...
describe('the API application lives in the workspace', () => {
  ...
  it('discovers the whole route surface, and would notice one missing', () => {
    ...
    expect(routeFiles.length).toBe(305);
  ...
  it('discovers the same 389 operations from the root, apps/api and apps/web', () => {
    ...
    // operations over five new modules, for the reason stated above the
    // route-module count.
    expect(report.operations).toHaveLength(389);
