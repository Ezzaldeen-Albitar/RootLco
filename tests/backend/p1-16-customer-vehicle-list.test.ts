 ...
 *  3. **The two lists over the same table refuse each other's cursors.** This
 *     list and the vehicle-centric one paginate the SAME relation by the SAME
 *     sort; only distinct ordering-contract keys stop a replayed cursor from
...
beforeAll(async () => {
  ...
  // Both codes exist in the seeded catalog (04_iam_permission_catalog.sql);
  ...
  await admin.query(
    ...
    [CUSTOMER_MAIN, CUSTOMER_PAGER, CUSTOMER_B, TENANT_A, USER_A, TENANT_B]
  );
...
describe('the parent is resolved first (cross-tenant)', () => {
  it.each([
    ['a customer in another tenant', CUSTOMER_MAIN],
    ...
    expect(((await response.json()) as PageBody).code).toBe('ERR-RES-001');
  });
...
describe('pagination', () => {
  ...
  it('refuses a cursor issued for a different list over the same relation', async () => {
    authAs(SUBJ_FULL);
    const vehicles = (await (await list(CUSTOMER_PAGER, '?limit=2')).json()) as PageBody;
    // This list and veh.vehicle-relationship-list paginate the SAME table by
    // the SAME sort; only the distinct ordering-contract key makes a replayed
    // cursor fail loudly instead of producing a plausible wrong page. The
    // contacts list is the in-suite stand-in for "any other list".
