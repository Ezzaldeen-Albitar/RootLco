...
**Counts, measured not assumed.** The repository publishes **352** operations at this head — counted three ways: `defineOperation(` call sites under `apps/api/src/app/api/v1`, unique `operationId` values in `docs/api/openapi.v1.json`, and `tests/ci/repository-paths.test.ts` ("discovers the same 352 operations from the root, `apps/api` and `apps/web`"). Any figure near 345 is the pre-P1-29 total and must not be reused. Of those 352, 49 are commercial operations — `svc` 11, `quo` 6, `inv` 14, `sal` 18 —
...
| S-02 | `svc.service-category-list` + `svc.service-category-create`               | **C** | the row shape and its projection exist (`findCategory`, single-row); the list wrapper and a writer do not. Without the writer `svc.service-create` cannot be called at all. **Corrected to C by Owner decision 2026-09-04:** publishing this needs a new ordering contract, a new SQL interface and mapper, a rewritten projection, a new repository method, a new view type and a new service method — "a new read model", which is C under this document's own definition at the head of §6                                                                                                                                                                                                                                                             |
| S-03 | `svc.service-version-create` (draft)                                      | C     | `svc.service-version-publish` needs a draft nothing can create; without it no API-created service is ever sellable and every quotation line is refused. **Contract pinned by Owner decision 2026-09-04: `idempotent: true`, `versionGuarded: false`.** Creating the next draft is not guarded by a version token it does not yet have, and two of the three live "create the next draft version" operations agree — `dia.template-version-create` (the structural twin, from the immediately preceding programme) and `shared.template-version-create`. Concurrency protection stays where the existing contract already puts it: on publication and on later mutation                                                                                    |
| S-04 | price-list **assignment** write                                           | C     | `svc.resolve_price` requires an active assignment row; nothing writes that table, so every price resolution fails. **Shape settled by Owner decision 2026-09-04: top-level `POST /price-list-assignments`, permission `svc.price.manage`.** `uq_price_list_assignments_signature` keys on `(tenant_id, company_id, branch_id, customer_class, priority)` and deliberately NOT on `price_list_id`, so a nested `/price-lists/{id}/assignments` would misrepresent the invariant: a create could be refused by an active row belonging to a different price list, which the caller can neither see nor read. `svc.price.publish` is refused as the code — this is assignment management, not publication, and `publish` is exercised only tenant-wide today |
...
| S-17 | opening batches by branch, and batch detail | C     | `inv.opening-batch-list`, `GET /opening-inventory-batches` (company and branch REQUIRED, keyset, `status` filter) and `inv.opening-batch-read`, `GET /opening-inventory-batches/{batchId}` (parent-addressed, 404 before any scope decision), both on `inv.stock.read`. **Added 2026-09-07, after A0.** A0 measured only the opening-batch WRITE surface (§4) and never asked whether the batch could be read back: it is write-only on the wire — three POSTs, no GET — so a reload, a sign-out, or the second person the maker-and-checker rule requires had no path to a draft. Class C: a new read model (one ordering contract, two row shapes, three view types, three repository methods), not an unrouted service method |

Two of the ten read seams A0 listed are class **A** — `sal.work-order-invoice-read` (S-10) and
...
**S-17 was not one of the ten.** It was found after A2 closed, by asking a question A0 never put to
the opening batch: can what was written be read back? The answer was no, and the consequence was not
a missing convenience but an unsatisfiable rule — `ck_opening_inventory_batches_maker` requires a
second person to approve, and that person had no way to reach a draft. Recorded here rather than
quietly shipped, because a seam this lane's own record does not name is the omission §6.2 exists to
prevent.
