...
| `inv.opening_inventory_batches`              | inventory       | `inv.opening-batch-create` · POST `/opening-inventory-batches`                         | `inv.stock.operate` branch                     | yes    | `/inventory/opening-stock` (#323)                        | 0        | no stock can be counted in                                              | item, location                       | closed (#323)              | none                                                                 |
| `inv.opening_inventory_lines`                | inventory       | `inv.opening-batch-line-create` · POST `…/{batchId}/lines`                             | `inv.stock.operate` branch                     | yes    | the same screen (#323)                                   | 0        | —                                                                       | batch, item, location                | closed (#323)              | none                                                                 |
| `inv.stock_movements` / `inv.stock_balances` | inventory       | `inv.opening-batch-approve` · POST `…/{batchId}/approval` (the only path from nothing) | `inv.adjustment.approve` branch                | yes    | the same screen's approval (#323)                        | 0        | maker–checker needs a second person (proved: 409 for the counter)       | batch, a second user                 | closed (#322 code, #323)   | none                                                                 |
...
| 5     | inventory setup screen: categories, units, items, locations; opening-stock entry (batch, lines, approval) | delivered (#323)       | Frontend P1-30 corrective   | PR #323, `develop` `ea8c0666`                |
