...
- **W9-R4 — RESOLVED (Owner decision of 2026-09-03; `20260903090000_dia_diagnostic_types_platform_vocabulary.sql`; exercised on the fresh tenant `rootlco_w7`, see `w9-acceptance-record.md`). As recorded when open:** a fresh organization holds no `dia.diagnostic_types` row — no seed ships and no route creates one (BR-04 DoD item OPEN since 2026-08) — so W7 cannot run for a real workshop and the canonical closure condition §3 cannot be met by code. Disposition requested: approve the initial diagnostic-type vocabulary and its delivery (seed migration or administration route). The QC check vocabulary is unseeded the same way (observation O6).
