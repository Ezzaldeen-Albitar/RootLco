...
- **19** CRM test files · **151** CRM test cases (all passing).
...
- The authoritative pass/fail signal is the CI **Database migrations and RLS
  tests** job, which runs the complete `test:db` suite (54 DB test files).
...
| [`crm-partner-identifiers.test.ts`](../../../tests/db/crm-partner-identifiers.test.ts)       |    13 | Identifier uniqueness (partial index), restricted classification, sensitive-view gate on raw/normalized values.                                                                                                                                  |
...
| [`crm-communication-timeline.test.ts`](../../../tests/db/crm-communication-timeline.test.ts) |     6 | Communication log; append-only timeline written only through `emit_timeline_event`.                                                                                                                                                              |
...
| [`crm-isolation.test.ts`](../../../tests/db/crm-isolation.test.ts)                           |     5 | **QA-006** — two-tenant isolation across all 21 CRM tables; auto-fails if a new CRM table lacks coverage.                                                                                                                                        |
...
| [`crm-security-hardening.test.ts`](../../../tests/db/crm-security-hardening.test.ts)         |     6 | **SEC-002/004** — cannot create a partner already blocked/merged; jsonb raw-value keys rejected at any depth, case-insensitively.                                                                                                                |
...
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`foundation.test.ts`](../../../tests/db/foundation.test.ts)     | Asserts the **exact** live inventory — any unexpected table, function, trigger, or policy fails the build. The 21 CRM tables, 12 functions, and their triggers/policies are in the allow-lists. |
