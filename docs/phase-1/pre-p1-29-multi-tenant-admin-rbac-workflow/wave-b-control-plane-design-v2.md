...
| Read privilege       | `GRANT SELECT ON org.tenants TO app_platform` under a policy satisfiable by **any** platform authority — deliberately not §6.5's read-code policy. The three codes are separable by design (§4.2), so a lifecycle-only holder must still be able to read the row it is about to change, or the operation answers `no_data_found` instead of acting                                                                                                                                                                                                                                                                                                                                  |
...
| Refusal shape        | The route resolves and validates the target per §11 **before** calling the function, so an absent or unreachable tenant answers the non-disclosing refusal rather than the function's `no_data_found`                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Audit                | §7                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| P-30  | Call the lifecycle operation holding **only** `platform.organization.lifecycle`                                                           | Succeeds — proving §6.4’s read privilege is not bound to the read code                                                                                                                                                     |
...
each carry a reads row. Revision 3 has been written; it has not been attacked.
