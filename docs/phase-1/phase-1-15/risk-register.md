...
| R-11 | `validate:seed-state` fails when run **after** `test:db`                        | Certain    | Low    | A **Phase 1-5** suite, `tests/db/shared-retention.test.ts`, overwrites three retention periods for its eligibility tests and does not restore them. `.github/workflows/ci.yml` runs the seed assertion at line 242, before `test:db` at line 290, so hosted CI always measures an untouched database    | Any restoration in that suite. P1-15 does not fix it: editing another phase's test to make its own run look cleaner is exactly what the review policy exists to prevent                                                                                                                                                             |
...
| R-13 | **P1-15-SR-014** — a period-resetting sequence could re-issue a number          | **Closed** | Medium | **Closed by migration 118 (DBCR-P1-15-002).** The allocator reads `clock_timestamp()` instead of `now()`, and the regression guard refuses any `current_period` that is not the clock's current key                                                                                                     | Nothing outstanding. Reproduced on protected `develop` before the fix and regression-locked by 19 database and 6 application proofs                                                                                                                                                                                                 |
| R-14 | The four unauthenticated auth routes hold only a coarse, shared throttle bucket | Certain    | Medium | Each declares `auth-adjacent` (10/min, security-relevant) and — since PMR-001 — resolves to it; since **PMR-006** the pipeline no longer skips it for want of an address, so the limit is enforced on one shared bucket. The authentication provider also enforces its own lockout on repeated failures | A resolved client address. `resolveClientAddress()` returns null unless the platform supplies a peer address or `TRUSTED_PROXY_IPS` names a proxy, and neither holds, so the ten-a-minute budget is global rather than per client and a hostile caller can deny logins. Closing it is infrastructure plumbing, not application code |
| R-15 | A keyset cursor can skip a row inside one millisecond                           | Possible   | Medium | Every page runs under RLS, so the failure mode is completeness and never disclosure; the cursor is bound to its ordering contract and tie-broken by `id`                                                                                                                                                | Microsecond fidelity. The cursor carries an ISO string with millisecond precision while `timestamptz` keeps microseconds. It is a P1-13 foundation contract used by every module and belongs to its own change; no P1-15 surface ships a paginated list                                                                             |
...
### R-13 — CLOSED: the latent re-issue was reproduced after the merge and fixed in migration 118

**This risk is closed.** It is kept, rather than deleted, because how it moved from "recorded" to
"fixed" is the useful part.

The Phase 1-15 feature branch recorded it and did not fix it, for a correct reason: the defect lives
in `shared.next_display_number` (migration 0003), changing a database function means a migration, and
P1-15 adds none. Recording it was the right action available to that branch.

It was **not**, however, a disposition — and the post-merge gate review did not inherit one. The three
bounds this section used to list were re-tested rather than re-quoted:

1. `SELECT count(*) FROM shared.number_sequences` still returns **0** on protected `develop`.
2. But `org.provision_organization()` inserts sequence rows with an operator-supplied
   `period_reset_rule`, so "nobody has made that choice yet" is a fact about configuration, not a
   property of the contract.
3. And the window was not the only precondition: the reproduction needed no long transaction at all,
   only a row already advanced to a later period — which is exactly what a next-period allocation
   leaves behind.

So it was reproduced on protected `develop` as `rootlco_test_runtime`: an allocation whose `now()`
resolved to the earlier day **re-issued `2026-07-23-000001`**, and the regression trigger permitted
the rewind.

**Closed by migration 118**
([DBCR-P1-15-002](../../database/change-requests/DBCR-P1-15-002-number-sequence-period-hardening.md)):
the allocator reads `clock_timestamp()` after the row lock, the guard refuses any `current_period`
that is not the clock's current key, and the service maps the guard's `23514` to `ERR-CON-001`.
Regression-locked by `tests/db/p1-15-number-sequence-period-hardening.test.ts` (19 proofs) and
`tests/foundation/p1-15-number-allocation-translation.test.ts` (6 proofs).

The guard's first draft refused only a _backwards_ move and was not enough — `SET current_period =
NULL` was still accepted and still restarted the run. That second defect is **PMR-004** in
[the post-merge security review](post-merge-security-review.md).

### R-14 — the unauthenticated auth routes have only a coarse, shared rate-limit bucket

Each of `iam.auth-login`, `iam.auth-logout`, `iam.auth-password-reset` and
`iam.auth-password-reset-completion` declares `auth-adjacent` — 10 requests a minute, keyed on
operation and client address, security-relevant — and since **PMR-001** each actually resolves to it
rather than to the looser `public-probe` the SR-004 fix had been substituting.

What is still missing is the address. `resolveClientAddress()` returns `null` unless the platform
supplies a peer address or `TRUSTED_PROXY_IPS` names a proxy, and neither holds here.

**This entry previously read "throttled by nothing", and that was true of the merged state.** The
pipeline skipped an unkeyable ip-policy for every `public` operation, which extended a
probe-specific argument — a shared bucket in front of a liveness probe causes the outage it exists to
prevent — to four credential endpoints it was never reasoned about. Before P1-15 the
pre-authentication branch had no skip, so those routes were enforced; the skip **removed** a control
the previous phase shipped. That is **PMR-006**, and it is fixed: the skip is now available only to a
policy that is not `securityRelevant`, so `public-probe` keeps the exemption and `auth-adjacent`
never gets it.

What remains, and why this entry stays open: with no address the bucket is **one shared counter per
operation**. Ten requests a minute is then a global budget, so a hostile caller can exhaust it and
deny logins to everyone. That is an availability exposure, and it is accepted here in preference to
unbounded credential stuffing on an unauthenticated credential endpoint — a bounded attacker rate is
worth more than uninterrupted service on a system with no production deployment. It is **not**
presented as a working per-client control.

What else stands in: the authentication provider enforces its own lockout on repeated failures, and
login answers with a uniform failure regardless of cause. Closing R-14 properly means plumbing a peer
address from the platform, which is an infrastructure decision rather than an application one; the
behaviour above is pinned meanwhile by `tests/foundation/p1-15-public-throttle-fallback.test.ts`.

### R-15 — a keyset cursor can skip a row inside one millisecond

`Cursor.v` carries the last-seen sort value as a string. For a timestamp column that string is an ISO
rendering with **millisecond** precision, while `timestamptz` keeps **microseconds** — so two rows
inside the same millisecond can straddle a page edge and one be skipped.

It is bounded rather than dismissed: every page still runs under RLS, so the failure mode is
completeness, never disclosure, and the `(sortValue, id)` tie-breaker keeps the order total for rows
whose stored value the cursor represents exactly.

It is **not fixed here** because the cursor is a P1-13 foundation contract used by every paginated
module, and changing its value encoding is a cross-module change that needs its own review. No P1-15
surface ships a paginated list, so nothing this phase exposes can reach it today. Recorded as
**PMR-007** in [the post-merge security review](post-merge-security-review.md).
