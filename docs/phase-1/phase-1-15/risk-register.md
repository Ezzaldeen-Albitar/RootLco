...
| R-14 | The four unauthenticated auth routes are throttled by nothing in **this** deployment | Certain    | Medium | Each declares `auth-adjacent` (10/min, security-relevant) and — since PMR-001 — resolves to it rather than to the looser `public-probe`. The authentication provider enforces its own lockout on repeated failures                                                                                   | A resolved client address. `resolveClientAddress()` returns null unless the platform supplies a peer address or `TRUSTED_PROXY_IPS` names a proxy, and neither holds, so an ip-keyed policy has nothing to key on and is skipped. Closing it is infrastructure plumbing, not application code |
...
### R-14 — the unauthenticated auth routes are throttled by nothing in this deployment
...
supplies a peer address or `TRUSTED_PROXY_IPS` names a proxy, and neither holds here, so an ip-keyed
policy has nothing to key on and the pipeline skips it rather than collapsing every caller into one
shared bucket (the reasoning is in
[health-endpoints.md](health-endpoints.md) §6.1). The consequence is stated plainly: **in this
deployment those four routes are not rate limited.**

What stands in for it: the authentication provider enforces its own lockout on repeated failures, and
login answers with a uniform failure regardless of cause. What does not exist: any request-rate bound
at the application edge. Closing it means plumbing a peer address from the platform, which is an
infrastructure decision rather than an application one.
