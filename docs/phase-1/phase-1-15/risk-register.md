...
| R-13 | **P1-15-SR-014** — a period-resetting sequence can re-issue a number | Possible   | Medium | `shared.number_sequences` holds **zero rows**: no sequence is provisioned, none ships, and `NumberAllocationService` refuses with `not-provisioned` rather than creating one. The defect additionally requires a `period_reset_rule` other than `never`, which is a per-row operator choice nobody has made | Any fix. `shared.next_display_number` derives its period key from `now()` — transaction-start time — so a transaction beginning before a period boundary can rewind `current_period`. It is a **database function**, and P1-15 adds no migration: remediation belongs to a controlled change request |
...
### R-13 — a latent re-issue in a database function this phase may not change

The final adversarial pass confirmed that `shared.next_display_number` computes its period key from
`now()`, which in PostgreSQL is transaction-**start** time. A transaction that begins a moment before
a period boundary and commits after it therefore compares against the _old_ period, and a sequence
configured to reset yearly, monthly or daily can rewind `current_period` and restart the run at 1 —
re-issuing a number it has already handed out.

Three things bound it, and all three are checkable rather than asserted:

1. `SELECT count(*) FROM shared.number_sequences` returns **0**. No sequence is provisioned; the
   no-fake-data policy forbids shipping one; and the application refuses rather than auto-creating.
2. The defect needs `period_reset_rule <> 'never'`, a per-row operator decision nobody has made.
3. The window is one boundary instant per configured period.

It is **not fixed here**, and the reason is the same rule that governs everything else in this
phase: the function lives in migration 0003, changing it is a database change, and a database change
goes through a change request with its own review — not through a feature branch that happened to
find it. Recording it is the correct action available; quietly patching a shipped migration would
not be.
