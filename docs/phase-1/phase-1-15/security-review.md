...
| **P1-15-SR-004** | **High** | Both unauthenticated health probes were throttled by nothing. `low-risk-metadata` is keyed on tenant, a public request has no tenant, and the post-authentication throttle is never reached on the public path.                                                                                                                                                     | **Resolved, with a stated limit — and twice corrected after the fact.** A `public-probe` policy keyed on operation + client address was added and enforced before the handler. The first fix substituted it for **any** `public: true` operation, which downgraded the four `iam.auth-*` routes (**PMR-001**); substitution now applies only to a session-keyed or absent declaration. The same fix also skipped an unkeyable ip-policy for any public operation, which removed the auth routes' limit entirely (**PMR-006**); the skip now requires a policy that is not `securityRelevant`. See §5.3.1. |
...
Worth recording because it is the more instructive half.

The obvious fix — "throttle public operations by client address" — has a failure mode worse than the
defect. `resolveClientAddress()` returns `null` unless the platform supplies a peer address or a
trusted proxy is configured, and neither is true today: the App Router route handlers do not pass a
peer address, and `TRUSTED_PROXY_IPS` is empty by default. An ip-keyed policy with a null address
collapses to **one global bucket**, so a hostile caller could exhaust it and the orchestrator's own
liveness probe would start receiving `429` — which an orchestrator reads as an unhealthy pod. The
control would cause the outage it exists to prevent.

So the pipeline enforces the public policy **only when an address is actually resolved**, and skips
it otherwise. That is a smaller fix than "public routes are now rate limited", and stating it that
way would be the kind of claim this review exists to catch. What is true:

- behind a configured trusted proxy, or on any platform that supplies a peer address, the two
  probes are throttled per client at 120 requests a minute, per operation;
- with neither, they are **not throttled**, exactly as before — and the reason is written into
  `route-handler.ts` beside the branch that skips it, with the collapse-to-one-bucket property
  pinned by `tests/foundation/rate-limit.test.ts`.
...
fixed on the remediation branch that became PR #62. Eleven more findings were recorded and not fixed,
and seven were refuted.

The gate pass after that merge then **reopened one of the eleven**. PMR-006 had been filed as an
infrastructure limitation; it is in fact a second regression from the same SR-004 fix. The pipeline
skipped the pre-authentication throttle for every public ip-keyed policy when no client address
resolved, and no address ever resolves here — so the four auth routes were throttled by nothing,
where the previous phase had enforced them. Reproduced on `4d1eff2` and fixed in a follow-up
remediation; the residual coarse bucket stays open as R-14. Both the finding and the fact that this
review first got its disposition wrong are recorded in §4.1 and §7.1 of
[the post-merge security review](post-merge-security-review.md), which is the full record with
evidence, reproduction and disposition for each.
