...
**`scripts/dev/owner-acceptance/context.mjs`** — `READER_PERMISSIONS` held no `rec.` code, so `rec.reception.read` was holdable by nobody in the Owner acceptance environment. **RESOLVED by the P1-28 integration wave, in both roles and for two different reasons.** The administrator role now derives its grant from the codes the P1-28 route pages consult, so every `apt.*`/`rec.*` code a screen checks is granted; the reader role gained `apt.appointment.read` and `rec.reception.read` by hand, because a read-only operator denied the calendar and the queue evidences the denial state and nothing about whether the product is usable — and because `rec.reception.read` without `iam.sensitive.view` is what makes that account the negative control `P1-28-SEC-002` needs. The fixture-data question is unchanged and unaffected: the no-fake-data policy still governs it, and an empty queue rendered as an empty queue is the correct observation.
...
6. **`rec.reception.read` is holdable by nobody — RESOLVED, see §2.5.** No seed maps any `rec.*` code to any role and that is still true (the catalogue's only INSERT is line 15, into `iam.permissions`), but `scripts/dev/owner-acceptance/context.mjs` now carries `rec.*` codes in both roles: the administrator's set is derived from the codes the P1-28 route pages consult, and the reader holds `rec.reception.read` and pointedly not `iam.sensitive.view`. The open half of this item was "whether to add it", and the P1-28 integration wave decided it; the fixture-data half is unchanged and still governed by the no-fake-data policy.
