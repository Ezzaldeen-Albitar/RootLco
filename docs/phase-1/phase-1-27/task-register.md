...
| `DOC-002` operator / developer guidance and change-log update                                                       | 15    | `2688635`            | none — `operator-guide.md`, `developer-guide.md`                              | **No automated proof.** No test and no gate reads either guide — a grep for `operator-guide`/`developer-guide` across `tests`, `apps/web/tests` and `scripts` returns nothing. `p1-27-qa.test.ts` checks that five NAMED documents exist and neither guide is among them. This cell previously read "`p1-27-qa.test.ts` — the guides must name files that exist", which contradicted `evidence/task-traceability.md:200` on the same task. |
