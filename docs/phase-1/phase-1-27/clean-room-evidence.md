...
project, and the current tree executes **3749** tests. Both are derived on every
...
**The 3749 is local, and it is pending attestation by this pull request's hosted
...
| Web tier — tests executed                   | 3749  | `--record web`, from the `vitest` JSON report                      |
