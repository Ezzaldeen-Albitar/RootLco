...
The live web suite holds **184 web test files**, every one matched by a `vitest`
project, and the current tree executes **6524** tests. Both are derived on every
...
**The 6524 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 184    | a walk of the tree                                                 |
| Web tier — tests executed                   | 6524  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 184    | the same report, cross-checked against the walk above              |
