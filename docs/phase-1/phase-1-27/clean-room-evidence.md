...
| Root unit tier — tests executed             | 3035  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 111   | the same report, cross-checked against the tier's include rule      |
...
| Migrations on disk                          | 128   | a walk of `supabase/migrations`                                    |
...
| Migrations tracked by git at `HEAD`                  | 128                                         |
