...
project, and the current tree executes **3788** tests. Both are derived on every
...
**The 3788 is local, and it is pending attestation by this pull request's hosted
...
| Web tier — tests executed                   | 3788  | `--record web`, from the `vitest` JSON report                      |
