...
| Root unit tier — tests executed             | 2979  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 107   | the same report, cross-checked against the tier's include rule      |
