...
project, and the current tree executes **4226** tests. Both are derived on every
...
**The 4226 is local, and it is pending attestation by this pull request's hosted
...
| Web tier — tests executed                   | 4226  | `--record web`, from the `vitest` JSON report                      |
