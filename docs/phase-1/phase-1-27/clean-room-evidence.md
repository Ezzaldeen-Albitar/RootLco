...
project, and the current tree executes **6032** tests. Both are derived on every
...
**The 6032 is HOSTED, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 6032  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 177    | the same report, cross-checked against the walk above              |
