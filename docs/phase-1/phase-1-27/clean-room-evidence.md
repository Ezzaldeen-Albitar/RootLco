...
The live web suite holds **136 web test files**, every one matched by a `vitest`
project, and the current tree executes **3797** tests. Both are derived on every
...
**The 3797 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 136    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3797  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 136    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3278  | `--record unit`, from the `vitest` JSON report                     |
