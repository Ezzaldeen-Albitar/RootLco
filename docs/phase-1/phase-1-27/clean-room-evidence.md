...
The live web suite holds **140 web test files**, every one matched by a `vitest`
project, and the current tree executes **3977** tests. Both are derived on every
...
**The 3977 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 140    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3977  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 140    | the same report, cross-checked against the walk above              |
