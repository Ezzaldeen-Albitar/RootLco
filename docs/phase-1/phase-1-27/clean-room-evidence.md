...
| Root unit tier — tests executed             | 3332  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 125   | the same report, cross-checked against the tier's include rule      |
