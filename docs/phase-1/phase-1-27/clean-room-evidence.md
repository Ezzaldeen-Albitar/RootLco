...
| Root unit tier — tests executed             | 3372  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 128   | the same report, cross-checked against the tier's include rule      |
