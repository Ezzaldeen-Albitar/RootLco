...
The live web suite holds **183 web test files**, every one matched by a `vitest`
project, and the current tree executes **6496** tests. Both are derived on every
...
**The 6496 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 183    | a walk of the tree                                                 |
| Web tier — tests executed                   | 6496  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 183    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3772  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 139   | the same report, cross-checked against the tier's include rule      |
...
| Migrations on disk                          | 160   | a walk of `supabase/migrations`                                    |

The floor and the measurement are different questions and they have different
authorities. The baseline file defines the FLOOR, so binding "the floor is 5500"
...
| Migrations tracked by git at `HEAD`                  | 160                                         |
