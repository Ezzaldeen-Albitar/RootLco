...
| Root unit tier — tests executed             | 3336  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 126   | the same report, cross-checked against the tier's include rule      |
