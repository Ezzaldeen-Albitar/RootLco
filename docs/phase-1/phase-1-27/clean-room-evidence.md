...
project, and the current tree executes **2721** tests. Both are derived on every
...
**The 2721 is local, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 2721  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 98    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 2528  | `--record unit`, from the `vitest` JSON report                     |
