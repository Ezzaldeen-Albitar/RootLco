...
The live web suite holds **135 web test files**, every one matched by a `vitest`
project, and the current tree executes **3793** tests. Both are derived on every
...
**The 3793 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 135    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3793  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 135    | the same report, cross-checked against the walk above              |
...
| Committed web floor (`minTests`)            | 3720  | `.github/ci-baselines/test-count-baseline.json`                    |
...
authorities. The baseline file defines the FLOOR, so binding "the floor is 3720"
...
| Migrations tracked by git at `HEAD`                  | 139                                         |
