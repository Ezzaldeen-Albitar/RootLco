...
The live web suite holds **147 web test files**, every one matched by a `vitest`
project, and the current tree executes **4212** tests. Both are derived on every
...
**The 4212 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 147    | a walk of the tree                                                 |
| Web tier — tests executed                   | 4212  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 147    | the same report, cross-checked against the walk above              |
