...
The live web suite holds **134 web test files**, every one matched by a `vitest`
project, and the current tree executes **3678** tests. Both are derived on every
...
**The 3678 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 134    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3678  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 134    | the same report, cross-checked against the walk above              |
