...
The live web suite holds **126 web test files**, every one matched by a `vitest`
project, and the current tree executes **3363** tests. Both are derived on every
...
**The 3363 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 126    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3363  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 126    | the same report, cross-checked against the walk above              |
