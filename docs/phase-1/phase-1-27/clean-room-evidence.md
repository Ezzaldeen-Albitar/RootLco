...
The live web suite holds **177 web test files**, every one matched by a `vitest`
project, and the current tree executes **5929** tests. Both are derived on every
...
**The 5929 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 177    | a walk of the tree                                                 |
| Web tier — tests executed                   | 5929  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 173    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3732  | `--record unit`, from the `vitest` JSON report                     |
