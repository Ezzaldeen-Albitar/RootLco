...
project, and the current tree executes **5522** tests. Both are derived on every
...
**The 5522 is HOSTED, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 5522  | `--record web`, from the `vitest` JSON report                      |
...
| Committed web floor (`minTests`)            | 4471  | `.github/ci-baselines/test-count-baseline.json`                    |
...
authorities. The baseline file defines the FLOOR, so binding "the floor is 4471"
