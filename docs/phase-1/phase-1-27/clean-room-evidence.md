...
| Root unit tier — tests executed             | 2398  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 96    | the same report, cross-checked against the tier's include rule      |
