...
The live web suite holds **93 web test files**, every one matched by a `vitest`
project, and the current tree executes **2405** tests. Both are derived on every
...
**The 2405 is local, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 93    | a walk of the tree                                                 |
| Web tier — tests executed                   | 2405  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 93    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 2253  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 94    | the same report, cross-checked against the tier's include rule      |
| Committed web floor (`minTests`)            | 2295  | `.github/ci-baselines/test-count-baseline.json`                    |
...
authorities. The baseline file defines the FLOOR, so binding "the floor is 2295"
