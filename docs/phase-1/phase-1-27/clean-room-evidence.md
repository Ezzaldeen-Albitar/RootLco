...
The live web suite holds **142 web test files**, every one matched by a `vitest`
project, and the current tree executes **4026** tests. Both are derived on every
...
**The 4026 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 142    | a walk of the tree                                                 |
| Web tier — tests executed                   | 4026  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 142    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3341  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 127   | the same report, cross-checked against the tier's include rule      |
...
| Migrations tracked by git at `HEAD`                  | 141                                         |
