...
The live web suite holds **131 web test files**, every one matched by a `vitest`
project, and the current tree executes **3552** tests. Both are derived on every
...
**The 3552 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 131    | a walk of the tree                                                 |
...
| Root unit tier — tests executed             | 3264  | `--record unit`, from the `vitest` JSON report                     |
