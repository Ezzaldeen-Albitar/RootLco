...
The live web suite holds **139 web test files**, every one matched by a `vitest`
project, and the current tree executes **3940** tests. Both are derived on every
...
**The 3940 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 139    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3940  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 139    | the same report, cross-checked against the walk above              |
