...
The live web suite holds **143 web test files**, every one matched by a `vitest`
project, and the current tree executes **4057** tests. Both are derived on every
...
**The 4057 is local, and it is pending attestation by this pull request's hosted
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 143    | a walk of the tree                                                 |
| Web tier — tests executed                   | 4057  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 142    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3399  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 129   | the same report, cross-checked against the tier's include rule      |
