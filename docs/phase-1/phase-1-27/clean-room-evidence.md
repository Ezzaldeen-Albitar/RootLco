...
project, and the current tree executes **3726** tests. Both are derived on every
...
**The 3726 is local, and it is pending attestation by this pull request's hosted
...
| Web tier — tests executed                   | 3726  | `--record web`, from the `vitest` JSON report                      |
