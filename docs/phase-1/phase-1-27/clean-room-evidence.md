...
project, and the current tree executes **3967** tests. Both are derived on every
...
**The 3967 is local, and it is pending attestation by this pull request's hosted
...
| Web tier — tests executed                   | 3967  | `--record web`, from the `vitest` JSON report                      |
