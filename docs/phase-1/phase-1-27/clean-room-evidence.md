...
The live web suite holds **94 web test files**, every one matched by a `vitest`
project, and the current tree executes **2456** tests. Both are derived on every
...
**The 2456 is local, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 94    | a walk of the tree                                                 |
| Web tier — tests executed                   | 2456  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 94    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 2273  | `--record unit`, from the `vitest` JSON report                     |
