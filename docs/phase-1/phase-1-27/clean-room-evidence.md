...
project, and the current tree executes **2627** tests. Both are derived on every
...
**The 2627 is local, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 2627  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 98    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 2464  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 97    | the same report, cross-checked against the tier's include rule      |
