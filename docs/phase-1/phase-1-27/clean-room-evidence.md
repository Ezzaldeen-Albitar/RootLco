...
The live web suite holds **128 web test files**, every one matched by a `vitest`
project, and the current tree executes **3433** tests. Both are derived on every
...
**The 3433 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 128    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3433  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 128    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3212  | `--record unit`, from the `vitest` JSON report                     |
...
| Migrations on disk                          | 137   | a walk of `supabase/migrations`                                    |
...
| Migrations tracked by git at `HEAD`                  | 137                                         |
