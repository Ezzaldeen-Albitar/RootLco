...
project, and the current tree executes **2638** tests. Both are derived on every
...
**The 2638 is local, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 2638  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 98    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 2475  | `--record unit`, from the `vitest` JSON report                     |
