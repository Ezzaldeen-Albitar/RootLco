...
The live web suite holds **122 web test files**, every one matched by a `vitest`
project, and the current tree executes **3212** tests. Both are derived on every
...
**The 3212 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 122    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3212  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 120    | the same report, cross-checked against the walk above              |
...
| Migrations tracked by git at `HEAD`                  | 136                                         |
