...
The live web suite holds **120 web test files**, every one matched by a `vitest`
project, and the current tree executes **3149** tests. Both are derived on every
...
**The 3149 is HOSTED, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 120    | a walk of the tree                                                 |
| Web tier — tests executed                   | 3149  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 117    | the same report, cross-checked against the walk above              |
