...
project, and the current tree executes **3315** tests. Both are derived on every
...
**The 3315 is HOSTED, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 3315  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 124    | the same report, cross-checked against the walk above              |
