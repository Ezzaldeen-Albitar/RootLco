...
project, and the current tree executes **2729** tests. Both are derived on every
...
**The 2729 is local, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 2729  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 98    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 2682  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 101   | the same report, cross-checked against the tier's include rule      |
...
| Migrations on disk                          | 122   | a walk of `supabase/migrations`                                    |
...
| Migrations tracked by git at `HEAD`                  | 122                                         |
