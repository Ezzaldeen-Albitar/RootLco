...
The live web suite holds **103 web test files**, every one matched by a `vitest`
project, and the current tree executes **2897** tests. Both are derived on every
...
**The 2897 is local, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 103    | a walk of the tree                                                 |
| Web tier — tests executed                   | 2897  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 103    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3044  | `--record unit`, from the `vitest` JSON report                     |
...
| Migrations on disk                          | 133   | a walk of `supabase/migrations`                                    |
...
| Migrations tracked by git at `HEAD`                  | 133                                         |
