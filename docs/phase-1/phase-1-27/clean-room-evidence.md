...
project, and the current tree executes **4885** tests. Both are derived on every
...
**The 4885 is HOSTED, and it is the binding measurement.** It is the output of
...
| Web tier — tests executed                   | 4885  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 168    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3637  | `--record unit`, from the `vitest` JSON report                     |
