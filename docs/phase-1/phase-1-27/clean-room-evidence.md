...
The live web suite holds **109 web test files**, every one matched by a `vitest`
project, and the current tree executes **2992** tests. Both are derived on every
...
**The 2992 is local, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 109    | a walk of the tree                                                 |
| Web tier — tests executed                   | 2992  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 109    | the same report, cross-checked against the walk above              |
