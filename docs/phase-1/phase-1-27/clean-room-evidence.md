...
project, and the current tree executes **4079** tests. Both are derived on every
...
**The 4079 is local, and it is pending attestation by this pull request's hosted
...
| Web tier — tests executed                   | 4079  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 143    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3468  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 132   | the same report, cross-checked against the tier's include rule      |
