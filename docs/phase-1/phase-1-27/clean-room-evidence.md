...
The live web suite holds **102 web test files**, every one matched by a `vitest`
project, and the current tree executes **2889** tests. Both are derived on every
...
**The 2889 is local, and it is the binding measurement.** It is the output of
...
| ------------------------------------------ | ----- | ------------------------------------------------------------------ |
| Web test files under `apps/web/tests`       | 102    | a walk of the tree                                                 |
| Web tier — tests executed                   | 2889  | `--record web`, from the `vitest` JSON report                      |
| Web tier — files the run reported           | 102    | the same report, cross-checked against the walk above              |
| Root unit tier — tests executed             | 3002  | `--record unit`, from the `vitest` JSON report                     |
| Root unit tier — files the run reported     | 110   | the same report, cross-checked against the tier's include rule      |
...
| Migrations on disk                          | 126   | a walk of `supabase/migrations`                                    |
...
| Migrations tracked by git at `HEAD`                  | 126                                         |
