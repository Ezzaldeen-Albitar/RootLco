...
| `QA-005`  | `TEST_OR_GATE_DEFECT`  | FIXED (sealed)      | itself    |
...
| `A42-06` | `FE-021` and `FE-022` carried root `FE-024` and commit `0272e1d` in the Summary, and root `FE-019`/`8daf8e9` and root `SEC-001`/`915b861` in their own sections                                                                                                                    | **RESOLVED, and this disposition is the thing that was wrong.** It said "a question for the change history, not for a document; nothing in this repository decides it" — but the change history IS in this repository, and reading it decides it in one pass. Each section states a remedy and each remedy is in exactly one commit. `FE-021`'s section says "Closed by the navigation fix": `8daf8e9` is the only one of the three that touches `VehicleSearchScreen.tsx`, and it adds `rowActions={(row) => (` there. `FE-022`'s section says item (1) is the missing permission gate on plate assignment and odometer recording: `915b861` is the only one of the three that touches `VehicleHistorySections.tsx`, where it adds `PlateSection`'s `veh.vehicle.manage` gate and `OdometerSection`'s `canRecord`, under a comment reading "Ownership had a real gate; those two never did". `0272e1d` touches neither surface — its diff is `VehicleDuplicateReviewScreen.tsx`, `VehicleProfileScreen.tsx` and `profile-contract.ts`, where it adds `isTerminal`. **Both attributions were true about different things**, which is why neither side looked wrong from inside itself: the sections carry the ORIGINAL adjudication, and these two Summary rows had been overwritten with the ROUND-ONE RECHECK commit — whose own table below already records `0272e1d` against "`FE-024`, `FE-021`, `FE-022` — one predicate, three shipped write surfaces". `root` is defined above as the duplicate root, not the recheck grouping, so `FE-024` was wrong in that column on both rows. **The two Summary rows are corrected to their sections** (`FE-021` → FIXED `8daf8e9`, root `FE-019`; `FE-022` → FIXED `915b861`, root `SEC-001`), and what they used to say is preserved in this cell — the same treatment `A42-03` gets, in the opposite direction, because there the Summary is the wrong reading kept as evidence and here it is the corrected one |
...
OPEN              =  0     (was 1: QA-005, reopened while its evidence described a superseded head; sealed on the candidate measurement -- task-matrix.json is the authority)
```
...
across all four trees the gate now scans. **A VIN in particular is never written
