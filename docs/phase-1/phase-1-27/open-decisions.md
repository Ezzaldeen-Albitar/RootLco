...
| 6   | `scripts/dev/owner-acceptance/context.mjs`                          | `WITHHELD_PERMISSIONS = ['crm.customer.merge', 'veh.vehicle.merge']` — the Owner-acceptance role is granted **43** codes and neither of these, so an acceptance run cannot pass while the affordance quietly exists. It read 30 when this row was written; the P1-28 wave added the codes its screens consult, and `tests/ci/p1-28-access-gate.test.ts` now asserts the withholding survives that derivation in all three roles rather than leaving it to a count nobody re-reads |
