...
`requirePermissions` is **behaviourally unchanged for every existing caller**:
its only edit is an optional fourth parameter (`options: EvaluationOptions = {}`)
which it forwards, and with `forceScoped` unset it reproduces
`requiresScopedEvaluation` exactly. (An earlier revision of this paragraph said
the function was "unchanged" with a "purely additive" diff. The behavioural claim
is the accurate one; the literal one was not, because the signature did change —
worth correcting so a later reader does not test the literal wording and find it
false.) It still accepts an empty target, because the pre-handler path
legitimately passes one for all ten of these operations; folding the guard into
...
| **P1-18-SEC-ROLEPROBE**   | **Two routes, not one — the entry previously recorded only the first.** The channel exists on `POST /receptions/{receptionId}/authorizations` (permission `rec.reception.authorization.verify`) **and** on `POST /receptions/{receptionId}/refusals` with `refusalType: 'authorization'`, which reaches the same `assertMayAuthorize` guard from the cheaper `rec.reception.signature.manage`. On the refusals route a hit is destructive — it writes a permanent append-only refusal that also blocks approval and conversion — and the caller must already hold branch-scoped write authority on the visit and know the partner UUID. Original text: `assertAuthorizingRoleHeld` refuses a role the partner does not hold with the same non-disclosing 409 as the authority guard, but the 201-versus-409 split still lets a caller holding `rec.reception.authorization.verify` learn which of the four authorizing roles a partner holds on a visit. A correct guess writes a real, attributable decision and the misses write nothing. Narrower than the alternative: accepting the claimed role would put a false role on an immutable, dispute-facing row. Closing the channel needs a read contract for party roles, which Field 23 does not allocate.                                                                                                                                                                                                                  |
...
| **P1-18-GATE-IDENTITY**   | **The authorization coverage gate does not prove operation-identity binding, and must not be described as if it did.** Mutation M6 bound the party-roles route to the sibling `rec.reception-authorization` declaration, so `rec.reception.party.manage` was never evaluated at all — and `npm run validate:authorization-coverage` still reported `OK: every operation is guarded and every route is registered` and exited 0. It checks that operations declare permissions and that routes are registered; it does not check that a route runs under its OWN declaration. The containment suite does not kill M6 independently either, because every permission-bearing fixture principal holds both sibling codes, so both bindings produce identical runtime outcomes. M6 is ALSO killed behaviourally by `tests/backend/p1-18-reception-parties.test.ts` (a principal holding only `rec.reception.authorization.verify` gets 201 instead of 403 under the sibling binding), discovered after the fact — so the coverage gate is the only GATE blind to it, not the only assertion. The binding is additionally pinned by dedicated assertions in `tests/foundation/p1-18-scoped-authorization.test.ts` (`runs under its OWN declaration`, `declares exactly one operation`, and the rule that no route may hand-roll `requirePermissions` / `requireScopedPermissions`). Generalising the check to the whole platform is the correct follow-up and is not attempted here. |
