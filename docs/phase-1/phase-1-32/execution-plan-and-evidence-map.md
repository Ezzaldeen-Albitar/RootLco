...
| FE-004 RTL/LTR                   | P1-25…P1-28, P1-30p, P1-31 | `apps/web/tests/render.tsx` helpers — 70 of 73 component suites use one, 57 of them with a right-to-left render; browser projects at `apps/web/playwright.config.ts:156-163`, `:174-181`, `:214-223`; `foundation.spec.ts:29`, `:131`. Local+hosted, executed                                                                                                                                                                                                      | Thirteen suites render left-to-right only (note 5); combined with no browser suite for P1-29 and most of P1-30, four modules have no right-to-left evidence of any kind                                                                                                                                                                                            | List per uncovered screen the components needing a right-to-left case and any logical-property violation found statically. NOT RUN: the web unit tier, the style check, the authenticated browser tier                                                                     |
...
5. Thirteen suites, re-derived at this head as the suites importing the left-to-right helper with
   no right-to-left import and no both-directions import:
   `crm-customer-components.dom.test.tsx:6`, `data-table.dom.test.tsx:11`,
   `diagnostics.dom.test.tsx:13`, `form-reset-runtime.dom.test.tsx:5`,
   `p1-27-permission-route-binding.dom.test.tsx:5`, `price-list-detail.dom.test.tsx:5`,
   `quality.dom.test.tsx:15`, `quotation-detail.dom.test.tsx:5`, `record-form.dom.test.tsx:6`,
   `record-form-consumers.dom.test.tsx:23`, `technician-workspace.dom.test.tsx:5`,
   `work-order-delivery-mount.dom.test.tsx:4`, `write-permission-gating.dom.test.tsx:7` — all
   under `apps/web/tests/`. Two of them, `data-table` and `record-form`, are shared-primitive
   suites sitting inside module groups this document records as covered, so the gap reaches the
   shared layer and not only the six business screens first counted.
...
| CC-64 (b) | `../phase-1-31/change-control-2026-09-08.md:8905` | "the coverage figures stay local although the hosted `web-quality` job ran". Its subject is the frontend coverage evidence FE-013 and QA-001 would inherit and re-certify; a locally measured floor is not the hosted evidence QA-005 packages                                                                                                                                                                                    |
...
3. **No second-engine browser project.** All nine browser projects use one desktop engine
   (`playwright.config.ts:150`, `:159`, `:168`, `:177`, `:186`, `:200`, `:207`, `:218`, `:292`); the
...
8. **No right-to-left render for thirteen suites' screens** — the thirteen listed in row note 5
   import the left-to-right helper only. Of the 73 component suites, 70 use a direction helper and
   57 of those import the right-to-left helper or both; the remaining three
   (`lib-coverage.dom.test.tsx`, `loading-boundary.dom.test.tsx`, `shell-viewport.dom.test.tsx`)
   use neither.
