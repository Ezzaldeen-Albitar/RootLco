...
| ------------ | ----------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-31-FE-001 | Ready-for-delivery list | Planned | **Buildable — the readiness contract is published.** Was: _Blocked — missing contract. The chapter declares `GET /api/v1/deliveries`; `deliveries/route.ts` exports POST only. No list, no detail, no by-work-order lookup. `DeliveryRepository.findLiveDeliveryForWorkOrder` exists and is called only from the create path, never routed._ The delivery-record list landed as `sal.delivery-list` (PR #358), and the Owner's **D-3** decision of 2026-09-09 separated the OPERATIONAL queue from it: `GET /api/v1/delivery-readiness` (`sal.delivery-readiness-list`) lists the work orders that satisfy the server's delivery-eligibility rules, including eligible work orders with **no delivery record yet**, published by PR (pending) — see [`delivery-readiness-seam.md`](./delivery-readiness-seam.md). |
...
3. **The delivery read seams** — FE-001's list or by-work-order lookup first, then the FE-003,
   ...
   delivery id. **Closed for FE-001:** the by-work-order lookup and the record reads landed as
   P-2 … P-5, the delivery-record list as `sal.delivery-list` (PR #358), and the Owner's **D-3**
   decision added the operational queue `sal.delivery-readiness-list`, which answers for eligible
   work orders that have no delivery record at all ([`delivery-readiness-seam.md`](./delivery-readiness-seam.md)).
...
| **P-11** | Provide a report-configuration writer and a report engine   | Backend — reporting                    | The report configuration and version tables have no writer and no seed; the definition view's `executable` is the literal `false` because the frozen reporting schema binds no data source; `rpt.report.configure` is declared by no operation. P1-23's own contract archaeology lists this as its open gap. **WRITER PUBLISHED** on `remediation/p1-31-backend-report-configuration-seam` (PR #361): seven operations publish the configuration and version surface, all seven declare `rpt.report.configure`, and the provisioning bundle carries it (74 to 76 on the merged tree, alongside P-10's code), which closes **CC-02** and obliges ONE operator run of `scripts/platform/backfill-tenant-administrator-bundle.mjs` after merge covering both newly approved codes. **ENGINE STILL PENDING**: `executable` remains the literal `false` and awaits the D-4 engine slice; see report-configuration-seam.md and change control CC-20 | FE-010, FE-011, FE-012, FE-013, FE-014, FE-016            |
