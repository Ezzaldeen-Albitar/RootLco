...
| P1-31-FE-002 | delivery eligibility    | Planned | **Buildable — frontend only.** `sal.delivery-eligibility-read` is complete and frontend-shaped. Entry still needs FE-001 to supply a delivery id. **Write path in an open pull request** (PR #362) on `feature/p1-31-delivery-execution`, stacked on PR #357; see `delivery-execution-screen.md`. End-to-end verification is not claimed there.                                                                                                                                                                                                                                                                            |
| P1-31-FE-003 | authorized receiver     | Planned | **Blocked — missing read.** The write exists; `findReceiver` is read by the eligibility composition but its row is collapsed into the `receiver_not_verified` blocker. A screen can learn _whether_, never _who_. Read resolved by P-4. **Write path in an open pull request** (PR #362) on `feature/p1-31-delivery-execution`, stacked on PR #357; the receiver is confirmed by name through the shared selector. The optional identity evidence is NOT captured — no seeded document category admits a person’s proof of identity, which is recorded as a new backend prerequisite in `delivery-execution-screen.md` §5. |
| P1-31-FE-004 | delivery checklist      | Planned | **Blocked — P1-27-INT-088 and PPD-12.** No template writer, no results read, gaps capped at 20, `missingCount` dropped, and the gap scan is company-scoped rather than template-scoped. Template writer resolved by P-9, results read resolved by P-4. **Write path in an open pull request** (PR #362) on `feature/p1-31-delivery-execution`, stacked on PR #357: the checklist is assembled from the ACTIVE templates and each item records its own outcome. The template administration screen is not in it.                                                                                                            |
| P1-31-FE-005 | final odometer          | Planned | **Buildable — frontend only.** A body field of `sal.delivery-complete`, read back through `veh.vehicle-odometer-history`. Trap: the route regex accepts two decimals, the column is `numeric(12,1)`, so `123.45` passes the route and is refused as a 422. **Write path in an open pull request** (PR #362) on `feature/p1-31-delivery-execution`, stacked on PR #357; see `delivery-execution-screen.md`. End-to-end verification is not claimed there.                                                                                                                                                                   |
| P1-31-FE-006 | delivery signatures     | Planned | **Blocked — no signature read.** `findSignature` is write-path-only. Acceptance is image-only. The route's P1-22-L-04 docblock is stale and is a live trap for a reader (see P-14). Read resolved by P-4, stale docblock resolved by P-14. **Write path in an open pull request** (PR #362) on `feature/p1-31-delivery-execution`, stacked on PR #357: capture, link and bind, through the one approved file input and the seeded signature category.                                                                                                                                                                      |
...
| **P-9**  | Provide a delivery-checklist template writer                | Backend — delivery configuration       | The checklist template and template-item tables are SELECT-only. **PPD-12**. **RESOLVED** by eight operations under /api/v1/delivery-checklist-templates (branch remediation/p1-31-backend-checklist-template-seam, PR #355); see delivery-checklist-template-seam.md and change control CC-14, which records the completion-gate defect this seam measured and did not fix                                                                                                                                                                                                                                                                                                                                                                                                                      | FE-004                                                    |
| **P-10** | Provide a warranty-policy and coverage writer               | Backend — warranty configuration       | The warranty policy and coverage tables have no writer; `wty.policy.manage` is seeded and declared by no operation. **PPD-04**. **Resolved** on `remediation/p1-31-backend-warranty-policy-seam` (PR #356): seven operations publish the policy and coverage surface, `wty.policy.manage` is declared by the five writes, and the provisioning bundle carries it (74 to 75), which closes **CC-01** and obliges an operator run of `scripts/platform/backfill-tenant-administrator-bundle.mjs` after merge                                                                                                                                                                                                                                                                                       | FE-008, FE-009                                            |
