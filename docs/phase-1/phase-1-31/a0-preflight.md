...
| **P-11** | Provide a report-configuration writer and a report engine   | Backend — reporting                    | The report configuration and version tables have no writer and no seed; the definition view's `executable` is the literal `false` because the frozen reporting schema binds no data source; `rpt.report.configure` is declared by no operation. P1-23's own contract archaeology lists this as its open gap. **ENGINE SLICE 1 OF 4 landed** on `remediation/p1-31-backend-report-engine-work-orders` (PR PENDING): a code-registered dataset registry, `rpt.report-run`, and `work_orders_by_status` implemented in the work-order module; `executable` is now registry membership rather than the literal `false`. The configuration WRITER and the other three baseline reports remain open; see report-engine-seam.md and change control CC-23 | FE-010, FE-011, FE-012, FE-013, FE-014, FE-016            |
