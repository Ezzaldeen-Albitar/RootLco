...
takes it: the Owner's D-27 is to "correct implementation or documentation according to that
authority; do not normalize codes merely for numerical consistency", and a slice that reassigned
status codes across two other lanes on its way past would be doing the second thing rather than the
first. _(This paragraph read "a slice that renamed four other lanes' status codes": the number was
