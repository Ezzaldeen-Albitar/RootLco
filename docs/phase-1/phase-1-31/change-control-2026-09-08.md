...
**Baseline:** protected `develop` **811e9891353b466b7788e7ca8a7bddee8496de72**, merged into this
branch on 2026-09-12. The branch was authored against `deb404c1` and the merge is recorded by its
own commit, so every figure below that refers to `develop` refers to `811e9891` and not to the head
the first three commits were written against.

### 52.1 Identifier allocation — re-read at `develop` `811e9891`, still PROVISIONAL

The heading and the identifier were taken high on 2026-09-12 against `develop` `8c4e6a9c`, where
the register ran to section 43, precisely so that a lane landing first would never be renumbered by
this one. Re-read at `811e9891` the picture is:

| identifier                              | belongs to                                              | state                                    |
| --------------------------------------- | ------------------------------------------------------- | ---------------------------------------- |
| sections 1–48 and 50, **CC-01 … CC-38** | the earlier P1-31 slices                                | settled, on `develop` `811e9891`         |
| section 49                              | `feature/p1-31-assurance-evidence`                      | unmerged, deliberately held              |
| section 51                              | unallocated                                             | free                                     |
| **section 52**                          | this slice                                              | free at `811e9891`; claimed here         |
| **CC-40**                               | this slice **and** `feature/p1-31-operational-overview` | **claimed twice, neither claim settled** |
| section 53, **CC-39**, **CC-41**        | `feature/p1-31-operational-overview`                    | unmerged                                 |

**The PROVISIONAL marking stands, and its reason has moved from the heading to the identifier.**
Section 52 is no longer at risk: sections 44 to 48 and 50 are settled on `develop`, the two numbers
still free (49 and 51) are both lower, and section 53 is claimed by a different lane, so nothing can
grow onto 52 without renumbering somebody else's record first. The identifier is the live risk.
`feature/p1-31-operational-overview` allocates **CC-39, CC-40 and CC-41** under its own section 53
for the operational overview, and that branch is unmerged as well, so CC-40 is claimed by two
branches and settled by neither. The register's own reconciliation rule decides it without
discussion: whichever of the two integrates second renumbers **its own** identifier, and nothing
else moves. This slice does not pre-emptively renumber, because CC-40 is unoccupied on `develop`
`811e9891` and moving now would leave a hole the other lane has no reason to fill.
...
| --------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | --------------------------- |
| **CC-40** | **the committed acceptance suite skips its whole surface without a handoff, and that is all it proves — re-measured 2026-09-12** | As first recorded, five of the eleven browser cases skipped for a second reason as well: they open screens that arrived with branches not then merged, so they asked the message catalogue whether the screen's own strings existed. That reason is now gone — `reports.catalogue.title` and `warranty.policies.title` both resolve in `en.json` and `ar.json` at `develop` `811e9891`, and the two conditions are retained as regression guards rather than as live skips. The residual that remains is the original one and is not reduced by the merge: **every case still skips when `ROOTLCO_P131_HANDOFF` is absent, which it is on every checkout, so the committed suite proves that the specs compile and type-check and nothing about the screens.** A skip was chosen over a failure because a committed suite must stay green on a checkout that has never run an acceptance; the alternative considered and rejected was asserting a 404, which also passes on a build that is merely broken and would convert a missing world into a green tick. | the integration lane | open, recorded, PROVISIONAL |
...
- **No section or identifier belonging to another lane was renumbered.** The union in this file is
  `develop`'s sections 44 to 48 and 50 in their own order, followed by this slice's 52; the
  contested **CC-40** is left where both branches put it, with the collision named in §52.1 instead
  of resolved unilaterally.
