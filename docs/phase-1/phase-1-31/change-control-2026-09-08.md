...
implied. The run itself was executed on **2026-09-10** in container
`rootlco-p131-isolation-20260910`, and section 9.2 records each suite with its result and exit code.
**No run ledger entry exists for the database or the backend tier** — the ledger records the unit
and web tiers only — and **no hosted result exists** for any of it.
...
**Four recommendations are pending, one sentence each, and stated once in the seam register**
(section 3): **A-3** backfilled legacy rows keep `status = 'inactive'`; **A-4** `employment_ref`
stays optional and unique per tenant when present; **A-5** the branch policy keeps the home branch
informational only, reading tenant-wide while writing stays branch-scoped; **A-6** an unresolved
legacy identity has no resolution command and stays listed for the Owner, to be resolved later
through one Owner-approved operator command rather than inside a migration. **A-1 — employee
identity independent of a login — is APPROVED, not pending**, as is A-2.
