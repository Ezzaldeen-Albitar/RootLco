...
|                     |                                                          |
| ------------------- | -------------------------------------------------------- |
| Protected `develop` | `cf67d15e` (audited against `a19709c4`; one merge since) |
...
| **RES-17**         | BR-01 merged with no execution record. **PR #278 then did the same**, carrying only a register update — so the programme has two. Both are now written: `openapi-success-status-record.md` is retrospective, from the protected tree. BR-01 remains outstanding.                                                                                                                                                            |
...
families PC-04 resembles were added under **P1-27**, not here. Naming a response
...
2. ~~**RES-04**~~ — in flight in the current slice.
3. **RES-12** — the `check-api-boundary.mjs` blind spot: the last unblocked **A**
   row unambiguously in scope, being a gate defect rather than new capability.
