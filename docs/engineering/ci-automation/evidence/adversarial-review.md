...
| AR-24 | The image history guard omitted `_TOKEN` while the env guard included it; both are name-based, so `DATABASE_URL=postgres://u:p@h` passed                            | patterns aligned, a value-shaped scan added, and both files redacted before upload to a public artifact |
