...
| **ERR-INV-001**  | Material draw not covered by an approved requirement | 409  | transition    | No        | conflict | A reservation or issue for a work order drew on no material requirement or on one that does not allow it: no requirement covers the item at all, not approved, no exact unit conversion, no confirmed specification, or more than the approved allowance plus approved exceptions. The response carries materialDraw (allowance, alreadyCommitted, requested in the requirement unit, and the reason). Nothing moved. Distinct from ERR-TRN-001: the remedy is an approval, a reference fact or an approved exception.                                                                                                                                                                                                                                                                                         |
...
| **ERR-CAP-001**  | Subscription capacity limit reached                  | 409  | capacity      | No        | conflict | The organisation already holds as many companies, branches or user seats as its active subscription plan permits, so the database refused the write. Not ERR-TEN-001, which means the plan does not include a feature at all; here the plan includes it and the allowance is spent. The problem document carries `capacity` — kind, limit and used — so an administrator can see which allowance was reached. Retrying does not help: a unit is released or the plan changes.                                                                                                                                                                                                                                                                                                                                  |
| **ERR-CAP-002**  | Organisation is not active                           | 409  | capacity      | No        | conflict | The write would have grown an organisation that is suspended or closed, and the database refused it. Distinct from ERR-CAP-001: there is no allowance to spend because the organisation itself is not running.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
`concurrency`, `throttling`, `stub`, `capacity`, `platform`.
