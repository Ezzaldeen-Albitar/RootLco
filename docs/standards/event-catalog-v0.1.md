...
> **Phase 1-13 published no domain events.** The catalog fixes names, schema versions, and owning
> modules so that Phases 1-14 … 1-23 cannot invent conflicting ones. When this document was written
> every entry in §3 carried `implementedIn: null`, which meant exactly what it said: **no code
> published it, and no consumer subscribed to it.** Registering a name still does not publish it —
> `implementedIn` records whether a producer exists, and a later phase sets it when it adds one.
...
added its producer — set in that change, never before.
