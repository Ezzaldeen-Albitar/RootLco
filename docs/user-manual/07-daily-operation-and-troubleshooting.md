...
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Add a company, a branch, a department or an employee      | There is no screen for any of them. They are created by whoever runs the platform, outside the application. The Organization screen says so: "The service publishes no company or branch directory, so references are shown rather than names."                                                      |
...
| **Tenant Administrator**       | The broad working role: users, roles, grants, sessions, approval limits, sensitive values, audit log; customers and vehicles; reception through to conversion; work orders, jobs, technicians, labour, diagnostics, quality and rework; services, price lists and publishing; quotations and decisions; inventory items, stock operations and adjustment approval; invoices, issuing, finance figures, payments and allocations; deliveries; warranties and warranty plans; reports; documents. | Report **export**; notifications; appointments (the whole module is unreachable); creating a work order directly; company, branch and settings management — so numbering, taxes, currencies and system settings are not writable and those rows are hidden; customer merge, notes, consents, restrictions and duplicate review; vehicle merge, odometer recording, status and relationships; reception evidence, evidence waiver, closing and catalogues; inventory costing, custody, external purchases and inventory audit; credit notes and reversals; document archiving; platform-level provisioning. |
...
2. **Appointments are unreachable for a new administrator**, because the role holds no appointment
   permission at all.
3. **Opening stock needs two people** — the counter may not be the approver.
4. **What you see in a menu is a convenience, not authority.** "What you see here is a convenience.
   Every request is checked by the service, and its decision is the one that applies." <!-- permissions.visibilityNotice -->
5. To see exactly what your own account may do, open "Your profile" <!-- nav.profile --> and read
...
8. **There is no company, branch, department or employee screen**, and no technician roster screen.
...
11. **Opening stock is maker-checker and cannot be recounted.**
12. **An invoice cancels only before issue**, and its number comes from the branch sequence.
...
16. **The delivery checklist-template screen is deferred** (backlog item P1-31-FU-001).
17. **This is a local, private, single-machine installation.** It is not a hosted service, it is not
...
18. **No phase certification is claimed.** The Owner recorded a conditional decision on 2026-09-16;
