...
while the seed grew to 116. It listed no `tech.` code at all — not even `tech.technician.read`,
...
| `platform.organization.lifecycle`             | platform | critical | Suspend, reactivate or close an existing tenant                                        |
| `platform.organization.provision`             | platform | critical | Create a tenant with its first company, branch and Owner                               |
| `platform.organization.read`                  | platform | medium   | Read tenants from the control plane                                                    |
