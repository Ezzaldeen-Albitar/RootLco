...
Introspected from the live catalog: **27 tables** (`sal` 19, `wty` 5, `rpt` 3), 427 columns, **16 restricted** (14 amount columns gated by `sal.finance.view`; 2 delivery-evidence columns gated by `sal.delivery.view`), 0 restricted-searchable. Money is `NUMERIC(18,4)`; every FK is `ON DELETE RESTRICT`. Full per-column tables live in `docs/phase-1/phase-1-11/phase-1-11-data-dictionary-{sal,wty,rpt}.md`.

## §SAL — invoices/`invoice_amounts` (restricted)/lines/`invoice_line_amounts` (restricted)/numbering configs/status history/payment methods/receipts/allocations/credit notes/receipt reversals/financial events/delivery records/checklist templates+items+results/authorized receivers/signatures/delivery status history.
...
One row per delivery whose pre-P-17 `delivering_employee_id` matched no same-tenant identity when `20260910091000` ran (P1-31 P-17). Written once by that migration and by nothing else: the application holds SELECT only, tenant-scoped and RLS-forced. It exists so an unresolved handover identity is visible to the Owner instead of being replaced by a fabricated person or by the migrating actor.
...
| `recorded_at`  | `timestamptz` | internal | no    | When the migration observed the value as unresolved. Not when the handover happened.         |
...
| `delivering_employee_display_name` | `text`        | internal | yes   | Immutable name snapshot stamped by `sal.stamp_delivering_employee_identity`; survives a rename or retirement; NULL only on a pre-P-17 row listed in `sal.delivery_legacy_identity_review`. |
...
## §RPT — report configurations/versions/user-owned saved filters.
