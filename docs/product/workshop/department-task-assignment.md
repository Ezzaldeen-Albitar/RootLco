...
This structure is exactly the right shape for the Owner's requirement: a workshop
creates as many departments as it has, names them as it wishes, and deactivates
the ones it stops using.
...
| **No business record anywhere names a department.**                             | Across `supabase/migrations`, `department_id` appears in exactly two places: `org.departments` itself, and `iam.grant_scopes` (where it narrows a *permission grant*, not a piece of work).   |
...
**None of the seven tables above has a write route.** See finding `DTA-08`.
...
requirement the assigner supplied. The check lives in
`apps/api/src/modules/technician/domain/technician.ts` and returns **every**
reason at once rather than the first, so the assigner does not learn one fact per
rejected attempt.

The refusal codes are fixed and are safe to show on screen:
...
*shape* of the value; the real graph lives in the tenant-overridable catalogue
...
| 7 | **Priority**                     | *nothing on a work order, job or assignment*                                                                    | —                                                 | —                                                       | **missing**     |
| 8 | **Instructions**                 | *nothing*; `wo.job_assignments.reason` is an **end-of-assignment** reason and is refused on creation             | —                                                 | —                                                       | **missing**     |
...
**Vehicle (1).** The work-order projection carries a vehicle *identifier*, not the
...
**Priority (7).** The word "priority" does appear in the schema, twice, and neither
occurrence is this one:

| where                    | what it is                                                                       |
| ------------------------ | ---------------------------------------------------------------------------------- |
| `dia.recommendations.priority` | How urgent a *diagnostic recommendation* is. `ck_recommendations_priority` — `low`, `medium`, `high`. |
...
column a later *ending* is required to fill, and putting text there at assignment
...
paused state", and the *reason* for pausing lives in the job's status history. Note
...
amounts are decimal strings paired with an ISO-4217 currency code, and the platform
ships no currency table — a tenant's currency comes from
`svc.price_lists.currency_code` and is immutable once set.
...
| Only two channels actually work: **`email` and `in_app`**. The frozen type also names `sms` and `whatsapp`; the policy refuses them with the rule `unsupported_channel`, because the database `CHECK` allows only the two. | `notification-policy.ts`, `ck_outbound_messages_channel`                            |
...
approve, preview, retire) and **not one of them is a read**. An operator cannot list
templates or read one back, which makes verifying the wording of an assignment
notification impossible today.
...
So a technician opening their inbox today would see that a message exists and what
state it is in, but nothing that would let the product take them to the vehicle. A
future assignment notification needs a way to carry that link. See `DTA-14`.
...
- **Both document reads are gated on a write code**, `shared.document.manage`.
  There is no `shared.document.read` code, and there is no document list or search
  — only one document by id and a vehicle-scoped list.
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **DTA-01** | No HTTP surface for `org.departments`. Nothing creates, lists, reads or edits a department. `org.department.manage` is seeded and used by no route.                                                                                                     | New Organisation Backend phase | New Administration Frontend  | Add create/list/read/update operations bound to `org.department.manage`; keep the read separate from the write code, or record why a write code gates the read.       |
...
| **DTA-08** | No route writes a technician profile, skill, certification or availability record. Seven `tech` tables are populated by no published operation; only `tech.labor_sessions` is written over HTTP.                                                       | Technician Backend (post-P1-19) | Administration Frontend      | Add profile and roster management operations. Until then, a new tenant's roster is empty and every eligibility check returns `profile-out-of-scope` or nothing.        |
...
| **DTA-10** | Nothing enqueues a notification when an assignment happens. `queueMessage` has exactly one call site in the API — the `POST /notifications` route. The `job.assigned` event is published and consumed by nobody.                                       | Shared-services Backend        | —                            | Build a consumer, or an explicit enqueue inside the assignment transaction. Decide which, and record why; an event with no consumer and a direct call have different failure modes. |
| **DTA-11** | Message templates have zero read operations. Eight write operations exist; none lists or reads a template back. The wording of an assignment notification could not be verified by an operator.                                                        | Shared-services Backend        | Administration Frontend      | Add list and read operations. Keep them on `org.settings.manage` unless the Owner decides otherwise — do **not** invent a `shared.template.*` code.                    |
| **DTA-12** | `EVT-NTF-001` (`message.delivery.changed`) is unimplemented and structurally so: the worker archetype has no request context and cannot publish. A delivery state change raises no event.                                                              | Shared-services Backend        | —                            | Either build a worker-side publication path or accept the message row plus its attempt history as the record. Do not mark the catalogue entry implemented without a producer. |
| **DTA-13** | A notification enqueue requires a consent decision granted within five minutes. The consent model that exists (`crm.consent_history`) is a **customer** model. There is no staff-consent source for notifying an employee about their own work.        | Shared-services Backend        | —                            | Decide whether internal operational notifications require consent at all; if not, the caller-supplied decision needs a defined internal source rather than an invented one. |
...
| **DTA-17** | Issued parts cannot be read back against a job. `wo.required_parts` records demand; `POST /stock-issues` performs the issue; no read joins the two. `inv.cost.view` is seeded and used by no route, so part cost is exposed nowhere.                   | Inventory Backend              | Task-board Frontend          | Add a read of issues by job or work order. Any cost figure must remain behind `inv.cost.view` and be a decimal string with an ISO currency code.                       |
...
| **DTA-19** | `GET /jobs/{jobId}/assignments` and `GET /technicians/{id}/queue` return a bare `{ items }` with no `LIMIT` in the query and no cursor. The assignment history is append-only and grows without bound.                                                 | Work-order Backend             | Task-board Frontend          | Convert both to the keyset page contract `{ items, nextCursor, hasMore }`. Never introduce a `total`.                                                                  |
| **DTA-20** | Expected labour minutes are unreachable over HTTP. `svc.standard_labor_times.standard_minutes` is returned only by an internal method used by pricing; `GET /services` omits it and there is no service detail read.                                   | Service-catalogue Backend      | Task-board Frontend          | Add a service detail read that publishes labour times. Keep `standardMinutes` a decimal string, as the existing projection already does.                               |
...
| ----------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------- |
| Organisational structure, including departments | P1-03 (schema)       | The table exists; no phase has yet built its HTTP surface.                                        |
...
| Reception and appointments                      | P1-18                | Write-only: twelve operations, **zero reads**. The seed states the omission is deliberate.        |
...
| Whether to buy a diagnostic-data, wiring-diagram or parts-data feed    | Purchasing or contracting a paid data provider is a **commercial decision reserved to the Product Owner**. This document recommends only that any such feed be *evaluated*, never that one be bought. |
| A cost or rate for labour                                             | No labour rate exists in the `tech` schema, and `inv.cost.view` is seeded but unused. Any money figure must be a decimal string with an ISO currency code, and the currency comes from `svc.price_lists.currency_code`. |
