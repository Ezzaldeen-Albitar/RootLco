...
**On the owning Frontend phase column.** Every row below reads "Not allocated",
and that is a deliberate statement rather than an omission. The parts surface has
**no named Frontend phase anywhere in this repository**. The closed phase records
name P1-28 (reception), P1-29 (work order) and P1-30 (service catalogue, pricing,
quotation and inventory) as Frontend counterparts of Backend phases, and
`docs/phase-1/phase-1-11/**` names its counterpart only jointly as "P1-30/1-31"
without dividing the work. **P1-30 is the nearest candidate for the whole of §15**
because it is the phase the P1-10 record names as owning the inventory Frontend —
but no scope statement, task register or start date exists for it, so writing
P1-30 into these rows would assert a commitment nobody has made. What would
allocate them is a Product Owner scope decision for P1-30 and P1-31, recorded in
the canonical phase plan. Until then, "Not allocated" is the honest entry, and
the consolidated register at `docs/product/README.md` carries the same value.
...
a route and gate it, not to design anything new. The other twelve need something
more: a schema change, a change to a contract that already exists, a whole
missing domain, or an Owner decision taken first. The shape of the whole list is
that the parts domain can record what happens and can prove it happened, and can
show almost none of it back.
