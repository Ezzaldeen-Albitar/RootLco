...
The twelve codes no route requires are listed in §7.6. They must never be cited
as gating anything today.
...
| Work order                | `wo.work_orders.state`     | Tenant-configured, via `wo.work_order_states` and `wo.work_order_transitions`. Format-checked only.        | A transition is legal only where an active edge exists. Terminals frozen.           |
...
| Job                       | `wo.jobs.state`            | Tenant-configured, via `wo.job_states` and `wo.job_transitions`.                                          | Same mechanism as the work order, independently.                                     |
...
| Customer (lifecycle)      | `crm.business_partners.lifecycle_status` | Own vocabulary, defaulting to `prospect`                                                     | Governance operations.                                                               |
| Customer (commercial)     | `crm.business_partners.commercial_status` | **A second field**, defaulting to `normal`                                                  | Restrictions and blocks.                                                             |
...
`rec.custody_history`, opened when the reception visit is created and closed only
when a delivery completes. The delivery module's own note records that
completing a delivery "closes the reception custody chain". A status field would
lose the middle of that chain; the chain is what a dispute is settled from.

### 3.4 The entities this workflow models

Fifteen distinct things, none of which is a status on another:
...
| Timeline / history            | `crm.timeline_events`, `veh.vehicle_attribute_history`, `wo.work_order_status_history`, `iam.audit_events` |
...
| Free text          | `complaint_text`, stored **restricted**                        | `description`, required non-blank                                          |
| Written by         | `rec.reception-condition-evidence` with `kind: 'complaint'`    | `dia.diagnostic-finding-record`                                            |
| Permission         | `rec.reception.evidence.manage`                                | `dia.diagnostic.record`                                                    |
...
| **Exception**               | A likely duplicate. `crm.duplicate-list` surfaces open candidates; `crm.duplicate-scan` is a **privileged write that emits an audit record**, so it is never run on keystroke                       |
...
| **Exception**               | A VIN already in use. The platform enforces active-VIN uniqueness and surfaces it as a conflict. `veh.vin_verifications` is a table **no code reads or writes** — there is no check-digit workflow and no override policy |
...
| **Exception**               | A mis-transcription. There is no edit: a superseding row cites `correction_of`, and both remain                                                               |
| **Evidence**                | `rec.complaints` (text stored **restricted**), `rec.complaint_details`, optionally a linked `shared.documents` row                                             |
| **Owning Backend contract** | **PARTIAL** — `rec.reception-condition-evidence` with `kind: 'complaint'`. **No read.** **WF-06**                                                              |
| **Owning Frontend phase**   | P1-28                                                                                                                                                        |
...
| **Evidence**                | Intended: `dia.dtc_records`. At reception, the only related evidence available is a **dashboard warning-lamp observation** (`rec.warning_light_observations` against the `rec.warning_light_codes` catalogue), which is an observation of a lamp, not a scan-tool code |
...
repository only inside `tests/backend/p1-19-helpers.ts` as a fixture constant used
to build a diagnostic template during a test. It is test scaffolding. It is not a
shipped catalogue entry, and citing it as one would be a fabrication.
...
| **Input**                   | A channel — `email`, `sms`, `whatsapp` or `in_app` — and a recipient reference                                                                                       |
...
| **Owning Backend contract** | **PARTIAL** — `shared.notification-enqueue` POST `/notifications` · `shared.notification-list` GET `/notifications` · `shared.notification-read` · `shared.notification-delivery-list`. **Assignment does not enqueue anything**, and there is no fan-out from a job to its assigned technicians. **WF-15** |
| **Owning Frontend phase**   | P1-29 for the trigger; the notification shell itself came from P1-26                                                                                                 |

Two constraints a screen must respect. `recipientRef` is a **UUID by schema, not by
convention** — an address cannot pass validation, which is what removes
arbitrary-destination sending. And `GET /notifications` returns **the authenticated
caller's own inbox only**; the recipient is never an input, so a supervisor cannot
read a technician's notifications.
...
**Pause and resume do not exist.** The permission `tech.labor.record` is described
as "Start, **pause, resume** and stop labor sessions", and only start and stop
operations are published. A screen must not offer a pause button. There is also
no `GET /labor-sessions/{sessionId}`. Recorded as **WF-17**.
...
be searched but never populated over the API. `inv.cost.view` is likewise unused,
so item cost is exposed nowhere. Recorded as **WF-18**.
...
| **Owning Backend contract** | **PARTIAL** — `inv.external-purchase-part-create` POST `/external-purchase-parts`. **No read of any kind.** **WF-18**                                               |
| **Owning Frontend phase**   | P1-30                                                                                                                                                             |
...
discouraged. The platform ships **no currency table and no jurisdiction defaults**:
a tenant's currency comes from its own price list, which is immutable once set.
Nothing may present a floating-point amount, and nothing may infer a currency
from a country.
...
| **Owning Backend contract** | **PRESENT** — `quo.quotation-item-decide` POST `/quotation-items/{quotationItemId}/decisions` · `quo.quotation-revision-decide` · `wo.additional-work-request` POST `/work-orders/{workOrderId}/additional-work` · `wo.additional-work-approval` POST `/additional-work/{requestId}/approval` · `wo.additional-work-detail-read` GET · `wo.additional-work-detail-record` PUT · `wo.additional-work-fulfillment` · `wo.additional-work-withdrawal` |
...
**The state names are not established.** `wo.work_order_states` and `wo.job_states`
are tenant-configured catalogues, and the repository ships **no seeded workflow** —
consistent with the standing no-fake-data policy. There is therefore no default
set of state names a screen may hard-code, and no default set to show the Owner in
a demonstration. Establishing the pilot tenant's state graph is configuration work
that must precede any P1-29 screen. Recorded as **WF-21**.
...
| **Permission**              | Create and resolve `qms.rework.manage` · sign off **`qms.rework.sign_off`** · read or record the **cost** additionally requires **`iam.sensitive.view`**                              |
...
`wo.work_order.transition`. A screen must present it as "record a reopen request
(which will be refused and logged)", never as "reopen".
...
| **Exception**               | The history is not in one place, and no operation assembles it                                                                                                                   |
| **Evidence**                | Append-only tables throughout: status histories, `crm.timeline_events`, `veh.vehicle_attribute_history`, `iam.audit_events`, and evidence rows bound to **exact immutable document versions**, never to a substitutable document |
...
| **WF-07**  | No document list or search; both document reads are gated on the **write** code `shared.document.manage`; upload is authorisation-only and the API accepts no file body                          | P1-15                | Not established       | **Blocked on `P1-OD-025`.** Once decided: add a `shared.document.read` code and a document list operation                                                   |
...
| **WF-09**  | **No operational record carries a department.** `org.departments` is referenced only by `iam.grant_scopes`; `org.department.manage` is required by no route; there is no department read or write operation | P1-14 / P1-19        | P1-29                 | Decide whether departments route work. If they do, this is new schema, new operations and new permissions — not a screen change                             |
...
| **WF-17**  | No pause or resume for a labour session, although the permission describes both; no `GET /labor-sessions/{sessionId}`                                                                          | P1-19                | P1-29                 | Either publish pause and resume operations or correct the permission description. A screen must not offer a button with no operation behind it              |
...
| **WF-21**  | No work-order or job state graph ships. The state names a screen would display are tenant configuration and are **not established**                                                             | P1-19                | P1-29                 | Configure the pilot tenant's state graph before any P1-29 screen work begins. Screens must never hard-code a state name                                     |
...
together. There is no conversion path anywhere in the platform. There is no
currency table and no jurisdiction default: a tenant's currency comes from its own
price list and is immutable once set. No screen may show a floating-point amount,
infer a currency from a country, or add two amounts in different currencies.
...
Most reads are class `none`. Six reads are not, and a screen should tell the
operator so before they open one:

| read                                                     | class      | why                            |
| -------------------------------------------------------- | ---------- | ------------------------------ |
| `iam.audit-event-list` / `iam.audit-event-detail`        | `security` | Looking at the log is logged   |
...
**The twelve permission codes required by no route today.** None of them may be
cited as gating anything: `iam.login.view_all`, `inv.cost.view`, `inv.item.manage`,
`org.branch.manage`, `org.company.manage`, `org.department.manage`,
`org.subscription.manage`, `org.tax.manage`, `rpt.report.configure`,
`sal.reversal.approve`, `wo.work_order.create`, `wty.policy.manage`.

**Six reads are gated on a write or issue code**, because no matching read code
exists: `GET /attachments/documents/{documentId}` and
`GET /vehicles/{vehicleId}/documents` → `shared.document.manage`;
`GET /invoices/{invoiceId}` → `sal.invoice.manage`; `GET /payment-methods` →
`sal.payment.record`; `GET /warranties/{warrantyId}` → `wty.warranty.issue`;
`GET /iam/grants/{grantId}/scopes` → `iam.role.read`.

### 7.7 Sensitive data

`iam.sensitive.view` is an **additive second gate**, not an alternative one. Four
operations require it on top of their domain permission — the additional-work
money detail (read and write) and the rework cost (read and write). Certain
filterable fields are also marked sensitive, and **filtering on them requires
`iam.sensitive.view`**, because filtering is a read oracle: a caller who can narrow
a list by a value can discover that value without ever seeing it.
...
Named at every step where they bind: **P1-OD-017** at steps 2, 3 and 24 (duplicate
and merge), and **P1-OD-025** at step 5 (media). Neither is worked around anywhere
in this document, and no screen may assert a rule either decision would set.
...
demonstration customer, vehicle, work order, price or invoice. The state graphs at
step 21 are unseeded for exactly this reason, and configuring them for a pilot
tenant is configuration, not seed data.
...
| The pilot tenant's work-order and job state names                     | **Not established** | Tenant configuration of `wo.work_order_states` and `wo.job_states`, before any P1-29 screen work    |
