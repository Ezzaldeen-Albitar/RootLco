...
| **P1-29** | Named in the P1-9 record as the Work Order frontend                                           | Not started |
| **P1-30** | Named in the P1-10 record as the service catalogue, pricing, quotation and inventory frontend | Not started |
| **P1-31** | Named only jointly, as "P1-30/1-31", in the P1-11 record                                      | Not started |
...
**The P1-30 / P1-31 split is not established.** The P1-11 record names its
frontend counterpart as "P1-30/1-31" and does not divide the work between the
two. Where a row in §13 names P1-31 alone for the billing, payment, delivery or
warranty surface, that attribution is **provisional** — it follows the subject
matter, not a recorded scope statement, and a Product Owner scope decision would
settle it.
...
| **Internal parts**             | `inv.item_master`, `inv.stock_reservations`, `inv.part_issues`            | **No.** No path exists from a part issue to a quotation line or an invoice line (finding `PPD-01`)         |
...
reason is honest rather than lazy: warranty records are generated _from_ a
...
**The Status column below reports on a server contract, never on a capability.**
"Exists" means a published operation was read out of a route file in this
repository. It does **not** mean a screen calls it, that a member of staff can
reach it, or that the stage works end to end. **No screen exists for any of the
twenty stages**, other than the vehicle reads P1-27 itself delivers, and this
document schedules none.
...
Twelve operations exist — eight on receptions, four on appointments — and none of
them is a `GET`. The permission catalogue states the reason in its own words
against the appointment codes: no read code is registered because no read
operation exists, and an unused permission is configuration that cannot be
tested. The reception codes carry no read code either, for the same reason,
though the catalogue does not repeat the note there. The practical consequence
for the commercial process is that a service adviser cannot look up the visit a
work order came from. Recorded as `PPD-06`.
...
must carry a document version while any other kind must not. What that document
version may contain is bound by **`P1-OD-025`**, which is open — see §12.2.

**One request may still decide a whole revision.** Recording the customer's
answer line by line is not the only route: `POST /quotation-revisions/{revisionId}/decisions`
records one decision against every still-undecided line of a revision in a single
transaction. It is an orchestration, not a second place a decision is stored —
every fact it writes lands in the same per-item rows, and a line already carrying
the opposite decision aborts the whole request rather than being overwritten.
...
any kind**, because there is no field for one. Which customer record that payer
is may be affected by **`P1-OD-017`**, which is open — see §12.1.
...
| The invoice has a status of `partially_paid` | **It does not.** `partially_paid` and `paid` exist only in the _status history_ vocabulary and are storable on the invoice row by no code |
...
2. **A caller without `sal.finance.view` is denied, not told zero.** The
   calculation reads three permission-gated sources using the caller's own
   authority, so a caller lacking that permission would compute `0 − 0 − 0` and
   receive a figure indistinguishable from a fully settled invoice, with no error
   and no signal.
...
**"Required evidence present" is not a blocker of its own.** The brief lists it
as a precondition, and the register has no code for it. What the six blockers
actually reach is evidence held by _another_ record: `B4` is unmet unless a
diagnostic report has been completed, and `B5` is unmet unless a quality-control
record exists and passed. Evidence attached to a reception, an inspection or an
approval is not counted or checked at closure, and no contract expresses "this
job needed three photographs and has two". A completeness rule of that shape
would have to be modelled before it could be gated, and modelling it is Backend
work in the owning phase, not a screen.
...
strictness: three of them — `receiver_not_verified`, `checklist_incomplete` and
`signature_missing` — are enforced inside the database routine itself (§7.2), so
an "override" would be accepted by the application and then fail at the database
...
database enforces both halves of that rule, not only the first.
...
| **No signature or identity content crosses the boundary**                                        | Signatures and identity evidence are references to immutable document versions. What such a document may be is bound by **`P1-OD-025`**, open — §12.2 |
...
| `GET /warranties/{warrantyId}` is gated on `wty.warranty.issue`    | There is no `wty.warranty.read` code to gate it with — §10.1, §13.1 |
...
| `rec.reception.convert`        | high   | Convert an approved reception into a work order                    |
...
| `veh.vehicle.status.manage`    | medium | Change a vehicle's lifecycle and workshop status (§9)              |
...
Four reads **on this surface** are gated on a write or issue code, because the
catalogue holds no read code for them. A screen designer must know that showing
any of these requires granting a write authority:
...
The same pattern occurs elsewhere in the platform — document reads gated on
`shared.document.manage`, approval-limit reads on `iam.approval.manage`, report
exports on `rpt.export`. A platform-wide total is **not established here**,
because only this surface was read exhaustively for this document.
...
against the other, and **there is no approved rule for reconciling them**.

The distinction that matters here, stated precisely because it is easy to get
wrong in either direction:

| Statement                                                         | True?                                                                                                                                                                                                              |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A server merge operation exists for a customer                    | **Yes** — `POST /customers/{customerId}/merge`, permission `crm.customer.merge`, and it demands a recorded approval reference                                                                                      |
| A server merge operation exists for a vehicle                     | **Yes** — `POST /vehicles/{vehicleId}/merge`, permission `veh.vehicle.merge`                                                                                                                                       |
| An approved business rule says **when** two records may be merged | **No.** That is `P1-OD-017`, and it is open                                                                                                                                                                        |
| Any screen may offer a merge action today                         | **No.** The P1-27 plan requires the merge affordance to be **absent**, not disabled — a disabled control would imply the capability exists and the user lacks permission, which is a different and false statement |

So the correct wording for any commercial screen is that duplicate **review**
exists and duplicate **merge** is pending an Owner decision. Neither "merge is
impossible" nor "merge is available" would be true.
...
that such an option be _evaluated_ if and when the need is demonstrated. It does
...
`P1-27-INT-nnn`: that series is the P1-27 finding register and numbers 001 to 009
are already allocated. Registering these under a phase series is a later
governance step.

Where a row names a single owning Frontend phase for the billing, payment,
delivery or warranty surface, read it with §1.3: the P1-30 / P1-31 split is not
established, and the attribution is provisional.
...
| **PPD-07** | **Over-allocation is bounded only inside the allocation routine.** No constraint or trigger bounds the sum, and the runtime role holds a raw insert grant on the allocation table                                                                      | P1-11                | **None.** An invariant about the only legal write path is Backend-internal and reaches no screen    | Keep the routine as the only write path and keep the structural test that proves the repository contains no raw insert                                                   |
