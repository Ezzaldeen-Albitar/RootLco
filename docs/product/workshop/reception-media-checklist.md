...
| **`INS-3` damage photos**   | An `evidenceDocumentId` on a condition item or a damage mark is the one association contract in this whole checklist that genuinely exists in the Backend. It is still unusable in practice: the document it would point at cannot be created, because no upload can be authorised (`RMC-05`).                                                                                                                                                |
...
| 3   | Permission             | `shared.document.manage` for every document read and write today. No `shared.document.read` code is seeded in `supabase/seeds/04_iam_permission_catalog.sql`. See `RMC-07`. |
...
| **This document does not claim upload completion.**      | `P1-OD-025` remains authoritative for accepted file types, size limits and retention. Nothing described here is implemented, and no upload could complete even if it were: §5.4 shows the first step cannot be authorised at all, and §5.3 shows no registered file can be downloaded. The only truthful statement about media today is that **Backend authorisation and registration contracts are defined while acceptance is unreachable** — never that media capture works, and never that a capture screen exists. |
...
| `RMC-07` | No `shared.document.read` permission is seeded. Both document reads are gated on `shared.document.manage`, a write code that also authorises upload, linking and download authorisation. `apps/web/src/config/navigation.ts` already names `shared.document.read` on a `planned` navigation entry, so a Frontend configuration references a code the catalogue does not hold. | P1-14 with P1-15     | P1-28 (and the P1-26 navigation configuration)     | Seed the read code, re-gate the two reads, re-run the catalogue check, and reconcile the navigation entry against the result.                                   |
