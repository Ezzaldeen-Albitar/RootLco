...
**Status:** Planning — no capture screen is implemented · **Owner:** Eng.
Ezzaldeen Al-Bitar · **Recorded:** 2026-08-06 · **Phase authority:** P1-27
Owner-acceptance remediation

**Corrected:** 2026-09-09 (Owner). The statements about whether a registered file
can be retrieved were true on 2026-08-06 and are no longer true: the P1-OD-025
wave added the scanner handoff, the seeded evidence categories and the read
permission code. Every superseded statement is kept in place and marked, because
the reason a design looks as it does is part of the record. The corrected
sections are §3.5, §3.6, §4.2, §4.6, §5.1, §5.2, §5.3, §5.4, §5.6, the findings
register and recommendations in §7 and §8, and §10. The access and privacy rules
in §4.6 and §5.6 are unchanged in substance: nothing in this correction relaxes
tenant isolation, reachability, permission, scope, version state or the
short-lived signed URL, and none of them was ever conditional on acceptance being
unreachable.
...
The 2026-09-09 corrections were read on `develop`, from
`supabase/migrations/20260815090000_shared_reception_evidence_foundation.sql`,
`supabase/seeds/05_shared_reference.sql`,
`apps/api/src/modules/shared-services/application/attachment-service.ts`,
`apps/api/src/modules/shared-services/provider/**` and the attachment route
handlers. Line numbers cited in a correction are the lines as read on that date.
...
| **Link**                | A row in `shared.document_links` tying a document to a business record. A link is what makes a document _reachable_. Knowing a document identifier is not reachability, and neither is a storage key. |
...
| **Verification**   | `veh.vin_verifications` exists as a table that **no application code under `apps/api/src` reads or writes**, confirmed by `docs/phase-1/phase-1-17/read-contract-remediation.md` §4. There is no check-digit routine, no override policy and no permission code for VIN verification. |
...
| **Acceptance**        | `accepted` is deliberately **not** required. Recorded 2026-08-06 as "because acceptance is unreachable while no role may write scan results"; **corrected 2026-09-09** — acceptance is reachable since P1-OD-025 (§5.3), but not on every deployment, so the contract still does not demand it. A `rejected` or `quarantined` version is refused. |
...
| **Findings**                | `kind: "condition_item"` hangs a finding off that header: `findingCategory` from `scratch`, `dent`, `crack`, `wear`, `missing_part`, `malfunction`, `other`; `vehicleZone`; optional `severity` from `minor`, `moderate`, `major`, `critical`; optional note and `evidenceDocumentId`. A damage **mark** carries a different vocabulary — `scratch`, `dent`, `crack`, `chip`, `rust`, `missing`, `other` — and the two must not be conflated.                                                                                     |
...
| **`INS-3` damage photos**   | An `evidenceDocumentId` on a condition item or a damage mark is the one association contract in this whole checklist that genuinely exists in the Backend. Recorded 2026-08-06 as "still unusable in practice: the document it would point at cannot be created, because no upload can be authorised (`RMC-05`)"; **corrected 2026-09-09** — authorisation now resolves against the seeded categories (§5.4), so what stands between this contract and use is an object store the Owner has not selected, not a missing category. |
...
| Accepted             | The file passed scanning and may be downloaded.                             | **Reachable since the P1-OD-025 wave** (correction of 2026-09-09). `POST /attachments/versions` calls `AttachmentService.registerVersionAndScan`, which advances the version `pending → scanning → accepted` or `quarantined` in the same request when the configured storage adapter can read the stored object back. It is **not** reachable under `STORAGE_PROVIDER=unconfigured` or `local_fake`, which cannot read and leave the version `pending`. See §5.3. |
| Rejected             | A reviewer refused the pending version.                                     | `POST /attachments/versions/{versionId}/rejection`, terminal.                                                                                                                                                                                                                                                                                                                                                                                                      |
| Quarantined          | A scanner found a threat.                                                   | **Produced by the scanner path** (correction of 2026-09-09): the verdict starts at `error` and must EARN `clean` (`apps/api/src/modules/shared-services/application/attachment-service.ts:621`), so a version is quarantined when the object cannot be read, cannot be decoded, or does not match its declared content type, byte size or checksum (`:646`, `:661`), and when an EICAR test signature is found in the bytes (`:638`).                              |

The honest description of the best reachable state today is **"registered and
pending until a scan verdict is recorded, and downloadable only once
accepted"** — and where no readable storage adapter is configured, the state
stops at `pending`. A screen must not present that as "uploaded", and must never
...
| Reception visit → work order | work order              | `POST /receptions/{receptionId}/convert-to-work-order` — operation `rec.reception-convert-to-work-order`, permission `rec.reception.convert`. This is the **only** way a work order is created; `/work-orders` exports `GET` alone. |
...
| 3   | Permission             | **Corrected 2026-09-09.** `shared.document.read` **is** seeded — `supabase/seeds/04_iam_permission_catalog.sql:104` — and two operations are gated on it: `shared.document-category-list` and `shared.document-version-read`. The write code `shared.document.manage` still gates the remaining document reads and every write, so `RMC-07` is narrowed, not closed. Recorded 2026-08-06: "no `shared.document.read` code is seeded", true when written. |
...
### 5.1 The eleven document-domain operations

**Corrected 2026-09-09.** This section was recorded on 2026-08-06 as "the nine
document-domain operations". The P1-OD-025 wave published two more, both gated on
the read code, and the two rows marked _added_ below are them. The heading and
the count are corrected rather than the table being left to disagree with itself;
nothing recorded in 2026-08-06 has been removed.
...
| `shared.document-category-list`        | GET    | `/attachments/categories`                                     | `shared.document.read`    | tenant |
| `shared.document-version-read`         | GET    | `/attachments/versions/{versionId}`                           | `shared.document.read`    | tenant |

The last two rows were _added_ by the P1-OD-025 wave and did not exist when this
...
- **Both document reads are gated on a write code.** `shared.document.manage` is
  ...
  and links". A member of staff who should only _view_ reception photographs
  cannot be granted viewing alone.
  **Narrowed 2026-09-09.** The sentence remains true of the two reads it names —
  `shared.document-read` and `veh.vehicle-document-list` are still gated on
  `shared.document.manage` — but it is no longer true of the document surface as
  a whole: the category list and the version read added by P1-OD-025 are gated on
  `shared.document.read`, which is seeded. Viewing-only access to those two is
  grantable; viewing-only access to the document record and the vehicle document
  list is still not.
...
**Corrected 2026-09-09.** Two sentences above are now out of date, and are kept
because the design intent they record has not changed. A second adapter exists:
`s3_compatible`
(`apps/api/src/modules/shared-services/provider/s3-storage-provider.ts:123`,
selected in `apps/api/src/modules/shared-services/index.ts:234`), the only one
that can read an object back. `unconfigured` is still the default and still
refuses every call, and `local_fake` is still the deterministic adapter that
issues no usable URL, so step 3 has no provider _unless a deployment configures
the S3-compatible one_. Step 5 also does more than the table says: the platform
records the pending version and then, when the adapter can read, scans it in the
same request (§5.3).
...
### 5.3 Acceptance requires a clean scan, and was unreachable until P1-OD-025

**Corrected 2026-09-09.** The heading of this section read "Acceptance is
unreachable, and that is deliberate". That was true when it was recorded and is
no longer true. The rule that has never changed is the one the heading now
states: acceptance is earned by a clean scan verdict and by nothing else.

What is still true, read from the migrations:

1. `shared.guard_document_version_transition()` permits a version to reach
   `accepted` only when a `clean` row exists in `shared.file_scan_results` for
   that version, and refuses outright if any `infected` row exists.
2. A download is issued only for an `accepted` version, and `accepted`,
   `quarantined` and `rejected` are terminal — a refused or quarantined version
   can never become downloadable later.
3. The request runtime still cannot write a scan verdict freely. It holds an
   `INSERT` on `shared.file_scan_results` and a **column** grant on
   `shared.document_versions.status` — not a table grant — so it can record a
   verdict and move a status, and can rewrite neither an existing verdict nor any
   other column.

**Historical note — recorded 2026-08-06, superseded by
`supabase/migrations/20260815090000_shared_reception_evidence_foundation.sql`.**
The three points below were accurate against the schema of 2026-08-06 and are
kept because they are the reason the current design looks as it does. They must
not be read as a statement about the platform today.

- _Superseded._ "`supabase/migrations/20260728090000_shared_services_runtime_write_capabilities.sql`
  ...
  this phase, so a document version cannot reach `accepted`." The 2026-08-15
  migration added the scanner handoff that the earlier migration deliberately
  withheld: the policy `ins_file_scan_results_scanner` on
  `shared.file_scan_results`, the policy `upd_document_versions_lifecycle` on
  `shared.document_versions`, the grants `INSERT ON shared.file_scan_results` and
  `UPDATE(status) ON shared.document_versions` to `app_runtime`, and the two
  functions `shared.begin_document_scan` and `shared.complete_document_scan`.
- _Superseded._ "The only status update the request runtime holds is
  `pending → rejected`, through the policy `upd_document_versions_reject`." The
  status constraint now admits `scanning` and `quarantined` as well, and
  `upd_document_versions_lifecycle` is the policy that carries the scan
  lifecycle.
- _Superseded._ "**Therefore no file registered through this API can be
  downloaded today.** A media screen that offers a 'view photograph' action would
  be offering an action that always fails. Recorded as `RMC-03`." The refusal a
  caller meets while a version is not `accepted` is `ERR-DOC-001`, and it is a
  **state check, not an impossibility** — the docblock on
  `AttachmentService.requestDownload` says so in the source
  (`apps/api/src/modules/shared-services/application/attachment-service.ts:688-698`).
  Whether any given deployment can reach `accepted` is a deployment question, not
  a platform one; see the reachability paragraph below. `RMC-03` is therefore
  closed as a permanent blocker and survives only as the deployment condition.

**Reachability is deployment-conditional.** `registerVersionAndScan` scans only
when the configured storage adapter can read the object back
(`storageProviderCanRead`, `attachment-service.ts:606`). Only the
`s3_compatible` adapter implements `readObject`
(`apps/api/src/modules/shared-services/provider/s3-storage-provider.ts:123`,
method at `:247`). Under `STORAGE_PROVIDER=unconfigured` or `local_fake` the
service returns the version as `pending` with `scannerAvailable: false`, so on
those settings nothing reaches `accepted` and nothing can be downloaded — the
2026-08-06 sentence remains a correct description of those two deployments, and
of no others.

The original withholding was correct engineering, not an oversight: if the
runtime could write scan verdicts freely, an `infected` verdict could be quietly
rewritten inside a tenant, and the table has no triggers to prevent it. That
argument is why the handoff added in 2026-08-15 is a column grant and two
`SECURITY INVOKER` functions rather than a table grant.

### 5.4 Document categories are seeded, and the first upload step now resolves

**Corrected 2026-09-09.** This section was recorded on 2026-08-06 under the
heading "No document category exists, so no upload can even be authorised". The
mechanism it describes is unchanged; the fact it rested on is not.
...
- **is seeded** — `supabase/seeds/05_shared_reference.sql:34` inserts the
  P1-OD-025 platform evidence categories at platform scope (`tenant_id NULL`),
  each carrying a `business_link_purpose` and a
  `device_capture_timestamp_required` flag added by the 2026-08-15 migration. A
  tenant row of the same code overrides a platform row through the existing
  dual-scope resolution. _Recorded 2026-08-06, superseded:_ "is **not seeded** —
  no file under `supabase/seeds/` inserts into it, and no migration does
  either";
- has **no management route** — there is no `/document-categories` path segment
  in the API. A read now exists: `GET /attachments/categories`
  (`shared.document-category-list`), gated on `shared.document.read`. Creating or
  editing a category is still not an operation any caller can perform;
...
So the first step of the upload sequence resolves for the seeded categories, and
`RMC-05` is closed on its stated cause. _Recorded 2026-08-06, superseded:_ "So
the very first step of the upload sequence cannot succeed. Recorded as `RMC-05`.
This is the cleanest possible demonstration that upload is not implemented: it is
not merely unfinished, it is not reachable." What remains true is §5.2: no route
accepts a file body, and step 3 of the sequence needs an object store the Owner
has not selected.
...
| **A signed URL is a bearer credential.**                 | It is never logged, never audited and never returned in an error. The application logs the _fact_ of an issuance with its purpose and time-to-live, and nothing else.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **Nothing here claims malware scanning.**                | **Still true, and now more important — restated 2026-09-09.** Recorded as: "No scanner is configured, `scanState` reports `scannerAvailable: false` as a hard-coded literal, and no code path fabricates a verdict." The registration path now runs an in-process content check, `rootlco_image_guard`, which reads the stored object back, verifies it against its declared content type, byte size and checksum, decodes it, and detects the EICAR **test** signature. That is **not** an anti-malware product and no anti-malware product is integrated. `scanState` still reports `scannerAvailable: false`; `registerVersionAndScan` reports `true` only for the adapter that actually read the object. No code path fabricates a verdict, and the verdict starts at `error` so `clean` has to be earned. |
| **This document does not claim upload completion.**      | `P1-OD-025` remains authoritative for accepted file types, size limits and retention. No screen described here is implemented. **Corrected 2026-09-09:** recorded as "no upload could complete even if it were: §5.4 shows the first step cannot be authorised at all, and §5.3 shows no registered file can be downloaded", and as "the only truthful statement about media today is that **Backend authorisation and registration contracts are defined while acceptance is unreachable**". The truthful statement now is that the Backend authorisation, registration, scan and acceptance contracts are defined and reachable **where a readable object store is configured**, and that no capture screen exists. Never that media capture works, and never that a capture screen exists.                  |
...
already holds `001`–`009` in `docs/phase-1/phase-1-27/findings.md` — and whoever
maintains that register must decide which of these to promote into it and under
what numbers.

`P1-28` is the Reception and Appointment Frontend phase, recorded in
`docs/phase-1/phase-1-8/p1-18-p1-28-boundaries.md`. It has no plan and no scope
document in this repository — there is no `docs/phase-1/phase-1-28/` directory —
so every reference to it below is a placement, not a commitment.
...
| `RMC-03` | **Superseded 2026-09-09 — see §5.3.** Recorded as: "Version acceptance is unreachable: it requires a `clean` row in `shared.file_scan_results`, which is granted to no role. No registered file can ever be downloaded." The scanner handoff was added on 2026-08-15 and acceptance is now reachable wherever the configured storage adapter can read the object; what survives is the deployment condition, not the blocker.                                                                                                                                                                                         | P1-15                | P1-28                                              | Introduce a scanning component with its own role and grant, or record an Owner-accepted exception. Provider selection is an evaluation.                         |
...
| `RMC-05` | **Superseded 2026-09-09 — see §5.4.** Recorded as: "No document category exists. `shared.document_categories` is unseeded and has no management route, so `POST /attachments/upload-authorizations` fails with `ERR-RES-001` for every caller." The platform evidence categories are seeded; a read route exists and a management route still does not.                                                                                                                                                                                                                                                               | P1-15                | P1-28                                              | Owner decides the category set, types and ceilings under `P1-OD-025`; then add a configuration path that does not violate the no-fake-data policy.              |
...
| `RMC-07` | **Narrowed 2026-09-09 — see §4.6 and §5.1.** Recorded as: "No `shared.document.read` permission is seeded. Both document reads are gated on `shared.document.manage`, a write code that also authorises upload, linking and download authorisation. `apps/web/src/config/navigation.ts` already names `shared.document.read` on a `planned` navigation entry, so a Frontend configuration references a code the catalogue does not hold." The code is seeded and gates two operations; `shared.document-read` and `veh.vehicle-document-list` are still gated on the write code, which is the part that remains open. | P1-14 with P1-15     | P1-28 (and the P1-26 navigation configuration)     | Seed the read code, re-gate the two reads, re-run the catalogue check, and reconcile the navigation entry against the result.                                   |
...
| 3   | Recorded 2026-08-06: "Do not build a 'view photograph' action while `RMC-03` is open. An action that always fails is worse than an absent one." **Corrected 2026-09-09** — the action no longer always fails, but it still fails wherever no readable object store is configured, so a view action must handle a non-accepted version as a state, not as an error. |
...
| -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The document-domain operations and their permissions (nine when recorded, eleven since P1-OD-025 — §5.1) | `apps/api/src/app/api/v1/attachments/**/route.ts`, `apps/api/src/app/api/v1/vehicles/[vehicleId]/documents/route.ts`                                                    |
