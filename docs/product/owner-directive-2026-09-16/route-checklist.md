...
| `/inventory/parts`                  | `apps/web/src/features/inventory/components/PartsScreen.tsx`                 | fixed (707)                                        | pass — opens on the work order it was reached from | fixed (712) — the job is found by name, not typed | pass — the item finder searches the catalogue   | pass | fixed (712) | pass | fixed (712) | pass |
...
picker, `WorkOrderPicker`: the working branch's jobs searched on the server through
`wo.work-order-list` (`q` over the number, a party's name, a plate or the chassis number), the
term held in memory and never in the address, and the picker offered only with
`wo.work_order.read`. The pricing and service branch pickers consult the working context first,
...
| `/services/[serviceId]`     | `apps/web/src/features/services/components/ServiceDetailScreen.tsx`     | fixed (709) — availability starts at the working branch | pass                                               | fixed (709)                                                   | n/a — one record, reached by address            | n/a                            | fixed (709) — the branch complaint on the one branch control | not swept | fixed (709) | not swept |
| `/pricing`                  | `apps/web/src/features/pricing/components/PricingScreen.tsx`            | fixed (709) — the lookup starts at the working branch   | pass                                               | fixed (709) for the branch; remaining — see below             | not swept                                       | not swept                      | fixed (709) — a submit with no branch to send is held        | not swept | fixed (709) | not swept |
...
| `/quotations`               | `apps/web/src/features/quotations/components/QuotationsScreen.tsx`      | pass                                                    | pass — opens on the work order it was reached from | fixed (709) — the job is found by name; remaining — see below | fixed (709) — the job search                    | not swept                      | fixed (709) — the chooser                                    | not swept | fixed (709) | not swept |
...
| `/invoices`                 | `apps/web/src/features/billing/components/InvoiceScreen.tsx`            | pass                                                    | pass — opens on the work order it was reached from | fixed (709) — the job is found by name; remaining — see below | fixed (709) — the job search                    | n/a — one work order's invoice | fixed (709) — the chooser                                    | not swept | fixed (709) | not swept |
| `/payments`                 | `apps/web/src/features/payments/components/PaymentsScreen.tsx`          | fixed (709) — the branch is the working context's       | fixed (709) — reads on arrival for that branch     | remaining — see below                                         | n/a — the read publishes no free-text parameter | not swept                      | not swept                                                    | not swept | fixed (709) | not swept |
...
5. `/inventory/parts` — the issue form's required-part box is a reference, prefilled from the
   required-part row it was opened from.
6. `/credit-notes` — reads the working branch through the shared inventory section (707); the
   screen's own docblock still describes a branch "chosen first", and it passes two props the
   section no longer reads.
7. `/quotations/[quotationId]` and every `not swept` cell above.
