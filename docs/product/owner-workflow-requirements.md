...
| Internal inventory parts                                                | Delivered — location list (A2 S-16), the item/category/location writers (#322), the setup and opening-stock screens (#323), proved end to end on a fresh organisation (`w9-acceptance-record.md`) |
...
| Complete delivery history                                    | ~~Partly blocked — delivery id and version unrecoverable (`INT-084`)~~ · **RETRACTED IN PART, 2026-09-13 — the VERSION limb only.** The struck wording is kept so the overstatement stays visible. Measured: the eligibility read has published the delivery record version in both body and ETag since commit `c5c80b14` (2026-07-30), an ancestor of the disposition's own read-at commit `bd9f7f54`, so **given a delivery id the version IS recoverable**. Evidence: [`a0-preflight.md`](../phase-1/phase-1-31/a0-preflight.md) line 267. The three id-discovery limbs of `INT-084` are **not** re-measured by this correction and nothing here claims they are closed. Corrected under P1-31 **DOC-001** |
