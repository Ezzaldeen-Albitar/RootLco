...
| D    | Duplicate demand and approvals                                | 13    | 10                 | 3                 |
| E    | Changing costs and prices                                     | 19    | 13                 | 6                 |
...
| H    | Reception workflow and camera/OCR                             | 27    | 22                 | 5                 |
...
- **Unverified:** Resolved 2026-09-06: sal.receipt-list has NO received-at window — payments/route.ts:92-101 accepts companyId, branchId, payerPartnerId, status, invoiceId, cursor and limit only.
...
- **Existing evidence:** No alert mechanism. Notifications exist as infrastructure only: purposes transactional/marketing/system (apps/api/src/modules/shared-services/domain/notification-policy.ts:43); the notifications nav entry is 'planned' (navigation.ts:431-439); one business module enqueues a notification (grep of apps/api/src/modules outside shared-services: one — the work-order module raises one: job-assignment-service.ts:41-42 imports notificationService, :529-556 publishes `job.assigned` with a notification block, :567-598 calls prepareNotification with template `job_assigned_notification` (:613), channels in_app+email, dedupeKey job-assigned:<assignmentId>; the consumer is registered at server/worker/consumers/index.ts:15,21; the path is silent because no template is authored on any tenant, :511-512; register P1-29 row 'Notify the assigned employee' Blocked INT-100). CRM customer alerts (register P1-27 row 12) are per-customer flags, not operational alerts.
...
- **Dependencies:** Owner alert list; OWR-2026-09-06-A-3; INT-100; OWR-2026-09-06-A-17 (proposed alert policy). Corrected 2026-09-06: the premise 'no module raises a notification' was false — INT-100 is answered (phase-1-29/canonical-plan.md:55) and the work-order module raises one (job-assignment-service.ts:529-598; worker consumer index.ts:15,21); the register row is refreshed. Prerequisites restated: (i) an alert evaluator/trigger in the reporting module, (ii) a notification template — none ships and no Owner decision names one (job-assignment-service.ts:511-512), (iii) the Owner's alert list. Pushed alerts reuse the existing event → worker → enqueue path with purpose 'system'; no P1-29 remediation lane is a dependency.
...
- **Delivery placement:** P1-31: A0 measurement of the reporting read surface (as P1-30 A0 did), a Backend prerequisite lane for execution/definitions/configure writer, then the reporting UI W-item; the bundle fix belongs to the P1-30 tenant-bootstrap corrective lane (§6.3 window). Reason: register ownership row; P1-30's measured-before-claimed method. Placement corrected 2026-09-06: the P1-31 Backend prerequisite lane, with the /reports gate (register row 'Reports', P1-31). The tenant-bootstrap slice is CLOSED (#321) and recorded rpt.report.read among the four gate codes accepted as residual TB-R3 (tenant-bootstrap-corrective-slice.md:150-155); the bundle is code in bootstrap-roles.ts (CC-04 precedent, no migration); the backfill for organisations provisioned before #321 is the CC-08 Owner decision.
...
- **Delivery placement:** Explicit follow-on after the Integration gate, i.e. beyond Phase 1 — recorded now as two Planned rows so they cannot drift into P1-31. Reason: the Owner's own instruction, plus the data prerequisite. (Label: 'beyond Phase 1' is the researcher's inference from the data prerequisite — a proposed placement, not Owner-stated; the Owner said only 'assign to explicit delivery slices'.)
...
- **Existing evidence:** docs/phase-1/phase-1-30/canonical-plan.md §3 :83-91 (the closure condition; the search sentence at :91 and :131) ('P1-30 RENDERS SERVER ARITHMETIC ONLY', the mechanical gate A0 names); stock-movements/route.ts:40-45 (why companyId/branchId are required); billing-contract.ts:94 (null totals).
...
- **Unverified:** Resolved 2026-09-06: the gate is scripts/ci/check-p1-30-server-arithmetic.mjs (`validate:p1-30-server-arithmetic`, package.json:125, inside verify:policies).
...
- **Existing evidence:** Each candidate maps to a delivered read: wo.* and wo.job_* tables (20260722095000_wo_work_orders.sql:37, 20260722097000_wo_jobs.sql:31), apt.appointment-list, rec.* reads, dia.diagnostic-list, qms.qc-record-list / qms.rework-list, sal.financial_events per event_type (20260724093000_sal_financial_events.sql:45-47 (the event_type CHECK; :48-49 source_type, :50 amount)), sal.receipts.payment_method_id (20260724092000_sal_payments.sql:81), inv.stock_balances (20260723094000_inv_ledger.sql:103-130), inv.stock_movements, inv.stock-reservation-list, tech.labor_sessions.
...
- **Dependencies:** OWR-2026-09-06-A-7; INT-100; a migration for condition storage (rpt tables are ROLLBACK-SAFE while unused, 20260724096000_rpt_reporting.sql:7-8). Corrected 2026-09-06: the premise 'no module raises a notification' was false — INT-100 is answered (phase-1-29/canonical-plan.md:55) and the work-order module raises one (job-assignment-service.ts:529-598; worker consumer index.ts:15,21); the register row is refreshed. Prerequisites restated: (i) an alert evaluator/trigger in the reporting module, (ii) a notification template — none ships and no Owner decision names one (job-assignment-service.ts:511-512), (iii) the Owner's alert list. Pushed alerts reuse the existing event → worker → enqueue path with purpose 'system'; no P1-29 remediation lane is a dependency.
...
- **Existing evidence:** F-01 residual: a0-read-surface-matrix.md:326-337 ('rpt.report.read … undocumented and unrecorded', the 67-code bundle at 23bd9f71 (48 → 65 in #321, → 67 in #322; no `rpt.` code in it) derived by walking one phase's routes); rpt.report.configure is asked for by no operation (end-to-end-workshop-workflow.md:1108-1113); live rpt tables hold zero rows; ADR-008 configuration-driven onboarding; memory: a platform vocabulary is a declared seed, never a migration.
...
- **Delivery placement:** (1) P1-30 tenant-bootstrap corrective lane (§6.3 window, one migration for the bundle + backfill); (2) and (3) P1-31 Backend prerequisite lane. Reason: reachability on a fresh organisation is the acceptance environment every phase closes on. Placement corrected 2026-09-06: the P1-31 Backend prerequisite lane, with the /reports gate (register row 'Reports', P1-31). The tenant-bootstrap slice is CLOSED (#321) and recorded rpt.report.read among the four gate codes accepted as residual TB-R3 (tenant-bootstrap-corrective-slice.md:150-155); the bundle is code in bootstrap-roles.ts (CC-04 precedent, no migration); the backfill for organisations provisioned before #321 is the CC-08 Owner decision.
...
- **Existing evidence:** Schema: inv.stock_locations, branch-scoped, types warehouse|storage|quarantine, parent must be a warehouse in the same scope (supabase/migrations/20260723093000_inv_reference.sql:312-338; hierarchy guard :88-113). Every inv operation table FKs to it (fk_part_issues_location 20260723095000_inv_operations.sql:285, fk_damaged_stock_from_location/_quarantine :408-409, fk_stock_movements_location 20260723094000_inv_ledger.sql:67). Read: inv.stock-location-list GET /stock-locations, inv.stock.read, branch target (apps/api/src/app/api/v1/stock-locations/route.ts:57-64; A0 seam S-16, docs/phase-1/phase-1-30/a0-read-surface-matrix.md:289). Screen: LocationPicker/useLocations in apps/web/src/features/inventory/components/shared.tsx, used by /inventory (W4, PR #317) and /inventory/parts (W5, PR #318). Tests: tests/backend/p1-30-w4-inventory.test.ts (fixtures create locations directly via tests/backend/p1-21-helpers.ts, not through a route).
- **Remaining gap:** No writer exists for inv.stock_locations in apps/api, seeds or migrations (F-02 / S-05, a0-read-surface-matrix.md:104-106; PROC-09 docs/product/workshop/parts-and-procurement-flow.md:691); live catalog: 0 rows across the tenants provisioned before #321 (six per CC-08; seven live on 2026-09-06 with the acceptance tenant). No permission code covers location management. A second, structure-only location model exists (org.warehouses :157 and org.storage_locations :210 in 20260717104000_org_operational_structure.sql) that inv.stock_locations never references (its FKs are to org.branches only) — two unreconciled location models. INT-066 ('no location list') is closed by S-16; the row stays blocked on the writer. Status corrected 2026-09-06: Contracted — the item/category/location writers and inv.uom-list are on #322 (this branch, 23bd9f71: stock-locations/route.ts:121-131, items/route.ts:114-121, item-categories/route.ts:49-72, units-of-measure/route.ts:18-24; inventory-catalog-service.ts; the bundle gains inv.item.manage and inv.adjustment.approve, 65 → 67), true on protected develop once #322 merges; the setup and opening-stock screen is decided as CC-05 (feature/p1-30-w10-inventory-setup).
...
- **Dependencies:** CC-02/CC-03 (F-02 corrected and the writers fixed in #322) (which master-data writers ship and in which lane); decision whether org.warehouses/org.storage_locations are retired or become the source inv.stock_locations mirrors; a permission code for location management (RES-05: mint only with a catalogue change).
...
- **Dependencies:** F-02 (items must be creatable); Owner decision listing which units allow fractions; B-15 writers.
...
- **Dependencies:** F-02 for acceptance on a fresh organisation; B-3 if in-transit becomes a fourth figure.
...
- **Delivery placement:** Named follow-on Backend slice on the P1-21 lane together with transfers (B-6 / policy B-16); not in the P1-30 field-5 scope (item search, stock balance, reservations, issues, returns, stock movements) and not in P1-31. The Integration gate proves it only after the slice lands. Named follow-on: 'stock transfers and transit' (Backend inventory).
- **Dependencies:** Owner decision on the transfer model (one-step vs two-step); B-6 transfers; B-13 accounting boundary when the ends are in different companies; the location writer (F-02).
...
- **Remaining gap:** No goods-receipt source kind, table, function or route; the only documented way for a purchased part to become stock is an opening batch or an approved adjustment, and the adjustment has no route (PROC-10, parts-and-procurement-flow.md:692, :494-497). No supplier master to receive against (register row 'External supplier' Blocked, INT-075 / PROC-05). Receipt cost has nowhere to land except the write-only external-purchase detail (B-19).
- **Owning module:** apps/api/src/modules/inventory; schema inv (new source table + reference_kind + provenance branch)
- **Delivery placement:** Owner decision first (PROC-18: is procurement in Phase 1 scope at all). If not, a minimal 'stock receipt' source or the adjustment surface (policy B-18) on the P1-21 lane as a P1-30 corrective makes 'stock arrived' expressible; a full goods receipt with supplier, order and invoice references belongs to a named procurement follow-on.
- **Dependencies:** PROC-18 / PROC-10 decisions; 'External supplier' row (supplier identity); B-19 cost snapshot; F-02 writers.
...
- **Dependencies:** F-02 (items and locations); an open work order (P1-29 journey); B-18 adjustment surface for corrections.
...
- **Delivery placement:** Named follow-on Backend slice on the P1-21 lane after an Owner decision on the model (policy B-16); outside P1-30 field-5 and P1-31 scope; Integration gate proves it once landed. Named follow-on: 'stock transfers and transit' (Backend inventory).
- **Dependencies:** Owner decision (same-branch only vs cross-branch vs cross-company; one-step vs two-step); B-3 in-transit if two-step; B-13 for cross-company legs; B-19 if cost travels with the stock; F-02 location writer.
...
- **Dependencies:** None for the ledger itself; B-6/B-4 add new reference kinds and provenance branches when they land.
...
- **Delivery placement:** On protected develop; the scale fix is policy B-14, a P1-30 corrective on remediation/p1-30-backend-* (the Frontend lane may not touch apps/api).
- **Dependencies:** Part return; B-14.
- **Acceptance criteria:** Before any return the list reads returnedQty '0.000' (after B-14) and after returns of 1.000 and 0.500 it reads '1.500'; the field is always a string matching ^\d+\.\d{3}$; no key named remaining/outstanding exists on the row.
...
- **Remaining gap:** The return echo has no replayed flag (no body key, unlike reservations). A return recorded in error cannot be reversed (PROC-19 analog; the only path would be an approved adjustment, which has no route — B-18).
...
- **Dependencies:** Part return; B-18 for corrections.
...
- **Remaining gap:** No route OUT of quarantine: inv.stock_adjustments and inv.approve_adjustment exist (:156-212, :756-766) but have no route (PROC-10) — the documented disposal path does not exist. 'scrapped' and 'returned_to_supplier' are labels set once with no movement, no supplier and no operation (PROC-17). No damaged-stock read or screen — the damage write has no P1-30 screen (its adapter mirror is still PENDING per scripts/ci/check-p1-30-payload-parity.mjs:107-115 ('inv.damaged-stock-create': PENDING — no P1-30 screen sends this); no W4 record file exists) and is not in field 5. A returned part has no condition: it always lands sellable at the issue's location, so a damaged return needs a second, separate damage record (policy B-17). 'Usable' is expressed only as 'not in quarantine'.
- **Owning module:** apps/api/src/modules/inventory; schema inv.damaged_stock, inv.stock_adjustments, inv.stock_adjustment_details
- **Delivery placement:** Backend: publish the adjustment/disposition routes over the existing tables on the P1-21 lane as a P1-30 corrective (policy B-18) — the contract the screen needs does not exist, which is why this is Blocked. Screen: a damage/disposition screen needs an Owner scope addition to P1-30 (not in field 5) or lands in the Integration gate's setup set. Placement decided: CC-05 (feature/p1-30-w10-inventory-setup).
- **Dependencies:** Owner decision on who approves disposal (inv.adjustment.approve, maker≠approver by constraint); B-17 return condition; B-20 supplier return; B-19 if scrap must carry a value impact (stock_adjustment_details is inv.cost.view-gated).
...
- **Remaining gap:** inv.stock_movements has no cost column (inv_ledger.sql:42-77) and opening lines are 'quantity only; valuation is out of scope' (20260723095000_inv_operations.sql:132, COMMENT ON TABLE inv.opening_inventory_lines) — there is no per-entry cost layer, no valuation method (no FIFO/average anywhere), and no way to say which cost an issue consumed. No route reads any cost table (PROC-11; INT-070 in the register). 'Lineage' is therefore unrepresentable today, not merely unread. Status note: the register's P1-30 table reads 'Partly blocked' (its own vocabulary); here Blocked = the parts-cost read contract does not exist (INT-070). This row keeps its external-part meaning; cost LINEAGE is B-21 (new, dated), which depends on it.
- **Owning module:** apps/api/src/modules/inventory; schema inv (new restricted movement-cost detail), inv.cost.view (iam)
- **Delivery placement:** Owner decision on valuation method and on who holds inv.cost.view first; the schema is a P1-21-lane slice (policy B-19); the cost read under inv.cost.view (PROC-11, publish-only over existing tables) can be a P1-30 corrective and its rendering is P1-30 W-scope ('inv.cost.view gates cost fields', canonical-plan §4 W4) or P1-31 reporting.
- **Dependencies:** B-4 receipts (cost enters with a receipt); B-6 transfers (cost travels); register row 'Accounting handoff' (Planned); Owner decision on valuation and visibility; never sum across currencies (parts-and-procurement-flow.md §4.4).
...
- **Delivery placement:** Supplier return: P1-21-lane Backend slice after the 'External supplier' decision (INT-075 / PROC-05) — policy B-20. Credit-note reference to a part return: P1-22-lane column + a Backend-lane column first (a schema/contract change under change control, canonical-plan.md:71-74), then a P1-30 corrective screen slice — W6 (#319) is closed. Not deliverable inside P1-30 Frontend scope as it stands. Named follow-on: 'supplier returns' (Backend inventory with billing), tied to the 'External supplier' row.
- **Dependencies:** Register row 'External supplier' (Blocked, no supplier master); B-11 quarantine disposition; sal credit-note contract (W6).
...
- **Existing evidence:** Multi-company structure exists: org.legal_companies (20260717104000_org_operational_structure.sql; org.company-list/update routes under apps/api/src/app/api/v1/org/companies); every inv row carries company_id+branch_id with composite FKs to org.branches and RLS on iam.allowed_company_ids()/allowed_branch_ids() (inv_ledger.sql:84-96 and every ledger and operation table (inv_ledger.sql:42-98, :103-175, :180-250; inv_operations.sql:156-212, :260-350, :382-434) — the four catalogue tables (units_of_measure, item_categories, item_master, item_cost_details; inv_reference.sql:116-305) carry no company/branch columns and tenant-only policies). Today no movement can cross a branch at all (damage refuses it, inventory-stock-service.ts:616-622; no transfer primitive, index.ts:30-33). No general ledger, journal or inter-company posting exists by design (salwtyrpt_schemas.sql:33-37; org.cost_centers comment 'no general-ledger implementation exists'); register row 'Accounting handoff' is Planned.
- **Remaining gap:** The boundary is enforced by impossibility, not by an approved rule: no approval object, no financial event kind for a cross-company move, and no repository document defines 'approved accounting boundary'. The whole capability depends on transfers (B-6) existing first.
...
- **Delivery placement:** Owner business-rule decision first (inter-company invoice vs transfer at cost vs approval role). Then the P1-21 transfer slice (B-6) carries the company-boundary check and the accounting handoff lane adds the event; the Integration gate proves the end-to-end rule. Not P1-30 or P1-31 scope as written. Named follow-on: 'cross-company movement', after the Integration gate.
- **Dependencies:** B-6 transfers; B-3 in-transit if two-step; B-19 cost (a cross-company move at cost needs a cost); 'Accounting handoff' row; Owner definition of the boundary.
...
### OWR-2026-09-06-B-14 — Proposed implementation policy · **Delivered**
...
- **Delivery placement:** P1-30 corrective on remediation/p1-30-backend-* (apps/api is Backend-only; a P1-30 Frontend branch may not change it). Blast radius: one field on two reads, in the harmless direction (scale only, value unchanged). Corrected 2026-09-06: fixed in #322 (CC-07) — the two `COALESCE(...,0)::text` sites in inventory-repository.ts now cast through numeric(12,3), so `returnedQty` and `totalReturned` are '0.000' before any return; the W5 suite pins it. True on protected develop once #322 merges.
- **Dependencies:** None.
...
- **Remaining gap:** Routes, repository INSERT/UPDATE, the allows_fractional column (one migration) enforced by the QuantityString check per item unit, a location permission code, and an administration screen that P1-30 field 5 does not list. Split 2026-09-06: writers → Contracted pending the #322 merge; the allows_fractional column (a real schema change, not in #322) → Undecided (B-22); a location-specific permission code → CC-09 (accepted residual, Owner least-privilege decision).
...
- **Delivery placement:** remediation/p1-30-backend-commercial-setup (the current branch, which carries #322's writers — 33 files against develop, including the four routes, inventory-catalog-service.ts and the two bundle codes) carries the writers (CC-03, fixed in #322 — no Owner decision remains on F-02, CC-02); the screen is decided as CC-05 — feature/p1-30-w10-inventory-setup.
- **Dependencies:** CC-02/CC-03 (F-02 corrected and the writers fixed in #322); RES-05 (minting a permission code is a catalogue change with parity gates); TB-R1 style backfill question for the existing tenants (six pre-#321 per CC-08; seven live on 2026-09-06) does not arise (writers, not bootstrap).
...
- **Delivery placement:** Named follow-on Backend slice on the P1-21 lane after the Owner decision; not P1-30/P1-31. Named follow-on: 'stock transfers and transit' (Backend inventory).
- **Dependencies:** Owner decision on the model; B-13 for cross-company; F-02 locations.
- **Acceptance criteria:** See B-6 and B-3; additionally a partial transfer cannot exist (both legs post in one transaction or neither) and the reconciliation read stays clean.
...
- **Delivery placement:** P1-21-lane corrective (publish-time change, no migration if reason/condition reuse existing columns — UNVERIFIED without a design pass); a P1-30 corrective screen slice (W5, #318, is closed), after the Backend field lands under change control under the same phase if the Owner confirms.
- **Dependencies:** Owner confirmation of the rule; B-11.
...
- **Delivery placement:** P1-30 corrective on the backend lane (publish-only); the screen by Owner scope decision (P1-30 addition or Integration-gate setup). Placement decided: CC-05 (feature/p1-30-w10-inventory-setup).
- **Dependencies:** Owner decision on who holds inv.adjustment.approve; B-19 if value impact must be recorded.
...
- **Existing evidence:** The restricted 1:1 detail pattern already exists three times (item_cost_details, external_purchase_part_details, stock_adjustment_details) with the same RLS shape; money is numeric(18,4) + ISO currency everywhere (tests/db/p1-10-precision.test.ts — which asserts money scale-4 (:43-66) and no real/double (:34-41) only; no quantity-scale assertion exists there, so the migration citations carry the claim alone); the platform forbids client arithmetic (canonical-plan §3).
...
- **Delivery placement:** P1-21-lane Backend slice after the Owner's valuation decision; read and rendering in P1-30 (cost fields under inv.cost.view) or P1-31 reporting. Named follow-on: 'inventory costing' — the same slice Area E names (E-1/E-2/E-4/E-6/E-7/E-10).
- **Dependencies:** Owner decision on valuation method and cost visibility; B-4 receipts; B-6 transfers.
...
- **Delivery placement:** P1-21 lane (operation) after the 'External supplier' decision; P1-16/crm for the supplier role; P1-22 lane for the credit-note reference; rendering in P1-30 W6 if the reference lands. Named follow-on: 'supplier returns' (Backend inventory with billing), tied to the 'External supplier' row.
- **Dependencies:** Register row 'External supplier' (Blocked, INT-075); B-11; B-18.
...
### OWR-2026-09-06-B-21 — Owner requirement · **Blocked**

> Preserve cost lineage

- **Normalised behaviour:** Every on-hand quantity can be traced to the incoming quantities that supplied it and the cost recorded for each, so a valuation as at a date is reproducible from the ledger.
- **Existing evidence:** inv.stock_movements carries no cost and inv.post_stock_movement has no cost parameter (20260723094000_inv_ledger.sql:283-301); the only cost rows are inv.item_cost_details.standard_cost and inv.external_purchase_part_details.unit_cost, both gated by inv.cost.view and neither linked to a movement (Area E, E-1/E-2).
- **Remaining gap:** Unrepresentable today, not merely unread: a movement-level cost and the lineage from receipts to issues (E-2), the valuation method (E-11, Undecided) and the document evidence (E-17).
- **Owning module:** inventory (inv), with the costing follow-on.
- **Delivery placement:** The named follow-on 'inventory costing' (with Area E), gated on the E-9 valuation decision.
- **Dependencies:** Register row 'Expected / approved / final cost' (INT-070); 'Accounting handoff' (Area I); E-2; E-9; E-11.
- **Acceptance criteria:** For an item with two receipts at different unit costs and one issue, the ledger read reproduces which receipt quantities the issue consumed and the valuation of the remainder under the decided method, server-computed.

### OWR-2026-09-06-B-22 — Owner requirement · **Undecided**

> units and fractional quantities where appropriate (the qualifier)

- **Normalised behaviour:** A unit declares whether fractional quantities are allowed; a fractional quantity against a unit that forbids them is refused by the field.
- **Existing evidence:** inv.units_of_measure has no allows_fractional column (20260723093000_inv_reference.sql:116-158); every quantity column is numeric(12,3) and QUANTITY_MIN is '0.001', so 'each' accepts 0.001 today; #322's inventory-catalog-service.ts has no fractional handling.
- **Remaining gap:** The per-unit flag (a schema change), its enforcement on every quantity write, and the units that carry it.
- **Owning module:** inventory (inv.units_of_measure and the quantity guards).
- **Delivery placement:** A P1-30 corrective Backend slice after the Owner names which units are whole-count, or the 'inventory costing' slice if sequenced together.
- **Dependencies:** B-1; the Owner's list of whole-count units.
- **Acceptance criteria:** Once the rule exists, an issue of 0.5 against a whole-count unit is refused with a 422 naming the field, and 0.5 of a fluid unit is accepted.
...
- **Delivery placement:** P1-30 owns the inventory Frontend (register phase table), so the screen is P1-30's; the Backend (migration + operations) cannot travel on a P1-30 Frontend branch (docs/phase-1/phase-1-30/canonical-plan.md §1.4 §1.4 :61-67, §2 :69-75) and must be a named Backend prerequisite on `remediation/p1-30-backend-*` under the `p1-30-backend` profile, merged before the screen. a further slice on the same P1-30 corrective lane (after commercial-setup) (bootstrap-only: payment method, sequences, administrator bundle). NOT P1-31 (delivery/warranty/reporting). The Integration gate only proves it. Because A0 did not name a count seam (a0-read-surface-matrix.md S-07..S-16) and W4/W5 are closed, adopting it into P1-30 versus a named follow-on is the Owner decision in C-15.
- **Dependencies:** F-02 (item and location writers; PROC-08/PROC-09); C-2 (scope), C-3 (counter), C-4 (cutoff), C-7 (adjustment routes, PROC-10); two distinct authorised users in the tenant for maker≠approver; Owner decision C-15 (placement); Owner decision on what "cycle" selects (by location, by item category, by explicit item list — not stated). Baseline (critique 2026-09-06): measured at develop 6f6236c3 — 17 inv operations, no item/location writer, inv.adjustment.approve outside the bundle. #322 (23bd9f71) adds five operations (22) and both bundle codes (65 → 67); the setup and opening-stock screen is owed under CC-05. 'A fresh tenant has nothing to count' is therefore: the API half on #322 (CC-03), the screen under CC-05.
...
- **Unverified:** Resolved 2026-09-06: on develop 6f6236c3 the bundle holds inv.item.read, inv.stock.read and inv.stock.operate (bootstrap-roles.ts:200-202) and NOT inv.adjustment.approve, so no first administrator can approve an opening batch or any adjustment there; #322 adds it (:215). Hard dependency for C-7.
...
- **Dependencies:** C-1; F-02 / PROC-09 (locations must be creatable in-product); Owner decision on whether quarantine locations are inside a full count by default. Baseline (critique 2026-09-06): measured at develop 6f6236c3 — 17 inv operations, no item/location writer, inv.adjustment.approve outside the bundle. #322 (23bd9f71) adds five operations (22) and both bundle codes (65 → 67); the setup and opening-stock screen is owed under CC-05. 'A fresh tenant has nothing to count' is therefore: the API half on #322 (CC-03), the screen under CC-05.
...
- **Existing evidence:** The exact pattern exists for opening batches and is the strongest reusable evidence in this area: `counted_by` is taken from the caller and the body schema is `.strict()` so a supplied `countedBy` is refused (apps/api/src/app/api/v1/opening-inventory-batches/route.ts:28-47; apps/api/src/modules/inventory/application/inventory-intake-service.ts:166-177); the database forbids approver = counter (`ck_opening_inventory_batches_maker`, supabase/migrations/20260723095000_inv_operations.sql:84; trigger `guard_opening_batch_approval` :26-38); the same rule is on adjustments as `requested_by` ≠ `approved_by` (:190, :40-52). Every movement carries `actor_id` and `created_by` from `iam.current_user_id()` (20260723094000_inv_ledger.sql:56 actor_id, :61 created_by; the INSERT at :293-294). Employee identity to render a NAME rather than a UUID: the register records that no technician/employee identity exists (P1-29 rows `INT-045`/`INT-047`) — UNVERIFIED in this pass whether P1-29's W-items closed it.
- **Remaining gap:** A count table carrying `counted_by` per header and per line/recount, bound to the principal exactly as the opening batch does; a name-resolution read so the screen shows a person, not a UUID (register shared-UX rule). (Shrinks to: render the counter through the existing user read.)
...
- **Unverified:** Resolved 2026-09-06: iam.user-detail / iam.user-list publish displayName (user-administration-service.ts:41,70) and tech.technician-list/-detail/-create are routed on develop — the counter's name can be rendered through the existing user read; the register rows 'no technician profile operation exists' (INT-045/INT-047) were stale and are refreshed.

### OWR-2026-09-06-C-4 — Owner requirement · **Blocked**

> a defined count cutoff or movement reconciliation method

- **Normalised behaviour:** Every count fixes, per cell, the ledger position it was counted against (the cutoff), and the variance is computed against the ledger sum at that position; movements posted after the cutoff are handled by the Owner's chosen method — a count cutoff OR a movement-reconciliation method (the Owner's either/or; C-12 carries the recommendation) — and are never silently absorbed into the variance.
- **Existing evidence:** The ledger supplies an exact cutoff anchor: `inv.stock_movements.seq` is `GENERATED ALWAYS AS IDENTITY` and the movement list is ordered by it because `occurred_at` is not a strict order (apps/api/src/app/api/v1/stock-movements/route.ts:8-12; inventory-repository.ts:1713 (the keyset sort `m.seq`) and :1735 (`m.seq::text AS seq`); :1575 is the `ORDER BY m.seq` of findOpeningMovementsForBatch); the list accepts `occurredFrom`/`occurredTo` as full ISO instants (route :52-55; repository :1688-1692) and a `seq` keyset cursor. `occurred_at` is stamped `now()` at posting (20260723094000_inv_ledger.sql:293-294) and no operation can backdate it — it is recording time, not physical time (docs/product/workshop/parts-and-procurement-flow.md:229-238). `inv.stock_balances.record_version` exists (20260723094000_inv_ledger.sql:113) but no read publishes it (grep of inventory-repository.ts: no balance recordVersion). Every balance writer takes `FOR UPDATE` on the cell (`inv.lock_stock_balance` :257-280). No location freeze/lock and no count exist.
...
- **Acceptance criteria:** Expected 7.000, counted 5.500 returns variance "-1.500" as a string from the read; the web feature has no arithmetic over these fields (the server-arithmetic gate passes non-vacuously over the count page); a line with zero variance is distinguishable from an uncounted line. (Illustrative fixture values, not tolerances or count sizes.)
...
- **Dependencies:** C-3 (maker≠approver), C-6 (variance); a second authorised person; Owner decision on who approves and whether any auto-approval threshold exists (recommend none); PROC-11 / `inv.cost.view` if the value impact of a variance is to be recorded (no cost read or writer exists, so `value_impact` cannot be computed today). Bundle fact: on develop no administrator holds inv.adjustment.approve, so even a shipped approve route is unreachable by any first administrator until #322 (bootstrap-roles.ts:215) merges.
...
- **Acceptance criteria:** A named test exists and is green in the hosted DB job: cell on_hand 10; count line opened (cutoff seq N, expected 10); second session issues 3 (seq N+1); counter enters 9; approval posts an adjustment of −1 (9 − 10), not −(9 − 7) = +2; after approval `on_hand` = 6 = Σ signed_qty; the count read lists the seq N+1 movement as post-cutoff. A second variant proves the reverse (return after cutoff). (Illustrative fixture values, not tolerances or count sizes.)
...
- **Dependencies:** Owner decision on a count-only authority; A0 F-01 bundle contents (UNVERIFIED). Baseline (critique 2026-09-06): measured at develop 6f6236c3 — 17 inv operations, no item/location writer, inv.adjustment.approve outside the bundle. #322 (23bd9f71) adds five operations (22) and both bundle codes (65 → 67); the setup and opening-stock screen is owed under CC-05. 'A fresh tenant has nothing to count' is therefore: the API half on #322 (CC-03), the screen under CC-05.
...
- **Delivery placement:** Decision before any code; this line is the gate for all others in Area C. Placement corrected 2026-09-06: P1-30 is the owning phase by the register's boundary (owner-workflow-requirements.md:34) and rule (:38-40); the Backend half is a further slice on remediation/p1-30-backend-* (as #322 carries P1-21-owned inventory work), sequenced after commercial-setup and after inv.adjustment.approve lands in the bundle; a post-Integration-gate follow-on is legitimate only as an explicit Owner DE-SCOPING of P1-30 — not an equal alternative. C-1..C-9 are therefore Blocked with P1-30 as owner.
- **Dependencies:** F-02 must close first either way (nothing to count on a fresh tenant); C-7 can land independently and earlier because it closes PROC-10/PROC-19. Baseline (critique 2026-09-06): measured at develop 6f6236c3 — 17 inv operations, no item/location writer, inv.adjustment.approve outside the bundle. #322 (23bd9f71) adds five operations (22) and both bundle codes (65 → 67); the setup and opening-stock screen is owed under CC-05. 'A fresh tenant has nothing to count' is therefore: the API half on #322 (CC-03), the screen under CC-05.
...
**Where this area stands today.** What exists is transport-level replay safety: every inventory write is `idempotent: true`, the foundation stores the key inside the command transaction and arbitrates concurrent first use by a unique index, and the stock-issue replay is proven not to move stock twice (D-2 Delivered); reservations additionally carry a body-level business key with lifetime uniqueness, a 200/`replayed` answer, and a form-held key in the web. What is missing is everything the Owner calls demand: `inv.part_issues` has no demand key, no job, no reason and no approval reference; no rule compares an issue to `wo.required_parts` or to an accepted quotation line; a required-part line cannot be cancelled; the only inventory approval code (`inv.adjustment.approve`) is routed only for opening-batch approval and, at develop 6f6236c3, outside the administrator bundle (#322 adds it, CC-04); and the web issue form mints a fresh key per press, so only the in-flight `busy` flag stands between a lost answer and a second issue. The concurrency primitives a rule needs — the work-order `FOR UPDATE` on every issue and the balance-cell lock, with a 10-session last-unit race proven — already exist, but there is no rule to enforce and no HTTP-level race on `/stock-issues`. Placement: the web key fix is a P1-30 W5 correction; the demand rule, job attribution, reason, and authorisation are one P1-30 corrective Backend slice on `remediation/p1-30-backend-*` (precedent: the tenant-bootstrap corrective slice) and cannot be scoped until the Owner decides the authoritative demand source and precedence (OD-D-a), the context unit (OD-D-b), and who approves an over-issue and whether a value ceiling applies (OD-D-c); if deferred past P1-30 W9 it must be recorded as an Integration-gate accepted risk or a named follow-on. No existing register row covers this area, so all lines are dated OWR-2026-09-06-D ids; the related rows are P1-30 'Part issue', 'Part consumption', 'Customer approval' and the PROC-10/-14/-19/-21 findings. Measurement head: develop 6f6236c3 (the inventory-repository.ts citations are displaced by +266 lines at 23bd9f71). Fixture quantities in acceptance criteria are illustrative.
...
- **Existing evidence:** Server: key row written INSIDE the command transaction so a key exists iff the command committed (idempotency.ts:24-27, 521-536; tests/backend/idempotency.test.ts:256-303); concurrent first use of one key arbitrated by `uq_idempotency_keys_scope`, loser raises `IdempotencyRaceError`, handler re-reads the winner on a fresh transaction (idempotency.ts:537-547, 568-588; route-handler.ts:429-436; tests/backend/idempotency.test.ts:207-255 'commits exactly one execution and stores exactly one row'). Stock-issue specifically: tests/backend/p1-21-inventory-stock.test.ts:548-576 replays the header key and asserts `on_hand` unchanged and `count(inv.part_issues)=1`. Reservation: one row/one audit/one event on replay (p1-21-inventory-stock.test.ts:225-231) and key+different quantity is 409 (:234-252). Principal binding stops one user replaying another's key (idempotency.test.ts:304-470). Web transport attaches the key on every `idempotent: true` operation (client.ts:306-353). Cross-cutting statement in docs/product/workshop/end-to-end-workshop-workflow.md:1063-1072 (§7.4). Precision (critique 2026-09-06): tests/backend/p1-21-inventory-stock.test.ts:548-576 proves on_hand unchanged and one `inv.part_issues` row on replay, accepting 200 or 201 (:563), and asserts no audit or outbox count for /stock-issues; only the reservation case (:225-231) counts audit rows and events. 'No second audit record and no second outbox event' for an issue is inferred from the foundation returning before `execute` (idempotency.ts:513-517), not tested on that route.
...
- **Delivery placement:** Delivered by P1-13 (foundation) and P1-21 (inventory writes), consumed by P1-30 W5; the screen-side reachability gap is placed with D-1 (P1-30 corrective). (`shared.idempotency_keys` is recorded as promoted by P1-03-DB-022 — 20260717107000_org_provisioning.sql:73; the runtime helper's phase is not cited: UNVERIFIED, immaterial to placement.)
...
### OWR-2026-09-06-D-3 — Owner requirement · **Undecided**
...
- **Delivery placement:** P1-30 corrective Backend slice on `remediation/p1-30-backend-*` (schema + rule; precedent: docs/phase-1/phase-1-30/tenant-bootstrap-corrective-slice.md). Reason: inventory issue is P1-30-owned and the Owner asks for server enforcement, which the Frontend lane may not add (canonical-plan.md §2). If the Owner defers it past P1-30 acceptance it must be recorded as an Integration-gate accepted risk, not left implicit. Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
### OWR-2026-09-06-D-4 — Owner requirement · **Undecided**
...
- **Delivery placement:** Same P1-30 corrective Backend slice as D-3 (the port is a Backend contract; the quotation and inventory Frontends are both P1-30 W3/W5). Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
- **Existing evidence:** Approval primitives exist elsewhere, none for stock issue: (i) `iam.approval_limits` — effective-dated monetary ceilings per role XOR user and `limit_type` (20260718093000_iam_approval_and_sensitive_data.sql:46-89; routes iam/approval-limits/route.ts:54-81, permission `iam.approval.manage`), consumed only by pricing discounts via `DISCOUNT_LIMIT_TYPE='discount'` (apps/api/src/modules/pricing/domain/pricing.ts:45; discount-authorization-service.ts:1-80 with maker≠approver and fail-closed defaults). (ii) `svc.pricing_approval_policies` threshold + required permission (20260723092000_svc_pricing.sql:355-392) — discounts only. (iii) `inv.stock_adjustments` carries `requires_approval`, `reason NOT NULL`, maker≠approver guard and `inv.approve_adjustment` (20260723095000_inv_operations.sql:40-52, 156-192, 756-768) under `inv.adjustment.approve` (high) — but no stock-ADJUSTMENT create/approve operation exists (PROC-10, parts-and-procurement-flow.md:692, is about that surface); the code itself IS routed — `inv.opening-batch-approve` declares it (opening-inventory-batches/[batchId]/approval/route.ts:42) and its maker≠checker approval is already exercised by opening batches (:577 records it as opening-batch approval only). At develop 6f6236c3 the code is absent from the 65-code administrator bundle (48 was the pre-#321 count) (no hit in apps/api/src/modules/iam/domain/bootstrap-roles.ts, which lists `inv.item.read`, `inv.stock.read`, `inv.stock.operate` at :200-202), so under `ins_role_permissions_delegable` no first administrator can grant it. (iv) `wo.additional-work-approval` records the CUSTOMER's decision, one active approval per request, If-Match mandatory (additional-work/[requestId]/approval/route.ts:121-140; 20260722100000:341-342) — a customer-scope commitment, not a stock authorisation. Live local DB: 0 rows in `iam.approval_limits` and 0 in `svc.pricing_approval_policies`. Corrected 2026-09-06: at 23bd9f71 (#322) the code IS in the bundle (bootstrap-roles.ts:215; 65 → 67, CC-04).
- **Remaining gap:** No over-issue authorisation exists: no permission code, no table/columns on `inv.part_issues` (no approved_by, no approval ref), no route, no audit action, no approval-limit `limit_type` for parts. The one inventory approval code (`inv.adjustment.approve`) is routed only for opening-batch approval and, at develop 6f6236c3, unholdable by a bootstrapped tenant; #322 puts it in the bundle (CC-04), so OD-D-c becomes a pure authority question — a dedicated over-issue code versus reuse of the existing high-risk inventory approval code, both holdable after #322 — not a reachability one.
...
- **Delivery placement:** P1-30 corrective Backend slice with D-3, but only after the Owner decision below; the permission code must also enter the administrator bundle in the same slice or it repeats A0 F-01 (docs/phase-1/phase-1-30/tenant-bootstrap-corrective-slice.md §1 row 3). Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
- **Owning module:** api `inventory`; schema `inv.part_issues` (new nullable-until-required `reason`, frozen by `tg_part_issues_immutable`, 20260723095000:297-298 — its column list at :298 omits reservation_id and required_part_ref, so a new reason column must be named there explicitly) or the D-5 authorisation record
- **Delivery placement:** Same P1-30 corrective Backend slice as D-5 (one migration); the list/echo change is consumed by P1-30 W5 PartsScreen. Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
### OWR-2026-09-06-D-7 — Owner requirement · **Undecided**
...
- **Existing evidence:** Visit dimension is representable today: issues are keyed to `work_order_id` (20260723095000_inv_operations.sql:265 (work_order_id), FK fk_part_issues_work_order :283), one ordinary work order per reception origin (end-to-end-workshop-workflow.md:530), and `inv.work-order-part-issue-list` is per parent (part-issues/route.ts:6-18). Job dimension exists on DEMAND only: `wo.required_parts.job_id` FK→`wo.jobs` (live catalog `fk_required_parts_job`; jobs from 20260722097000_wo_jobs.sql (20260901100000 adds department routing only)); the issue can cite the demand line via `requiredPartRef` (stock-issues/route.ts:54) but without an FK (PROC-14).
- **Remaining gap:** `inv.part_issues` has no `job_id`, so an issue is not attributable to a task except through the advisory pointer; any D-3 rule keyed on (work order, item) alone would wrongly refuse a second job's legitimate use. The web IssueForm prefill from a required part carries `requiredPartRef` (PartsScreen.tsx:561, apps/web/tests/inventory-parts.dom.test.tsx:309) but the server does not validate it belongs to the named work order.
...
- **Delivery placement:** Same P1-30 corrective Backend slice as D-3 — it is a constraint on D-3's key, not a separate build. Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
### OWR-2026-09-06-D-8 — Owner requirement · **Undecided**
...
- **Delivery placement:** Same P1-30 corrective Backend slice as D-3; the race test is the slice's acceptance, following the gated two-connection pattern. Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
- **Unverified:** A migration on remediation/p1-30-backend-* has precedent (#321 carried the 137th migration); the open question is the change-control disposition (CC-11), not lane authority.

### OWR-2026-09-06-D-12 — Owner requirement · **Undecided**

> cancellations (sentence 3 — a cancelled or terminal JOB as a demand-cancelling event)

- **Normalised behaviour:** A job that reaches a terminal state (cancelled or otherwise closed) releases or cancels the demand its required-part lines still carry, so a later issue for that job is a repeat, not an open demand.
- **Existing evidence:** `wo.jobs` carries a data-driven state graph (20260722097000_wo_jobs.sql:15,39) and `wo.required_parts.job_id` points at it; a cancelled WORK ORDER is already refused by `requireWorkOrderAcceptingParts` (inventory-stock-service.ts:857-880 at 6f6236c3); no code treats a terminal JOB as a demand-cancelling event.
- **Remaining gap:** The event itself: which job states cancel demand, what happens to an active reservation for that job, and whether the required-part line is closed or left open.
- **Owning module:** work-order (wo.jobs, wo.required_parts) with inventory (reservations).
- **Delivery placement:** With D-3 (CC-11 / the named follow-on 'demand rules and over-issue approval'), after OD-D-a.
- **Dependencies:** OD-D-a; D-3.
- **Acceptance criteria:** Cancelling the job that owns an open required-part line with an active reservation releases the reservation (audited), and a subsequent issue citing that line is refused or flagged as a repeat, per the Owner's decision.

### OWR-2026-09-06-D-13 — Owner requirement · **Undecided**

> a justified repeat (sentence 3, applied to RESERVATIONS)

- **Normalised behaviour:** A repeat RESERVATION for the same work order, item and context is subject to the same approval and recorded reason as a repeat issue (D-5, D-6); D-3's 'issue (or reservation)' scope is stated, not implied.
- **Existing evidence:** `inv.stock-reservation-create` keeps a body `idempotencyKey` for the reservation's whole life (uq_stock_reservations_idempotency, 20260723094000_inv_ledger.sql) and refuses nothing about a second reservation carrying a new key.
- **Remaining gap:** No approval or reason field on `inv.stock_reservations`; no repeat detection.
- **Owning module:** inventory.
- **Delivery placement:** With D-5/D-6 (CC-11 / the named follow-on).
- **Dependencies:** OD-D-c; D-5; D-6.
- **Acceptance criteria:** A second reservation for the same work order, item and location without an approval reference is refused or flagged exactly as a second issue would be, and a justified one records its reason.
...
- **Delivery placement:** P1-30 corrective Backend slice `remediation/p1-30-backend-duplicate-demand` (name illustrative), merged before P1-30 W9 acceptance; otherwise a named follow-on recorded at the Integration gate. Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
- **Remaining gap:** Unbuilt; OD-D-c precedes it. `inv.adjustment.approve` is routed only for opening-batch approval and, at develop 6f6236c3, absent from the bundle; #322 adds it (bootstrap-roles.ts:215, CC-04), so reuse no longer inherits the F-01 blind spot — the remaining question is authority (a dedicated over-issue code or the existing high-risk code), OD-D-c.
- **Owning module:** api `inventory` + `iam` seed + platform bootstrap bundle; schema new `inv` table, `iam.permissions`
- **Delivery placement:** Same slice as D-10; the bundle change needs the TB-R1 backfill decision for organisations created before the slice. Placement corrected 2026-09-06: this is new Backend capability (a table or columns, a permission code, a migration, three open Owner decisions), not a fresh-tenant defect, so the remediation lane needs its own change-control row — CC-11 ('demand rules and over-issue approval'; Backend inventory P1-21 with work-order P1-19; pending OD-D-a/b/c). If the decisions are not taken before P1-30 W9, it is the named follow-on 'demand rules and over-issue approval' after the P1-30 gate.
...
- **Delivery placement:** Blocked → named follow-on Backend slice 'inventory costing' after the E-9 decision (Backend lane; before the Integration gate only if the Owner elects valuation into Phase 1). Reason: needs new migrations (receipt movement/cost lot) which a P1-30 Frontend branch cannot carry (docs/phase-1/phase-1-30/canonical-plan.md §2 (:71-73) and §5 item 5 (:123)) and which A0 named as no Backend prerequisite. The holdability half (E-14) fits the P1-30 §6.3 corrective window as a bootstrap correction. Placement corrected 2026-09-06: a P1-30 corrective Backend slice on remediation/p1-30-backend-inventory-costing — the named slice 'inventory costing', the same name Area B's B-19 uses — gated on the E-9 decision; if the decision is not taken before P1-30 closes, an EXPLICIT follow-on commitment under that name (the register's follow-on section). Precedent for a migration on this lane: #321.
- **Dependencies:** E-8/E-9 (valuation method decision), E-13 (receipt primitive; Owner decisions PROC-03 arrival vocabulary and PROC-18 procurement boundary), E-14 (who may hold inv.cost.view — PROC-11 'Owner confirms who may see cost'), E-15 (currency rule; OIR-04 open), F-02 (no item/location writer — a fresh tenant has no item to receive). Register row 'Expected / approved / final cost' (P1-30, INT-070 — parts cost readable by nothing): this line's cost-lineage content is that row's refresh; the dated id carries only what the row does not.
...
- **Delivery placement:** Blocked → follow-on Backend slice 'inventory costing' (same slice as E-1). Reason: pure schema/function change behind the P1-30 Frontend boundary; cannot exist before the method decision. Placement corrected 2026-09-06: a P1-30 corrective Backend slice on remediation/p1-30-backend-inventory-costing — the named slice 'inventory costing', the same name Area B's B-19 uses — gated on the E-9 decision; if the decision is not taken before P1-30 closes, an EXPLICIT follow-on commitment under that name (the register's follow-on section). Precedent for a migration on this lane: #321.
- **Dependencies:** E-9 (method), E-12 (storage policy), E-1 (there must be a cost to assign), E-14. Register row 'Expected / approved / final cost' (P1-30, INT-070 — parts cost readable by nothing): this line's cost-lineage content is that row's refresh; the dated id carries only what the row does not.
...
- **Delivery placement:** Blocked → follow-on Backend slice 'inventory costing'; any valuation REPORT is P1-31 (Reporting) scope and consumes it. Reason: no method exists and the Owner asked for a proposal before any valuation-dependent posting (E-9). Placement corrected 2026-09-06: a P1-30 corrective Backend slice on remediation/p1-30-backend-inventory-costing — the named slice 'inventory costing', the same name Area B's B-19 uses — gated on the E-9 decision; if the decision is not taken before P1-30 closes, an EXPLICIT follow-on commitment under that name (the register's follow-on section). Precedent for a migration on this lane: #321.
...
- **Delivery placement:** Blocked → follow-on Backend slice 'inventory costing'. Reason: depends on E-9's method and E-2's storage; a Frontend branch cannot add the guards. Placement corrected 2026-09-06: a P1-30 corrective Backend slice on remediation/p1-30-backend-inventory-costing — the named slice 'inventory costing', the same name Area B's B-19 uses — gated on the E-9 decision; if the decision is not taken before P1-30 closes, an EXPLICIT follow-on commitment under that name (the register's follow-on section). Precedent for a migration on this lane: #321.
...
- **Unverified:** Resolved 2026-09-06: an UPDATE policy exists — upd_external_purchase_part_details_gated (20260723095000_inv_operations.sql:583-585) with GRANT UPDATE (:586) — so unit_cost is mutable in place by any inv.cost.view holder (the route docblock says 'SELECT, INSERT and UPDATE', external-purchase-parts/route.ts:23). The gap is stronger than stated: the one recorded cost is overwritable today.
...
- **Delivery placement:** Blocked → follow-on Backend slice 'inventory costing'. Reason: cost lineage is impossible until issues carry cost. Placement corrected 2026-09-06: a P1-30 corrective Backend slice on remediation/p1-30-backend-inventory-costing — the named slice 'inventory costing', the same name Area B's B-19 uses — gated on the E-9 decision; if the decision is not taken before P1-30 closes, an EXPLICIT follow-on commitment under that name (the register's follow-on section). Precedent for a migration on this lane: #321.
- **Dependencies:** E-2, E-9, E-12; existing register row 'Part return' (P1-30). Register row 'Expected / approved / final cost' (P1-30, INT-070 — parts cost readable by nothing): this line's cost-lineage content is that row's refresh; the dated id carries only what the row does not.
...
- **Delivery placement:** P1-30 corrective (documentation only, no code). Reason: it is an inspection whose output is this register entry; it lands when this entry merges. (The inspection itself was performed 2026-09-06 and is this line; the register vocabulary has no 'documented' status — Planned names the decision-record task that follows.)
- **Dependencies:** None.
...
- **Owning module:** Owner decision register (a new P1-OD number is the register owner's to assign — docs/phase-1/phase-1-27/open-decisions.md:1167 (P1-OD-042 the highest treated as existing; P1-OD-043 is referenced at :58 and :368) records that P1-OD-042 is the highest treated as existing and P1-OD-043 is already referenced); then apps/api/src/modules/inventory and schema inv
...
- **Delivery placement:** Blocked → written in the follow-on Backend slice 'inventory costing' and repeated as an Integration-gate scenario (the gate proves the end-to-end journey). Reason: it is the acceptance proof of E-1/E-2/E-7 and cannot precede them. Placement corrected 2026-09-06: a P1-30 corrective Backend slice on remediation/p1-30-backend-inventory-costing — the named slice 'inventory costing', the same name Area B's B-19 uses — gated on the E-9 decision; if the decision is not taken before P1-30 closes, an EXPLICIT follow-on commitment under that name (the register's follow-on section). Precedent for a migration on this lane: #321.
...
- **Existing evidence:** The code is seeded (supabase/seeds/04_iam_permission_catalog.sql:48, risk high) and enforced by RLS, but is absent from the bootstrap bundle (apps/api/src/modules/iam/domain/bootstrap-roles.ts:188-220 (19 codes at 23bd9f71: the 17 screen-derived ones plus inv.item.manage :206 and inv.adjustment.approve :215) lists 17 commercial codes without it) — deliberately, per docs/phase-1/phase-1-30/a0-read-surface-matrix.md:265-267 (:258-263 lists the 24 INCLUDED commercial codes); ins_role_permissions_delegable admits a mapping only when the acting administrator already holds the code (supabase/migrations/20260726090000_iam_org_runtime_administration_capabilities.sql:299-312); the bundle is written once at provisioning and earlier organisations keep their set (bootstrap-roles.ts:109-111). Consequence proven by the existing suite shape: without the code the external-purchase cost write is refused (tests/backend/p1-21-inventory-intake.test.ts:624-641) — in a provisioned tenant that is every principal. PROC-11 already asks 'Owner confirms who may see cost' (docs/product/workshop/parts-and-procurement-flow.md:693).
...
- **Delivery placement:** Undecided → P1-30 corrective (§6.3 window) once the Owner names the holder. Reason: it is a bootstrap correction of the F-01 kind, needs no migration, and unblocks the one cost write that already exists. Mechanism precision: a bundle change is a bootstrap-roles.ts code change (CC-04 precedent, no migration) plus the CC-08 backfill decision for organisations provisioned before it — not a §6.3 window write.
- **Dependencies:** Owner decision (PROC-11); backfill decision for pre-existing organisations.
- **Acceptance criteria:** On a freshly provisioned tenant an administrator can create a role holding inv.cost.view and grant it; that principal records an external-purchase unit cost (201, costRecorded true); a principal without the code still receives 403 with full rollback; the navigation/permission-parity gates stay green. Both codes are needed: `inv.cost.view` AND `inv.external_purchase.record` (CC-12).
...
### OWR-2026-09-06-E-17 — Owner requirement · **Blocked**

> Retain each incoming quantity's acquisition-cost EVIDENCE (the document half)

- **Normalised behaviour:** Each recorded acquisition cost keeps a reference to the document that proves it (supplier invoice, receipt) as a linked, tenant-scoped document version, not a bare identifier.
- **Existing evidence:** PROC-06 'no supplier invoice or external reference' and PROC-07 'evidence_ref is not a document link' (parts-and-procurement-flow.md:688-689); inv.external_purchase_parts.evidence_ref is a bare nullable uuid with no FK (20260723095000_inv_operations.sql:506) and the route accepts evidenceRef as an optional uuid (external-purchase-parts/route.ts:77).
- **Remaining gap:** A document link (FK to shared.document_versions, or the Area B document-link line) on the cost-bearing rows; E-1 normalises 'evidence' to a unit cost and currency only.
- **Owning module:** inventory (inv.external_purchase_parts and the costing rows) with shared-services documents.
- **Delivery placement:** With E-1 on the 'inventory costing' slice.
- **Dependencies:** E-1; the shared document store (P1-15); PROC-07.
- **Acceptance criteria:** Recording an acquisition cost with an evidence document stores a link that resolves to a document version the same tenant can read; a dangling reference is refused by the field.

### OWR-2026-09-06-E-18 — Owner requirement · **Undecided**

> make a concrete technical proposal FOR THE AUTHORIZED DECISION MAKER

- **Normalised behaviour:** The valuation-method proposal (E-11) is addressed to a named decision maker and recorded as their decision.
- **Existing evidence:** The register's vocabulary makes Undecided 'a Product Owner commercial or business-rule decision' (owner-workflow-requirements.md:21); DEP-06 assigns 'cost values' to the Inventory Owner (phase-1-10-open-decisions.md:23); P1-11 defers accounting to a future integration (phase-1-11-no-general-ledger-boundary.md:10-12).
- **Remaining gap:** The decision maker is not named anywhere: the Product Owner (register rule) and the Inventory Owner (DEP-06) are both candidates, and no accounting authority exists.
- **Owning module:** product decision register.
- **Delivery placement:** With E-9 (the Owner decision register entry names the addressee).
- **Dependencies:** E-9; E-11.
- **Acceptance criteria:** The recorded decision names who decided and cites E-11 as the proposal.

### OWR-2026-09-06-E-19 — Owner requirement · **Blocked**

> sale-price revisions (kept distinct) — for PARTS

- **Normalised behaviour:** A part's selling price can be revised as a new version without touching the acquisition cost, and every document snapshot names the version it priced from — the services-chain rule (E-3) applied to parts.
- **Existing evidence:** None for parts: svc.price_rules.service_id is NOT NULL (20260723092000_svc_pricing.sql:199) and every quotation line is itemKind 'service' (quotation-service.ts:1156); E-16 carries the mechanism as a proposed policy.
- **Remaining gap:** A part-priced rule or price-list line, its version chain, and the snapshot fields on quotation/invoice lines for parts.
- **Owning module:** pricing (svc) with inventory (item identity).
- **Delivery placement:** The 'inventory costing' slice or a named follow-on 'parts pricing' — an Owner scope decision.
- **Dependencies:** E-3; E-16; B-21.
- **Acceptance criteria:** Revising a part's selling price creates a new version; an issued document keeps the version it was priced from; the acquisition cost is unchanged.
...
- **Delivery placement:** Named follow-on: 'Vehicle knowledge — fluid specification', under the register's Cross-phase vehicle-catalogue row. Not P1-30 corrective (that slice is the tenant-bootstrap repair and is closed; no P1-30 lane owns it: oil specification is not among canonical-plan §1.1's scope items (§2 :75-77 binds only a P1-30 Frontend branch; §1.4 :61-66 provides remediation/p1-30-backend-* for Backend prerequisites — the lane #322 uses). Backend lane first (schema + vehicle module + port), Frontend consumption after.
...
- **Delivery placement:** Same follow-on as F-1, Backend lane; the engine-variant read is the first deliverable and is independent of the provider decision (it publishes an existing table). The `veh.engine_history` read is assigned to P1-17 Backend by VCAT-09 (catalogue-architecture.md:980) and VHM-20 (vehicle-history-model.md:1002): it is a P1-17 remediation the follow-on consumes, not part of the follow-on.
...
### Cross-phase — the vehicle catalogue (Manual fallback is mandatory) — Owner requirement · **Blocked**
...
- **Remaining gap:** A source marking (manual vs catalogue vs provider) on the specification carried by the line, with entered-by/entered-at readable; a 'no coverage' state that opens the manual path rather than a blank; the five markings of manual-entry-policy §5 (:376-392) applied to fluid entries. Split (critique 2026-09-06): the VEHICLE-level source-of-entry marking is MVE-01 — P1-07 (column) / P1-17 (field, projection) Backend with P1-27 FE-018/FE-019 as consumer, following the tech.labor_sessions.source precedent (manual-entry-policy.md:495; §5.2 :424-447) — and does not exist, so this line is Blocked (a named contract does not exist), with generic quantity entry already Contracted/Delivered. Only the LINE-level marking on wo.required_parts / quo.quotation_items is genuinely new and belongs to the fluid-specification follow-on.
...
- **Dependencies:** OWR-2026-09-06-F-1; OWR-2026-09-06-F-2. Area I: AI-5 (model outputs are untrusted candidate data) and AI-9 (a confidence score is not accuracy) govern the 'model-generated guess' clause.
...
- **Existing evidence:** None on screen. No provenance columns exist (VCAT-07 catalogue-architecture.md:978); the on-screen provenance requirement is specified as planned at §3.5 (:480-495); the services web contract exposes only id/serviceCode/name/description/categoryId/lifecycleStatus/recordVersion (apps/web/src/features/services/services-contract.ts:112-118); vehicle detail exposes catalogue names only (apps/web/src/features/vehicles/profile-contract.ts:64-68 — makeName/modelName/trimName, rendered by components/VehicleProfileScreen.tsx:453-455; contract.ts:77 has search publishing catalogue ids and no name).
...
- **Dependencies:** Cross-phase vehicle-catalogue provider clause (provenance columns); OWR-2026-09-06-F-1. Parent clause: the register's vehicle-catalogue provider rule ('source-attributed', the Cross-phase vehicle-catalogue row of owner-workflow-requirements.md); this line is its on-screen requirement.
...
- **Delivery placement:** Undecided until the Owner names the policy; then Backend work in the same follow-on, with the customer-approval half riding the existing P1-30 'Additional-work approval' and 'Customer approval' rows and its journey proof at the Integration gate. Placement corrected 2026-09-06: with D-5 (CC-11 / the named follow-on 'demand rules and over-issue approval'), not a separate follow-on.
- **Dependencies:** Owner business-rule decision on which approval applies and whether a tolerance exists (no numeric tolerance may be invented); INT-015 (party-role id for additional-work approval); OWR-2026-09-06-F-1 (the proposed quantity as baseline). OWR-2026-09-06-D-5, D-6 and policy D-11 (OD-D-c) own the over-issue approval mechanism; extra oil consumption is an over-issue of a fluid line and inherits D-5's placement. The consumption event itself is the register's P1-30 rows 'Part issue' and 'Part consumption', which the overrun comparison attaches to.
...
- **Existing evidence:** supabase/seeds/07_inv_units_of_measure.sql:20-21; wo.required_parts.unit text; quo.quotation_items has no unit column (live catalog); inv.item_master.uom_id references units_of_measure (20260723093000_inv_reference.sql). No units-of-measure list read exists on develop 6f6236c3 (`inv.item-search` joins the unit at inventory-repository.ts:405,422); #322 adds `inv.uom-list` (units-of-measure/route.ts:19, inv.item.read, tenant scope) — any unit picker depends on that merge. Overlaps OWR-2026-09-06-B-1 (units and fractional quantities) and the parts rule that numeric(12,3) is where fluids are counted (parts-and-procurement-flow.md:249-250): this line is the fluid-specific case of B-1.
...
- **Dependencies:** OWR-2026-09-06-F-2. OWR-2026-09-06-B-1; `inv.uom-list` (#322).
...
- **Delivery placement:** After the F-6 Owner decision, same follow-on. Placement corrected 2026-09-06: with D-5 (CC-11 / the named follow-on 'demand rules and over-issue approval'), not a separate follow-on.
- **Dependencies:** OWR-2026-09-06-F-6; INT-015. OWR-2026-09-06-D-5, D-6 and policy D-11 (OD-D-c) own the over-issue approval mechanism; extra oil consumption is an over-issue of a fluid line and inherits D-5's placement. The consumption event itself is the register's P1-30 rows 'Part issue' and 'Part consumption', which the overrun comparison attaches to.
...
- **Existing evidence:** No platform-owner page exists in apps/web: apps/web/src/config/navigation.ts:451-620 (Administration group) lists only tenant screens; apps/web/src/features/ and apps/web/src/app/[locale]/(dashboard)/ hold no platform feature or route (directory listing 2026-09-06); the only web mention of the three platform operations is the operation-registry mirror apps/web/src/lib/api/idempotent-operations.ts:1413-1433 on develop (:1434-1454 at 23bd9f71), which is metadata, not a screen. The web session carries exactly one tenantId (apps/web/src/features/authentication/types/session.ts:22) and no platform authority. Backend contracts a console could consume: platform.organization-read / -provision / -lifecycle (apps/api/src/app/api/v1/platform/organizations/route.ts:160-189; .../[tenantId]/status/route.ts:62-74), proven by tests/backend/pre-p1-29-platform-control-plane.test.ts:325-851.
- **Remaining gap:** Whole UI; a web session/authorization branch that recognises platform authority (apps/api/src/server/auth/authorization.ts routes platform.* codes to iam.has_platform_authority for API calls, but the web navigation/permission model has no platform branch); operations for every capability beyond the organization trio. Status corrected 2026-09-06: the organization list is Contracted (three platform.* operations); the console as 'every Area G capability' has no contract — Planned as a whole.
...
- **Existing evidence:** org.tenants (supabase/migrations/20260717101000_org_tenants.sql:87-117, status provisioning|active|suspended|closed) with append-only org.tenant_status_history (:135-163) and graph guard org.change_tenant_status (:172-221). Control plane: GET/POST /platform/organizations and POST /platform/organizations/{tenantId}/status (route files above); repository reads the tenant root only (apps/api/src/modules/platform/data/platform-repository.ts:64-96) under sel_tenants_platform (20260831093000_iam_platform_privilege_graph.sql:90-97). Live stack 2026-09-06: 7 tenants on 2026-09-06, all active, one being platform_operators (psql).
...
- **Delivery placement:** P1-30 corrective for the usability residuals that block the P1-30/P1-31 fresh-organization acceptance (F-02 writers, TB-R1..R3 decisions); the onboarding UI itself belongs to the Platform Owner Console follow-on. Placement corrected 2026-09-06: the P1-21 inventory Backend lane inside the P1-30 corrective slice (#322); 'eleven writers' withdrawn.
- **Dependencies:** F-02 Owner decision (which Backend lanes P1-20/P1-21/P1-22 write the the master-data tables A0 counted as eleven — corrected 2026-09-06 (f02-remeasurement.md): writers existed on develop for the svc tables (A1 #311: svc.service-category-create, svc.service-version-create, svc.price-list-assignment-create), five tables are valid-but-unconfigured, and the true deficit was three inventory writers plus two bundle codes, delivered in #322 (23bd9f71) with the screen owed under CC-05.
...
- **Remaining gap:** No org.company-create or org.branch-create operation exists anywhere (only GET on org/companies/route.ts:49 and org/branches/route.ts:37; POST routes absent) — a tenant can never grow beyond its first company/branch through the product; org.company.manage/org.branch.manage are seeded and inert for creation (scope.md:49 — stale: org.company.manage gates org.company-update (org/companies/[companyId]/route.ts:61) and org.company-status-set (…/status/route.ts:59), org.branch.manage gates org.branch-update (org/branches/[branchId]/route.ts:67); no CREATE operation exists (org/companies/route.ts:52 and org/branches/route.ts:40 are GET-only); and the administrator bundle deliberately EXCLUDES both codes (bootstrap-roles.ts:62-63), so a fresh First Owner cannot update a company or branch until it maps them itself (CC-13) G-4); no platform-side cross-tenant read of companies/branches after provisioning; no UI on the platform side.
- **Owning module:** api `iam` organization-administration (tenant side) and `platform` (cross-tenant read); schema org
- **Delivery placement:** Company/branch CREATE is a Backend contract gap for the tenant Administration screens (owner: the org/IAM Backend lane, a NAMED follow-on 'tenant structure administration' (Administration, P1-26, is closed; pre-P1-29 GAP-16 recorded the gap but is not a register placement)) and blocks any multi-branch acceptance; the platform-side view belongs to the Platform Owner Console follow-on.
...
### OWR-2026-09-06-G-11 — Owner requirement · **Undecided**
...
### OWR-2026-09-06-G-12 — Owner requirement · **Undecided**
...
- **Remaining gap:** After the bootstrap window closes (first legal transition, upd_tenants_platform_lifecycle :101-115; L8 :445) the platform has NO write path into a tenant's IAM: it cannot replace a lost First Owner, re-invite an administrator, or widen the bundle of a pre-slice organization (TB-R1). Only one platform operator can ever exist through the sanctioned genesis (G2, platform-genesis test :268) and platform grants have no in-product writer (20260831090000_iam_platform_authority.sql:121 (the COMMENT ON TABLE iam.platform_grants; :106-118 are the constraints, ck_platform_grants_no_self_grant :116)) — there are no 'delegated platform administrators' either.
- **Owning module:** api `platform` + `iam` (tenant-bootstrap-service.ts); schema iam (platform_grants, roles, role_grants)
- **Delivery placement:** Backend contract on the Backend lane before the Platform Owner Console; the TB-R1 backfill decision is P1-30 corrective territory because it gates the fresh-organization acceptance of already-provisioned pilots. Register placement: the named follow-on 'Platform Owner Console' (Backend lane first).
...
### OWR-2026-09-06-G-15 — Owner requirement · **Contracted**
...
- **Remaining gap:** Commercial usability is Blocked by F-02 — the master-data tables A0 counted as eleven — corrected 2026-09-06 (f02-remeasurement.md): writers existed on develop for the svc tables (A1 #311: svc.service-category-create, svc.service-version-create, svc.price-list-assignment-create), five tables are valid-but-unconfigured, and the true deficit was three inventory writers plus two bundle codes, delivered in #322 (23bd9f71) with the screen owed under CC-05.service_categories, svc.service_versions, svc.price_list_assignments, svc.discount_rules, svc.pricing_approval_policies, inv.item_categories, inv.item_master, inv.stock_locations, sal.invoice_numbering_configs, org.tax_classes, org.tax_rates; tenant-bootstrap-corrective-slice.md §4; a0-read-surface-matrix.md:87); eight navigation gates unheld (TB-R3); prefix/pad/reset rules unconfigurable (TB-R5). Status corrected 2026-09-06: Contracted pending the #322 merge and the CC-05 screen; the eleven-table blocker is withdrawn.
...
- **Delivery placement:** Backend lane after the Owner decisions; enforcement is tenant-runtime code so it lands before any console UI. Register placement: the named follow-on 'Platform Owner Console' (Backend lane first).
...
- **Remaining gap:** Platform-side delegation (one operator granting another a platform.* code) has no in-product path (20260831090000_iam_platform_authority.sql:121 (the COMMENT ON TABLE iam.platform_grants; :106-118 are the constraints, ck_platform_grants_no_self_grant :116) 'out-of-band operator act').
...
- **Delivery placement:** With G-16 on the Backend lane. Register placement: the named follow-on 'Platform Owner Console' (Backend lane first).
...
- **Existing evidence:** Delivered: DelegationPolicy unrestricted-delegation refusal and hierarchical scopeCovers (delegation-policy.ts:73-100,143+), normalizeScopes de-duplication (access-administration-service.ts:68-90), scope containment at :394-410, deferred constraint backstop (architecture-decisions.md:227 names 20260727090000_iam_grant_delegation_scope_backstop.sql), composite FKs on iam.grant_scopes (20260718092000_iam_role_grants_and_scopes.sql:132-138 (fk_grant_scopes_grant/company/branch/department; 20260726090000:397,401 are the ins/del policies)); tests iam-access-administration.test.ts cases 4,5,9,10/11,12,13 (:237-303) and scope add/remove (:443-444).
...
- **Existing evidence:** Delivered for the three control-plane operations: actor is iam.current_user_id(), never the request (route docblocks; P5 refuses actor_id; A1 asserts actor_id = operator, control-plane test :810-835); provisioning's audit record is written INSIDE the target tenant's trail with the target as entity_id (organization-service.ts:166-206; A1 asserts tenant_id = new tenant); org.tenant_status_history rows carry the server-stamped actor in the target tenant (20260831093000:119-136; L6/L7 :421); the genesis writes platform.operator.genesis (G4 :390). Precision: changeStatus's appendAudit(db, …) is at organization-service.ts:294 and provision's appendAudit(target, …) at :166; activation at provisioning calls repository.changeStatus directly (:208-213), so no org.tenant.status_changed record is written for that transition in either tenant (the org.tenant.provisioned record carries the details), and A2 (pre-p1-29-platform-control-plane.test.ts:850-862) counts records without asserting the holding tenant.
- **Remaining gap:** The lifecycle audit record (org.tenant.status_changed) is appended with db.context.principal.tenantId — the operator's HOME tenant (audit.ts:105) — so the target organization's own audit trail carries only the status-history row, not the audit record naming the reason; no operation lets a tenant read its status history (no SELECT for app_platform either, :137-139; UNVERIFIED whether a tenant-side read of org.tenant_status_history is published — grep of route ids shows none); nothing exists yet for support access (G-22).
...
- **Acceptance criteria:** A platform operator can page the audit trail of organization X and of the operator home tenant without a tenant session; iam.audit_verify_chain(p_tenant uuid) (20260718095000_iam_audit_subsystem.sql:312) passes on both after platform acts; every support-access session start and end is an audit record in X.
...
- **Existing evidence:** Precedents: platform-only tables (20260717102000_org_subscriptions.sql:89 (the org.feature_flags COMMENT; :95-100 is tg_feature_flags_immutable); 20260831090000:121 (the same COMMENT)); the platform module prefix and its authority branch (platform/organizations/route.ts:1-21); the ownership-profile mechanism used for p1-30-backend / p1-30-frontend (canonical-plan.md §1.4).
...
- **Delivery placement:** Backend lane, small slice, before the Platform Owner Console; it is the runtime half of G-6/G-8. Register placement: the named follow-on 'Platform Owner Console' (Backend lane first).
...
- **Existing evidence:** Provisioning already writes in the target trail (organization-service.ts:166-206, A1), lifecycle writes only in the operator's home tenant (changeStatus :286-299 via audit.ts:105) — the two operations disagree today; the target tenant's audit policies for app_platform are tenant-matching (20260831093000:396-416) so a dual write needs a retarget, which withPlatformTarget refuses outside the provisioning window.
...
- **Delivery placement:** Small Backend slice on the platform module ahead of the console; the pattern G-27 depends on. Register placement: the named follow-on 'Platform Owner Console' (Backend lane first).
...
### P1-28 #16 "Conditional road test" / #19 "Unsafe-to-road-test outcome" — Owner requirement · **Undecided**
...
- **Delivery placement:** Backend prerequisite on the P1-18/P1-19 lane, then a P1-28 corrective slice (reception-time) — the register owns it under P1-28 rows 16–21; it cannot be placed in P1-30 or P1-31. Sequence before the Integration gate because Step 8 is on the journey. Placement corrected 2026-09-06: Backend on the rec (P1-18) or dia (P1-19) lane per the Owner's placement decision — end-to-end-workshop-workflow.md:462 ('P1-28 if placed at reception; P1-29 if placed on a job'); INS-04/INS-05 (inspection-and-diagnostics.md:831-832) give the schema to P1-09 and the application to P1-19 — then the matching P1-28 or P1-29 corrective slice, before the Integration gate.
...
### Cross-phase "Vehicle history — all visits, plates, odometer readings, reception evidence, diagnostics…" (the historical-record read) — Owner requirement · **Blocked**

> The historical record includes seven exterior photos, a dashboard image, first-visit VIN evidence, odometer/fuel/SOC/warning lights, the road-test outcome…

- **Normalised behaviour:** The captures the reception rows deliver per VISIT are readable per VEHICLE across visits — one read that returns a vehicle's reception evidence, readings and road-test outcomes in date order.
- **Existing evidence:** veh.vehicle-history reads exactly one table, veh.vehicle_attribute_history (vehicle-history-model.md:39-50); WF-19: 'veh.vehicle-history is attribute changes only … no operation returns a vehicle's service history across receptions, work orders, invoices and deliveries' (end-to-end-workshop-workflow.md:967, 1010); the only evidence read is per visit — rec.reception-evidence-binding-list (receptions/[receptionId]/evidence-bindings/route.ts:59-70).
- **Remaining gap:** The per-vehicle read across visits; the WF-19 Owner decision 'whether a unified vehicle service history is a deliverable' (end-to-end-workshop-workflow.md:1227).
- **Owning module:** vehicle (veh) reading reception (rec) evidence bindings and readings.
- **Delivery placement:** The existing cross-phase register row 'Vehicle history — all visits, plates, odometer readings, reception evidence, diagnostics…' — owner the Integration gate (register line 36); no new id.
- **Dependencies:** WF-19 decision; the P1-28 capture rows above; P1-27 #23 odometer history.
- **Acceptance criteria:** For a vehicle with two receptions, one read returns both visits' evidence bindings, readings and road-test outcomes in date order, tenant-scoped, with no client assembly.
...
- **Existing evidence:** Operations present on develop: rec.reception-convert-to-work-order; wo.* (38 ops incl. wo.additional-work-approval, wo.job-work-log-record, wo.job-evidence-record, wo.work-order-closure-eligibility, wo.work-order-timeline); quo.* 10; inv.* 17; qms.* 15 (qc-record-open/finalize, rework-create/sign-off, reopen-attempt); sal.* 20 (invoice, payment); route tree apps/api/src/app/api/v1/work-orders/[workOrderId]/{additional-work,closure,closure-eligibility,evidence,invoice,invoice-preview,jobs,part-issues,quality-controls,quotations,required-parts,rework,service-lines,timeline,transition}; web features: work-orders, diagnostics, technicians, quality, quotations, inventory, billing, payments (apps/web/src/features/*). P1-29 closed 2026-09-03 (docs/phase-1/phase-1-29/closure-record.md); P1-30 W1–W7 merged and protected-verified (PRs #314–#320; the rows refreshed in owner-workflow-requirements.md), W8/W9 not yet taken.
- **Remaining gap:** The end-to-end journey on a fresh tenant is F-02 was re-measured on 2026-09-06 (tenant-bootstrap-corrective-slice.md:126-146; f02-remeasurement.md): the service-catalogue, pricing, quotation, payment and invoice writers exist with screens (A1 #311, W1–W7); the residual was three inventory tables, written by five operations in #322 (operations 368 → 373; inv.* 17 → 22 on that head) with the setup and opening-stock screen owed under CC-05; tax classes/rates, numbering mode, discount rules and approval policies are valid-but-unconfigured register gaps (CC-06), not acceptance blockers.md:124-146 — no service category/version writer, no price-list assignment, no item/location writer), so no quotation line, part issue or invoice line can be produced without direct SQL; P1-30 Owner acceptance (W9) is outstanding; the three histories remain sectioned (INT-043/WF-19).
...
- **Delivery placement:** P1-30 corrective (F-02 writers, Backend lane remediation/p1-30-backend-*) then the Integration gate for the linked proof — the register assigns the end-to-end proof to the Integration gate. Placement corrected 2026-09-06: P1-30 W8/W9 acceptance for the commercial half; the Integration gate for the linked end-to-end proof (register line 36).
- **Dependencies:** P1-30 W9 acceptance; H-3/WF-19 for a unified history.
...
- **Remaining gap:** No role-oriented 'my next actions' surface or inbox; prerequisite explanations are per-screen and phase-local, not consistent across the journey; the reception→work-order→QC→billing→delivery handoffs are not surfaced as a single 'what is owed next'. Also a session finding not recorded in the repository: several navigation gates are unreachable by the first administrator's permission bundle (grounded 2026-09-06 against apps/web/src/config/navigation.ts and TENANT_ADMINISTRATOR_ROLE: tenant-bootstrap-corrective-slice.md:150-155 TB-R3 records the four gate codes outside the bundle (apt, iam.audit, rpt, shared) as an accepted residual) — a role that cannot reach a screen cannot be told its next action there.
...
- **Existing evidence:** None. The shipped control is a plain <input type="file"> with no capture attribute and no accept list unless the server's list is passed (apps/web/src/features/receptions/components/CaptureFileField.tsx:60-68); the bytes cross to a Server Action which performs the object PUT server-side (apps/web/src/features/attachments/api.ts:31-56, 269-274; evidence-capture.ts:96-200). A camera path is EXPLICITLY BANNED in the reception tree: apps/web/tests/p1-28-reception-media.test.ts:68-72 ('No camera path is sanctioned') and :897-915 (regex getUserMedia|getDisplayMedia|mediaDevices|ImageCapture|MediaStream|capture=|<video|<canvas asserted absent from every reception file); docs/phase-1/phase-1-28/media-capture-decision-record.md:85 (the neighbouring rows at :88-101) (§2.1 'A file input, drag target or camera control — absent') and :160-168 (the P1-27 gate covers no camera construct). grep getUserMedia/ImageCapture/tesseract/ocr across apps/web/src and apps/api/src: no matches.
- **Remaining gap:** The whole camera path, and the deliberate reversal of the test ban under this Owner direction; CSP img-src is 'self' data: only (apps/web/src/lib/security/csp.ts:156) so a blob: preview would be blocked; Permissions-Policy: apps/web/next.config.ts:56-59 sets `camera=(), microphone=(), geolocation=(), payment=()`, which denies getUserMedia to the document — a named blocker beside csp.ts:156 img-src for any live-preview camera path; whether `<input capture>` is affected is browser-dependent (UNVERIFIED); device policy (which devices/browsers at the desk) not established.
...
- **Dependencies:** Owner decision: approach and provider (commercial); H-9 candidate record; P1-27 #20 VIN verification (the reused row P1-27 #20); RMC-09 reading↔photo association; H-5 camera.
- **Acceptance criteria:** Capturing a VIN plate yields a candidate shown beside the image labelled 'unverified — confirm or correct' (a provider score is never displayed as accuracy — AI-9); confirming writes through the existing operations and their guards; nothing is written without confirmation; with the provider unavailable the manual path works unchanged.
...
- **Normalised behaviour:** Extraction runs server-side behind a provider-abstracted port reading the accepted document version from the store (never from the browser to a third-party API), returns candidates with raw text and, at most, an opaque sanitised provider detail that is stored but never displayed as accuracy (AI-9), and is recorded as an audited, tenant-scoped operation; on-device recognition, if ever used, still submits its candidate through the same server record.
...
- **Dependencies:** H-7 decision; STORAGE provider configured (already true for the acceptance launcher per apps/web/src/features/receptions/media/media-decision.ts:26-32). Area I's provider-neutral rules govern this port and step — AI-3 (egress allow-list), AI-4 (retention), AI-5 (untrusted candidates), AI-6 (tenant isolation), AI-7 (provider port), AI-8 (bounded consumption), AI-9 (no displayed accuracy); AI-2's use case is this one, named by the Owner here. AI-P1's port is this port — no second port name.
...
- **Dependencies:** H-7/H-8; the count pins a new migration owes (the CI structural baseline, validate:baseline-manifest and the tests/ci migration pins) (memory: schema baseline pins three values that move together).
...
- **Existing evidence:** Normalisation only: supabase/migrations/20260720090000_veh_normalization.sql:17-21, 43-60 (veh.normalize_vin uppercases and strips separators, deliberately preserves I/O/Q — 'never silently corrects'; format/checksum validation is a distinct concern for veh.vin_verifications); active-VIN uniqueness (vehicle-write.ts:7, 12-14); vehicle create/update accept any raw string up to 64 chars (vehicle-write.ts:26-28, 161-162, 196-199; vehicles/route.ts:98). The verification table exists — supabase/migrations/20260720094000_veh_vin_verifications.sql:37-83 (check_kind checksum|format|manual|external, result passed|failed|overridden, override_reason, INSERT policy for app_runtime :83) — but is read and written by no code (the only textual mention is in that test's prose, vin-create-validation.dom.test.tsx:35) and no seed (grep), live row count 0; reception-media-checklist.md:146-147 (RMC-17) and workflow :339 confirm no operation, no check-digit routine, no override policy, no permission code. Shipped web VIN check omitted above: apps/web/src/features/vehicles/components/VinField.tsx (P1-27-FE-020; verdicts nonStandardLength / available / duplicate / invalidFormat / checkUnavailable at :172-186; 'No 17-character rule … No check-digit arithmetic … No I/O/Q exclusion' at :51-57) with apps/web/tests/vin-create-validation.dom.test.tsx:11-36 — partial evidence for 'validate against applicable identifier rules' (length observation and uniqueness preview), which is why the register row reads Delivered.
...
- **Dependencies:** H-7, H-9, P1-27 #20 verification verdict, P1-27 #23 comparison. Area I's provider-neutral rules govern this port and step — AI-3 (egress allow-list), AI-4 (retention), AI-5 (untrusted candidates), AI-6 (tenant isolation), AI-7 (provider port), AI-8 (bounded consumption), AI-9 (no displayed accuracy); AI-2's use case is this one, named by the Owner here. AI-P1's port is this port — no second port name.
...
- **Existing evidence:** RLS enabled with tenant predicates: supabase/migrations/20260718100000_shared_document_categories_and_documents.sql:233-250 (sel_documents_tenant), 20260718101000:197-206 (versions, scan results), 20260718102000:127-130 (links); reception capture tables FORCE RLS (20260815100000:796-887); storage key server-built, no business data (apps/api/src/modules/shared-services/domain/storage-key.ts (a sibling of attachment-policy.ts) per checklist §5.6); browser never PUTs or holds a signed URL (attachments api.ts:31-56, 269-274); download only for accepted versions with TTL (download-authorizations/route.ts:4, 26-28; checklist §4.6 rows 1–6); real S3 adapter + scanner in place (modules/shared-services/provider/s3-storage-provider.ts; storage-round-trip.test.ts; apps/web/src/features/receptions/media/media-decision.ts:26-32); shared.document.read seeded (seeds/04:83) and used by shared.document-version-read (:12-17) and shared.document-category-list (:9-14).
- **Remaining gap:** (a) No media VIEWER anywhere in the web — shared.attachment-download-authorize is registered in apps/web/src/lib/api/idempotent-operations.ts and cited by features/vehicles/components/VehicleDocumentsSection.tsx:93-97 and documents-contract.ts:43 to explain that NO download control is rendered (auditClass security) and rendered by no screen (grep img/downloadUrl in features: none); the wizard shows version state words, never the image. (b) shared.document-read, shared.attachment-download-authorize and veh.vehicle-document-list are still gated on the WRITE code shared.document.manage (routes :22, :27, :32) — RMC-07 only half closed. (c) RMC-04 token mismatch persists: vehicle read asks 'veh.vehicle' (modules/vehicle/domain/vehicle-history.ts:22) while links accept 'veh.vehicles' (attachment-policy.ts:42-52). (d) /documents navigation entry is status planned (navigation.ts:422-430). (e) Whether reception media is restricted data needing iam.sensitive.view is not established (checklist §9). Status precision: Delivered, with a stated gap — the three reads (shared.document-read attachments/documents/[documentId]/route.ts:22, shared.attachment-download-authorize …/download-authorizations/route.ts:27, veh.vehicle-document-list vehicles/[vehicleId]/documents/route.ts:32) are gated on the WRITE code shared.document.manage (RMC-07 half-open); the re-gate is H-14 (P1-15 lane).
...
- **Unverified:** Resolved 2026-09-06: FORCE ROW LEVEL SECURITY is in effect on shared.documents, document_versions, document_links and file_scan_results (live catalogue relforcerowsecurity = true on all four); the literal is not in the three base migrations, so cite the catalogue.
...
- **Existing evidence:** This entry (lines H-5…H-14) is the comparison. The earlier exclusions being superseded: media-capture-decision-record.md:85 (the neighbouring rows at :88-101) (§2.1 — no camera control, no disabled control), apps/web/tests/p1-28-reception-media.test.ts:68-72 and :897-915 (camera ban), CaptureFileField.tsx:60-68 (file chooser only). What is NOT superseded: the no-invented-media-limit and no-upload-path rules (canonical-plan.md:263-270) and the single-sanctioned-input rule — a camera path must still read its policy from the server.
...
- **Acceptance criteria:** The register carries this line with status and owner; no AI call site exists on develop until AI-2's evaluation is recorded; grep of apps/ for an AI SDK stays empty until the follow-on branch opens. Scope the grep to tracked source and package manifests (apps/_/src, apps/_/package.json): this register carries the word 'rewards', inspection-templates/[templateId]/versions/route.ts:22 carries 'incentive' in a docblock, and .next-dev caches match openai|gpt.
...
- **Existing evidence:** The evaluation pattern exists for other providers: docs/product/vehicle-catalogue/provider-evaluation.md §4 axes (lines 246-265), §10 Owner questions, §13 spike rules (heading :588) — 'Nothing the spike produces enters the product database' (line 606), 'No account is created and no terms are accepted' (line 610). Candidate use cases the journey already exposes, none chosen: dashboard photo → odometer value (DSH-1, docs/product/workshop/reception-media-checklist.md:93,122-124; RMC-09 line 491 — the reading cannot yet be bound to its photograph); VIN/chassis plate photo (IDN-1, lines 96,143-144); fault-code plain-language description (INS-03, docs/product/workshop/inspection-and-diagnostics.md:830 — description is free text, no catalogue; the deterministic alternative is a licensed fault-code dictionary, line 869); VIN decode (VS-07, provider-evaluation.md:223; decodeVin is 'a proposal, never a fact', docs/product/vehicle-catalogue/catalogue-architecture.md:578,593).
- **Remaining gap:** No evaluation document exists for any AI use case; the Owner has already named a use case — assisted VIN/odometer extraction (Area H, H-7); what is missing is the EVALUATION; every image-based candidate is blocked on P1-OD-025 and on an object store; the fault-code candidate competes with a deterministic licensed dictionary the docs already recommend evaluating.
...
- **Delivery placement:** Owner-commissioned evaluation first (planning work, no code), then the named follow-on. Never P1-30 corrective. Reconciled with Area H 2026-09-06: H-7..H-11 own the VIN/odometer extraction as a P1-28 corrective slice with Backend prerequisites; this area's AI-3..AI-9 are the provider-neutral rules that slice must satisfy (H-8 and H-11 cite them); no second port name is proposed — AI-P1's port is H-8's port.
...
### OWR-2026-09-06-AI-3 — Owner requirement · **Undecided**
...
- **Remaining gap:** No AI port, no adapter, no AI_* config keys, no deterministic local double; no outbound-egress allow-list exists yet, but two standing security lines already name the obligation: docs/security/owasp-api-top-10-2023-matrix.md:35 (API7:2023 SSRF, RL-API-API7-2023, Planned — 'the rule lands with the first integration'; its own 'no outbound-fetch code exists' is stale since iam/provider/supabase-provider.ts fetches) and :38 (API10:2023 unsafe consumption, RL-API-API10-2023).
- **Owning module:** shared-services (provider port + unconfigured default + local double); server/config (env keys); iam untouched
- **Delivery placement:** Named follow-on, first slice: the port and the refusing default, before any adapter — the order provider-evaluation.md:576 (VDP-05 row; :548 is the §11 port/adapter row saying the same) prescribes ('Build the port before any adapter'). (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.)
...
### OWR-2026-09-06-AI-4 — Owner requirement · **Undecided**
...
- **Delivery placement:** Named follow-on, same slice as the port (AI-3); the classification rule is a documentation change any Backend lane may carry once the Owner approves the class list. (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.) Lane named: the IAM Backend lane carries the documentation change.
...
### OWR-2026-09-06-AI-7 — Owner requirement · **Undecided**
...
- **Delivery placement:** Named follow-on, first slice with the port. (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.)
...
### OWR-2026-09-06-AI-8 — Owner requirement · **Undecided**
...
- **Delivery placement:** Named follow-on; schema change through change control (docs/product/README.md §5 steps 1-7), never inside P1-30. (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.)
...
### OWR-2026-09-06-AI-9 — Owner requirement · **Undecided**
...
- **Delivery placement:** Named follow-on, same slice as AI-8. (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.)
...
- **Unverified:** Resolved 2026-09-06: 'confidence' is not in scripts/ci/check-plain-language.mjs.
...
- **Delivery placement:** P1-30 (existing row) for the handoff read/export contract via a remediation/p1-30-backend-* branch if the Owner keeps it in P1-30; otherwise P1-31 Reporting. The full accounting domain is NOT here — see OWR-2026-09-06-AI-11. Placement corrected 2026-09-06: a financial-events read/export is a NEW operation no A0 seam names, so remediation/p1-30-backend-* is not its lane (canonical-plan §1.4 :63-65, §2 :71-73; the profile's why, check-phase-ownership.mjs:582-585); the P1-30/P1-31 billing split is recorded as not established (docs/product/README.md §6). Placement is an Owner decision through README §5 change control; until then the named follow-on 'accounting handoff' (the register's follow-on section).
...
- **Unverified:** Resolved 2026-09-06: no code reads sal.financial_events (grep 'FROM sal.financial_events' in apps/api/src: none — the billing/payments references are writers and docblocks); the a0 matrix names no financial-event or export seam; the only export/report operations are shared.export-authorize, shared.export-catalogue, rpt.report-catalogue and rpt.report-read; sal.finance.view IS in the administrator bundle (bootstrap-roles.ts:218).
...
- **Existing evidence:** The deferral is recorded, the commitment is not: phase-1-11-no-general-ledger-boundary.md:34 ('the entire FIN general-ledger domain' deferred); phase-1-12-owner-gate.md:401-413 and phase-1-13/14 precondition reports repeat 'no general ledger'; the OpenAPI title still calls the product a 'CRM and ERP Platform' (docs/api/openapi.v1.json:4). No accounting module directory exists under apps/api/src/modules (listing: billing, crm, delivery, diagnostics, iam, inventory, meta, payments, platform, pricing, quality, quotation, reception, reporting, service-catalog, shared-services, technician, vehicle, warranty, work-order). The P1-13 repetition is docs/phase-1/phase-1-13/evidence/local-and-clean-room-validation.md:86 (an evidence file, not the precondition report); P1-14's is docs/phase-1/phase-1-14/phase-1-14-precondition-report.md:158.
...
- **Acceptance criteria:** Register row exists with a one-sentence definition, owner, and acceptance sentences; until then grep for reward|loyalty in apps/ and supabase/ stays empty. Scope the grep to tracked source and package manifests (apps/_/src, apps/_/package.json): this register carries the word 'rewards', inspection-templates/[templateId]/versions/route.ts:22 carries 'incentive' in a docblock, and .next-dev caches match openai|gpt.
...
- **Unverified:** Resolved 2026-09-06: the p1-30-backend profile (scripts/ci/check-phase-ownership.mjs:581-611) allows apiSource, migrations, dbSeeds, webGenerated, docs, tooling, tests and rootConfig and forbids web, webContract, apiConfig and supabase; its `why` (:582-585) limits it to the read seams A0 proved missing, one branch per seam.

### OWR-2026-09-06-AI-P1 — Proposed implementation policy · **Undecided**
...
- **Existing evidence:** Pattern to copy verbatim: message-provider.ts (port, UnconfiguredMessageProvider, LocalMessageProvider behaviours accept|timeout|outage|reject), identity-provider.ts (ProviderFailure reasons, retryable), backend-config.ts:226-231 (regex-constrained provider code, bounded timeout), provider-evaluation.md:576 (VDP-05 row; :548 is the §11 port/adapter row saying the same) ('Build the port before any adapter').
- **Remaining gap:** Not built; naming and method shape wait on the use case.
- **Owning module:** shared-services/provider; server/config
- **Delivery placement:** Follow-on first slice. (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.)
...
### OWR-2026-09-06-AI-P2 — Proposed implementation policy · **Undecided**
...
- **Delivery placement:** Follow-on, via a Backend change request; never P1-30. (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.)
...
- **Dependencies:** P1-OD-025; storage provider decision; Owner ranking. AI-2 defers to H-7: the Owner named assisted VIN/odometer extraction; 'AI-assisted capture' survives as a separate follow-on only if a second use case (INS-03 fault-code description) is chosen.
...
### OWR-2026-09-06-AI-P5 — Proposed implementation policy · **Undecided**
...
- **Delivery placement:** Follow-on second slice (after the port and the candidate row). (Status Undecided until the Owner names the follow-on and the use case — or these rules are re-homed under Area H's lanes, where H-8/H-9/H-11 must satisfy them.)
...
- **Existing evidence:** supabase/migrations/20260718090000_iam_user_accounts_and_profiles.sql:193-194 (the comment); docs/phase-1/phase-1-9/README.md:12 and 20260722094000:64-66 (P1-9 built operational profiles only).
...
- **Delivery placement:** the IAM Backend lane (P1-04 owns the iam.user_employee_links schema; the retention standard is a docs/database artefact) — a COMMENT-only migration moves the pinned migration count (CI structural baseline, validate:baseline-manifest); it is documentation-only in effect but migration-borne in mechanism.
...
### OWR-2026-09-06-AI-P7 — Proposed implementation policy · **Delivered**
...
- **Remaining gap:** Section does not exist; owners and acceptance criteria are the Owner's to state. Register line citations hold at develop 6f6236c3; b66a6642 inserted the nine-area section at :294 (histories :335, catalogue :356). The 'Follow-on commitments' section is added to the register on 2026-09-06 by this correction — Delivered.
