...
- **Remaining gap:** No org.company-create or org.branch-create operation exists anywhere (only GET on org/companies/route.ts:49 and org/branches/route.ts:37; POST routes absent) — a tenant can never grow beyond its first company/branch through the product; org.company.manage/org.branch.manage are seeded and inert for creation (scope.md:49 — stale: org.company.manage gates org.company-update (org/companies/[companyId]/route.ts:58) and org.company-status-set (…/status/route.ts:59), org.branch.manage gates org.branch-update (org/branches/[branchId]/route.ts:63); no CREATE operation exists (org/companies/route.ts:52 and org/branches/route.ts:40 are GET-only); and the administrator bundle deliberately EXCLUDES both codes (bootstrap-roles.ts:62-63), so a fresh First Owner cannot update a company or branch until it maps them itself (CC-13) G-4); no platform-side cross-tenant read of companies/branches after provisioning; no UI on the platform side.
