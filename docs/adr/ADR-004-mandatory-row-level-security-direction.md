...
> **Status update (2026-07-17) — this gate has since been satisfied for the Phase 1-2 scope.** The paragraph above, and the statements elsewhere in this record that Phase 1-2 has not started and that no RLS test exists, describe the position on 2026-07-16 and are retained as the historical record rather than rewritten. Phase 1-2 has since implemented RLS — enabled **and forced**, default-deny — on the only tenant-owned table it created (`shared.number_sequences`), with negative-path isolation tests executed as a **non-owner runtime role** and recorded in the [Phase 1-2 evidence register](../phase-1/phase-1-2/phase-1-2-evidence-register.md); the automated check called for under Consequences now exists (`tests/db/foundation.test.ts` asserts every module-schema table has RLS enabled and forced). The gate recorded **Go — Technical Gate Passed** on 2026-07-17 ([gate record](../phase-1/phase-1-2/phase-1-2-owner-gate.md)). The obligation itself is unchanged and binds every later phase: each new tenant-owned table lands with RLS enabled, forced, and tested. This evidence is owner-authorized self-review, not independent verification; P1-EC-016 remains open.
