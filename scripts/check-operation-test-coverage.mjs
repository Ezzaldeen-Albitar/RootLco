...
export const MANIFEST = {
  ...
  'rec.reception-list': {
    files: ['tests/backend/p1-27-reception-reads.test.ts'],
    required: ['success', 'denial', 'isolation'],
    note: 'P1-27-INT-011. No branch board and no per-vehicle visit list existed. Company and branch are required and travel as the authorization target so the scope-blind pre-handler evaluation never decides alone (P1-18-A-01); recordVersion travels per row because the guarded writes are addressed from the list. Owner directive P1-32-PRE-OD-UX: branchId is now OPTIONAL beside companyId — omitted, the page covers the branches of that company the caller actually holds this operation codes in, resolved one branch at a time by resolveAuthorizedBranches, because iam.has_permission_in_scope company arm matches only a company-typed grant scope row and would therefore refuse the branch-scoped operator the feature exists for. A caller with no such branch is REFUSED rather than answered with an empty page, a named branch is decided exactly as before, and row-level security still bounds every row underneath. branchId joined the row for the same reason: a page that may span branches has to name each row own.',
  },
  ...
  'apt.appointment-list': {
    ...
    required: ['success', 'denial', 'isolation'],
    note: 'P1-27-INT-019. All four apt. operations were writes; no calendar existed. The date filter and ordering run over the CONFIRMED window falling back to requested (COALESCE), per the P1-8 boundary record; recordVersion travels per row for the three guarded lifecycle commands. Owner directive P1-32-PRE-OD-UX: branchId is now OPTIONAL beside companyId — omitted, the page covers the branches of that company the caller actually holds this operation codes in, resolved one branch at a time by resolveAuthorizedBranches, because iam.has_permission_in_scope company arm matches only a company-typed grant scope row and would therefore refuse the branch-scoped operator the feature exists for. A caller with no such branch is REFUSED rather than answered with an empty page, a named branch is decided exactly as before, and row-level security still bounds every row underneath. branchId joined the row for the same reason: a calendar that may span branches has to name each slot own.',
  },
