#!/usr/bin/env node
/**
 * P1-09-SEC-003 / P1-09-DO-001 — Work Order / Technician / Diagnostics / Quality
 * personal-data classification guard.
 *
 * Single canonical implementation, run identically locally
 * (`npm run validate:wo-tech-dia-qms-classification`) and in the CI "Database
 * migrations and RLS tests" job. It introspects the live `wo`, `tech`, `dia`, and
 * `qms` schemas and reconciles them against
 * docs/database/wo-tech-dia-qms-personal-data-classification.json. It FAILS when:
...
function fail(msg, items) {
  console.error(`✖ WO/TECH/DIA/QMS classification guard FAILED: ${msg}`);
...
try {
  const { rows } = await client.query(
    `SELECT table_schema || '.' || table_name || '.' || column_name AS col, data_type
      ...
      WHERE table_schema IN ('wo', 'tech', 'dia', 'qms')
      ORDER BY 1`
  );
  dbRows = rows;
} finally {
...
if (missing.length) {
  fail(`${missing.length} wo/tech/dia/qms column(s) have no classification entry`, missing);
}
...
console.log(
  `✔ WO/TECH/DIA/QMS classification guard OK: ${dbSet.size} columns all classified ` +
