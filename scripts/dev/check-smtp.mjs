 ...
 * and transmitted byte for byte, whatever it contains and however long it is. A
 ...
 *
 * That neutrality is the reason a change of provider costs this file nothing.
 ...
 *
 * ## The four stages, and which one each mode reaches
 *
 * Outbound mail is proven in four separate stages. They are separate because a
 * pass at one is no evidence at all for the next, and reporting them as one
 * result is how "the relay accepted it" becomes "the mail arrived":
 *
 *   Stage 1  SMTP AUTHENTICATION  the relay accepts the credential.
 *   Stage 2  RELAY ACCEPTANCE     the relay accepts one message for delivery.
 *   Stage 3  INBOX RECEIPT        that message is seen in the destination
 *                                 mailbox. Acceptance is not arrival.
 *   Stage 4  APPLICATION FLOWS    a recovery and an invitation generated by the
 *                                 application complete end to end.
 *
 * This script reaches stages 1 and 2 and NOTHING FURTHER. Stage 3 is a human
 * looking in a mailbox; there is no mode for it and no IMAP client here, because
 * reading a mailbox is not needed to send mail. Stage 4 needs `enabled = true`
 * in `supabase/config.toml` and a restarted stack, so it is not something a
 * script that touches no container can measure.
 *
 * ## Four modes, one stage each, only one of which sends mail
 *
 * The modes are graded by what they risk. Exactly one must be chosen, and each
 * maps to exactly one stage — or, for the first two, to no stage at all:
 *
 *   --settings      NO STAGE. A precondition check: reads the environment and
 *                   prints names and booleans. No socket, no credential, no
 *                   network, and no value of any kind — not even a non-secret
 *                   one. It proves that the six names are present and mutually
 *                   consistent, which is a statement about this machine and
 *                   about no relay. Run it before activating the relay in
 *                   `supabase/config.toml`.
 *   --probe         NO STAGE. Reachability only: opens the connection,
 *                   negotiates encryption, prints the AUTH mechanisms the relay
 *                   advertises, and quits. NO credential is transmitted, so this
 *                   is safe against a relay whose lockout policy is unknown —
 *                   and, for the same reason, it says nothing about stage 1.
 *   --authenticate  STAGE 1, and only stage 1. Negotiates encryption and
 *                   authenticates, then quits without an envelope. SMTP permits
 *                   this: AUTH is a session command and QUIT after it is a
 *                   complete, legal session. It proves the password and nothing
 *                   about delivery.
 *   --send --to <address>
 *                   STAGE 2, which subsumes stage 1: the above, plus one real
 *                   message to one real recipient. The recipient has no default
 *                   and must be named on the command line, so mail is never sent
 *                   to an address this script chose. The message is sent AS
 *                   `SMTP_ADMIN_EMAIL`, not as `SMTP_USER`: those are two
 *                   identities, and the one the auth service will present is the
 *                   former. A relay that lets the mailbox log in but refuses
 *                   that sender address is a real failure mode, and this mode is
 *                   where it surfaces. A 250 here is the relay taking custody of
 *                   the message; stage 3 is a different observation.
 *
...
const USAGE = [
  'Choose exactly one mode. Each reaches exactly one stage, or none:',
  '  --settings                    no stage: names and booleans only; no socket, no credential',
  '  --probe                       no stage: connect and list AUTH mechanisms; sends no credential',
  '  --authenticate                stage 1 (authentication): connect, authenticate, quit; no mail',
  '  --send --to <address>         stage 2 (relay acceptance): the above, plus one real message',
  '',
  'Stage 3 (inbox receipt) is confirmed by looking in the destination mailbox, and',
  'stage 4 (application recovery and invitation flows) needs the relay activated in',
  'supabase/config.toml and the stack restarted. This script reaches neither.',
  '',
];
...
export function reportSettings(environment, write) {
  ...
  write('This mode reaches NO stage: it opens no socket, so it is a statement\n');
  write('about this machine and about no relay.\n\n');
  ...
  write('\n  Stages 1 (authentication), 2 (relay acceptance), 3 (inbox receipt)\n');
  write('  and 4 (application flows) are all UNPROVEN by this mode.\n');
  return ready;
}
...
export async function run({
  ...
  const STAGE_OF_MODE = {
    probe: 'none — reachability only, no credential transmitted',
    authenticate: '1 of 4 — SMTP authentication',
    send: '2 of 4 — relay acceptance (subsumes stage 1)',
  };

  out(`Relay    : ${host}:${port}\n`);
  out(`Mode     : ${mode}\n`);
  out(`Stage    : ${STAGE_OF_MODE[mode]}\n`);
  ...
  try {
    ...
    if (mode === 'probe') {
      ...
      out('This mode reaches NO stage. The mechanisms above are what the relay\n');
      out('offers, not a verdict on the credential: stage 1 (authentication)\n');
      out('stays UNPROVEN, as do stages 2, 3 and 4.\n');
      return 0;
    }
    ...
    if (mode === 'authenticate') {
      out('\nSTAGE 1 (authentication) PROVEN: the relay accepted the credential.\n');
      out('Nothing was sent, so this says nothing about whether a message would\n');
      out('be delivered. Stages 2 (relay acceptance), 3 (inbox receipt) and 4\n');
      out('(application flows) remain UNPROVEN.\n');
      return 0;
    }
    ...
    out(
      '\nSTAGES 1 (authentication) and 2 (relay acceptance) PROVEN: the relay\n' +
        'accepted the message for delivery. Acceptance is not arrival, so STAGE 3\n' +
        '(inbox receipt) is still UNPROVEN — confirm in the recipient mailbox, and\n' +
        'watch the sending mailbox for a delivery-failure notice. STAGE 4\n' +
        '(application recovery and invitation flows) is a separate measurement\n' +
        'that needs the relay activated and the stack restarted.\n'
    );
    return 0;
  } catch (error) {
