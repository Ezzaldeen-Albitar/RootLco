#!/usr/bin/env node
/**
 * P1-08-SEC-003 / P1-08-DO-001 — Appointment + Reception personal-data guard.
 *
 * Single canonical implementation, run identically locally
 * (`npm run validate:aptrec-classification`) and in the CI "Database migrations
 * and RLS tests" job. It introspects the live `apt` and `rec` schemas and
 * reconciles them against docs/database/apt-rec-personal-data-classification.json.
 * It FAILS when:
 *   - an apt/rec column has no classification entry (new unclassified column);
...
// APTREC_CLASSIFICATION_REGISTRY is a TEST-ONLY override so the guard's failure
// modes can be exercised against controlled negative fixtures. CI and local
// validation always use the committed default path.
const REGISTRY =
  process.env.APTREC_CLASSIFICATION_REGISTRY ??
  join(HERE, '..', 'docs', 'database', 'apt-rec-personal-data-classification.json');
...
function fail(msg, items) {
  console.error(`✖ APT/REC classification guard FAILED: ${msg}`);
...
if (missing.length) {
  fail(`${missing.length} apt/rec column(s) have no classification entry`, missing);
}
...
console.log(
  `✔ APT/REC classification guard OK: ${dbSet.size} apt/rec columns all classified ` +
