 ...
 * docs/database/sal-wty-rpt-personal-data-classification.json. It FAILS when:
...
// SAL_CLASSIFICATION_REGISTRY is a TEST-ONLY override so the guard's failure modes
// can be exercised against controlled negative fixtures. CI and local validation
// always use the committed default path.
const REGISTRY =
  process.env.SAL_CLASSIFICATION_REGISTRY ??
  join(HERE, '..', 'docs', 'database', 'sal-wty-rpt-personal-data-classification.json');
...
function fail(msg, items) {
  console.error(`✖ SAL/WTY/RPT classification guard FAILED: ${msg}`);
...
try {
  const { rows } = await client.query(
    `SELECT table_schema || '.' || table_name || '.' || column_name AS col, data_type
      ...
      WHERE table_schema IN ('sal', 'wty', 'rpt')
      ORDER BY 1`
  );
  dbRows = rows;
} finally {
...
if (missing.length) {
  fail(`${missing.length} sal/wty/rpt column(s) have no classification entry`, missing);
}
...
console.log(
  `✔ SAL/WTY/RPT classification guard OK: ${dbSet.size} columns all classified ` +
